...
/**
 * BlobStore implementation backed by the local filesystem. Intended for local development only — no
 * pre-signed URLs, no lifecycle policies.
 *
 * <p>Layout: {basePath}/{bucket}/{hash[0:2]}/{hash} Same content-addressing as
 * S3CompatibleBlobStore, so BlobRefs are portable.
 */
public class FilesystemBlobStore implements BlobStore {
  ...
  private Path resolve(String bucket, String key) {
    return basePath
        .resolve(bucket)
        .resolve(key.replace("/", FileSystems.getDefault().getSeparator()));
  }
