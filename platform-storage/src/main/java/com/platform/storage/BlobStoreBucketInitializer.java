...
/**
 * Creates required buckets on startup if they don't already exist. Also applies lifecycle rules and
 * CORS policies.
 *
 * <p>Registered as a {@code @Bean} in {@link BlobStoreAutoConfiguration} so it runs in every
 * service that depends on platform-storage — no component-scan of {@code com.platform.storage}
 * required. Only active when an {@code S3Client} bean is present (i.e. storage type is {@code
 * minio} or {@code s3}).
 */
public class BlobStoreBucketInitializer {
  ...
  private static final List<String> ALL_BUCKETS =
      List.of(
  ...
  private void tryApplyTraceCors() {
    try {
      ...
      log.warn(
          "Could not apply CORS to bucket {} (status={}) — "
  ...
  private Set<String> existingBuckets() {
    return s3.listBuckets().buckets().stream().map(Bucket::name).collect(Collectors.toSet());
  }
  ...
  private void tryApplyLifecycle(String bucket, int expiryDays) {
    try {
      ...
      log.warn(
          "Could not apply lifecycle policy to bucket {} (status={}) — "
              + "objects will not expire automatically",
          bucket,
          e.statusCode());
    }
  }
