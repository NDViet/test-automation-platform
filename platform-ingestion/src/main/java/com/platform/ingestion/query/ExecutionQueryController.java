...
public class ExecutionQueryController {
  ...
  public List<ExecutionSummaryDto> listByProject(
      @PathVariable UUID projectId, @RequestParam(defaultValue = "20") int limit) {
  ...
  /**
   * Links an automated execution to a sprint / area after the fact, so it appears in sprint reports
   * without re-running or modifying the original CI execution.
   *
