...
/**
 * Hierarchy of a requirement: its parent (if any) and its direct children — used by the
 * detail panel to surface work-item relationships (e.g. ADO parent/child links).
 */
public record RequirementRelationsDto(Ref parent, List<Ref> children) {
    ...
    public record Ref(UUID id, String externalId, String title, String issueType,
                      String status, int depth, String sourceUrl) {
        public static Ref from(PlatformRequirement r, String sourceUrlBase) {
            return new Ref(r.getId(), r.getExternalId(), r.getTitle(), r.getIssueType(),
                    r.getStatus(), r.getDepth(), sourceUrl(sourceUrlBase, r.getExternalId()));
        }
