...
public class KafkaConfig {
  ...
  public NewTopic testResultsAnalyzedTopic() {
    return TopicBuilder.name(Topics.TEST_RESULTS_ANALYZED).partitions(6).replicas(1).build();
  }

  @Bean
  public NewTopic flakinessEventsTopic() {
    return TopicBuilder.name(Topics.FLAKINESS_EVENTS).partitions(3).replicas(1).build();
  }

  @Bean
  public NewTopic integrationCommandsTopic() {
    return TopicBuilder.name(Topics.INTEGRATION_COMMANDS).partitions(3).replicas(1).build();
  }

  @Bean
  public NewTopic alertEventsTopic() {
    return TopicBuilder.name(Topics.ALERT_EVENTS).partitions(3).replicas(1).build();
  }
}
