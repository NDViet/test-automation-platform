...
public class TestExecutionMonitorController {
    ...
    /** Primary view: releases grouped by Team with pass-rate + coverage; scoped by area/team/iteration. */
    ...
    public TestExecutionMonitorService.MonitorOverview byRelease(@PathVariable UUID projectId,
            @RequestParam(required = false) String area, @RequestParam(required = false) String team,
            @RequestParam(required = false) String iteration) {
        return service.byDimension(projectId, Dimension.RELEASE, area, team, iteration);
    }

    @GetMapping("/by-sprint")
    public TestExecutionMonitorService.MonitorOverview bySprint(@PathVariable UUID projectId,
            @RequestParam(required = false) String area, @RequestParam(required = false) String team,
            @RequestParam(required = false) String iteration) {
    ...
    @GetMapping("/by-area")
    public TestExecutionMonitorService.MonitorOverview byArea(@PathVariable UUID projectId,
            @RequestParam(required = false) String area, @RequestParam(required = false) String team,
            @RequestParam(required = false) String iteration) {
    ...
    @GetMapping("/by-team")
    public TestExecutionMonitorService.MonitorOverview byTeam(@PathVariable UUID projectId,
            @RequestParam(required = false) String area, @RequestParam(required = false) String team,
            @RequestParam(required = false) String iteration) {
