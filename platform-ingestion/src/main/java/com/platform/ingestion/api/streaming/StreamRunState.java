 ...
 *
 * Test case results are now persisted to the DB immediately on each {@code pushTest()} call,
 * so this class only needs to track metadata (for the Kafka event at finishRun) and live
 * counters (to compute final totals without an extra DB round-trip).
 *
 * Thread-safe: parallel Playwright workers can push results concurrently.
 */
public class StreamRunState {
    ...
    public StreamRunState(String runId, String orgSlug, String projectSlug, String teamSlug,
                          String areaSlug, String iterationPath, String branch, String environment,
                          String commitSha, String ciRunUrl,
                          String ciProvider, String workflow, String trigger) {
        this.runId         = runId;
    ...
    public Instant getLastActivityAt() { return lastActivityAt.get(); }
    ...
    public int     getBroken()      { return broken.get(); }
}
