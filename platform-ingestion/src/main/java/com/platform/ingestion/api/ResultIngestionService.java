...
public class ResultIngestionService {
  ...
  public ResultIngestionService(
      ResultParserFactory parserFactory,
  ...
  public IngestResponse ingest(IngestRequest request, List<MultipartFile> files)
      throws IOException {
    ...
    for (UnifiedTestResult result : results) {
      ...
      if (result.testCases() != null) {
        coverageService.ingestFromResults(execution.getProject().getId(), result.testCases());
      }
    }
    ...
    log.info(
        "Ingested runId={} format={} teamId={} total={} mode={} parallelism={}",
        runId,
        request.format(),
        request.teamId(),
        totalTests,
    ...
    return new IngestResponse(runId, "ACCEPTED", totalTests, "/api/v1/executions/" + runId);
  }
