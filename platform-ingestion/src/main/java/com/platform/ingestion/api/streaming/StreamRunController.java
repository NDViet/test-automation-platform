 ...
 * </pre>
 *
 * <p>Auth: same X-API-Key header as the batch ingest endpoint.</p>
 */
@RestController
@RequestMapping("/api/v1/stream")
@Tag(name = "Streaming Ingestion", description = "Real-time test result streaming from Playwright / Node runners")
public class StreamRunController {
    ...
    public PushTestResponse pushTest(
            ...
            @RequestBody TestEventRequest request) {
        try {
    ...
    public record TestEventRequest(
            ...
            String browser,
            /**
             * Non-tag Playwright annotations: fixme, slow, fail, skip, and user-defined.
             * Each map has "type" and optional "description" keys.
             */
            ...
            /**
             * Arbitrary key-value labels for filtering and ownership.
             * Each map has "key" and "value" entries.
             */
