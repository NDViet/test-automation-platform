...
/**
 * CRUD + test-connection for scoped, encrypted {@link IntegrationCredential}s (the centralized
 * "Admin PAT" + per-scope overrides). Secrets are encrypted at rest via {@link CredentialCipher}
...
public class CredentialService {
  ...
  /**
   * Lists credentials at a scope. ADO-first: every scope is id-keyed — ORG=organization id,
   * PROJECT=project id, TEAM=team id — so scopeId is required.
   */
  ...
  public CredentialDto save(SaveCredentialRequest req, String actor) {
    ...
    IntegrationCredential existing =
        repo.findByScopeAndScopeIdAndIntegrationType(scope.name(), scopeId, req.integrationType())
            .orElse(null);

    String secretCiphertext =
        encryptIfPresent(req.secret(), existing == null ? null : existing.getSecretCiphertext());
    ...
    if (existing == null) {
      IntegrationCredential c =
          new IntegrationCredential(
              ...
              req.displayName(),
              req.baseUrl(),
              nullToEmpty(req.connectionParams()),
              secretCiphertext);
      ...
      if (req.syncIntervalMinutes() != null)
        existing.setSyncIntervalMinutes(req.syncIntervalMinutes());
  ...
  private static <T> T required(T value, String name) {
    if (value == null)
      throw badRequest(
          name + " is required (ORG=organization id, PROJECT=project id, TEAM=team id)");
    return value;
  }
