...
/**
 * Manages mapping-rule overrides at ORG and PROJECT scope. Save validates the JSON
 * against the {@link MappingRules} schema; reset deletes the override so resolution
 * falls back to the parent (PROJECT → ORG → built-in default).
 */
@Service
@Transactional
public class MappingRulesService {
    ...
    public MappingRulesService(MappingRulesetRepository repo, MappingRulesProvider provider,
    ...
    public MappingRulesetView getOrg(UUID orgId) {
        ...
        return repo.findByScopeAndScopeId(Scope.ORG.name(), orgId)
                .map(rs -> new MappingRulesetView("ORG", true, "ORG", rs.getRulesJson(), rs.getUpdatedBy(), rs.getUpdatedAt()))
                .orElseGet(() -> new MappingRulesetView("ORG", false, "DEFAULT", provider.defaultJson(), null, null));
    }
    ...
    public MappingRulesetView getProject(UUID projectId) {
        ...
        if (own.isPresent()) {
            var rs = own.get();
            return new MappingRulesetView("PROJECT", true, "PROJECT", rs.getRulesJson(), rs.getUpdatedBy(), rs.getUpdatedAt());
        }
        // Inheriting — show what is currently effective (ORG override or built-in default).
        UUID orgId = projectRepo.findById(projectId)
                .map(p -> p.getOrganization().getId()).orElse(null);
    ...
    private MappingRuleset upsert(Scope scope, UUID scopeId, String json, String actor) {
        ...
        return repo.findByScopeAndScopeId(scope.name(), scopeId)
                .map(rs -> { rs.setRulesJson(normalized); rs.setUpdatedBy(actor); return repo.save(rs); })
    ...
    private record ViewFrom(MappingRuleset rs) {
        MappingRulesetView asOrg()     { return view("ORG"); }
        MappingRulesetView asProject() { return view("PROJECT"); }
        private MappingRulesetView view(String scope) {
            return new MappingRulesetView(scope, true, scope, rs.getRulesJson(), rs.getUpdatedBy(), rs.getUpdatedAt());
        }
    }
}
