...
/**
 * Periodically refreshes the GitHub repo cache for credentials that have a
 * positive {@code sync_interval_minutes} value. Checks every 60 seconds and
 * triggers a sync when {@code lastSyncedAt + interval < now}.
 */
@Component
@Configuration
@EnableScheduling
public class GitHubSyncScheduler {
    ...
    public GitHubSyncScheduler(IntegrationCredentialRepository credRepo,
    ...
    public void checkAndSync() {
        ...
        for (IntegrationCredential cred : candidates) {
            ...
            Instant due = cacheRepo.findFirstByCredentialIdOrderBySyncedAtDesc(cred.getId())
                    .map(GitHubRepoCache::getSyncedAt)
                    .map(t -> t.plusSeconds(intervalSeconds))
                    .orElse(Instant.EPOCH);  // never synced → sync now

            if (due.isBefore(now)) {
                log.info("[GitHubSync] Auto-syncing credential {} (interval={}m)",
                        cred.getId(), cred.getSyncIntervalMinutes());
                try {
