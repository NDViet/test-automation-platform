...
public class TestCaseManagementService {
  ...
  public void setCaseSuites(UUID projectId, UUID caseId, List<String> suiteIds) {
    List<UUID> ids =
        suiteIds == null ? List.of() : suiteIds.stream().map(UUID::fromString).toList();
    suiteResolver.setCaseSuites(projectId, caseId, ids);
  }

  /**
   * Scope-filtered, searchable picker for run creation. Narrows test cases by the monitoring scope
   * (area / iteration / team — resolved through each case's LINKED REQUIREMENTS) and a free-text
   * query that matches the test-case title, the test-case external id, or any linked requirement's
   * external id.
   *
   * <p>Necessary because the flat case list does not scale: this lets a user pick the relevant
   * subset for a Sprint/Area/Team run without scrolling thousands of cases.
   */
  @Transactional(readOnly = true)
  public List<SelectableTestCaseDto> selectable(
      UUID projectId, String status, String area, String iteration, String teamId, String q) {
  ...
  public List<ManagedTestCaseDto> list(
      UUID projectId, String status, String suiteId, String search) {
    return list(projectId, status, suiteId, search, null, null, null);
  }

  /**
   * As above, additionally scoped by ADO Area / Team / Iteration (via each case's linked
   * requirements).
   */
  @Transactional(readOnly = true)
  public List<ManagedTestCaseDto> list(
    ...
    if (suiteId != null && !suiteId.isBlank()) {
      Set<UUID> members =
          suiteRepo
              .findByProjectIdAndId(projectId, UUID.fromString(suiteId))
    ...
    return cases.stream().map(tc -> ManagedTestCaseDto.from(tc, loadSteps(tc.getId()))).toList();
  }

  /**
   * IDs of test cases whose linked requirements fall in the given Area and/or the given Team's
   * owned area subtree. Returns {@code null} when neither is requested (no scoping), else the
   * (possibly empty) matching set.
   */
  ...
  public ManagedTestCaseDto create(UUID projectId, CreateTestCaseRequest req) {
    ...
    if (req.linkedRequirementIds() != null) {
      for (String rid : req.linkedRequirementIds()) {
        try {
          tc.linkRequirement(UUID.fromString(rid));
  ...
  public ManagedTestCaseDto replaceSteps(
      UUID projectId, UUID tcId, List<CreateTestCaseRequest.StepRequest> steps) {
    PlatformTestCase tc = loadAndVerify(projectId, tcId);
    stepRepo.deleteAllByTestCaseId(tcId);
  ...
  public ManagedTestCaseDto applyAnalysisSuggestion(
      UUID projectId, UUID tcId, ApplySuggestionRequest req) {
    PlatformTestCase tc = loadAndVerify(projectId, tcId);
    if (req.title() != null) tc.setTitle(req.title());
    if (req.description() != null) tc.setDescription(req.description());
    if (req.expectedResult() != null) tc.setExpectedResult(req.expectedResult());
  ...
  public ManagedTestCaseDto triggerAutomationGeneration(
      UUID projectId, UUID tcId, String githubConfigId) {
    PlatformTestCase tc = loadAndVerify(projectId, tcId);
  ...
  private void replaceLinks(PlatformTestCase tc, List<String> requirementIds) {
    new ArrayList<>(
            tc.getLinkedRequirementIds() != null ? tc.getLinkedRequirementIds() : List.<String>of())
        .forEach(
            id -> {
              try {
                tc.unlinkRequirement(UUID.fromString(id));
              } catch (IllegalArgumentException ignored) {
              }
            });
  ...
  /**
   * Keeps {@code sourceRequirementId} (the primary) consistent with the linked set: set it to the
   * requested primary when that is part of the set, otherwise clear it (e.g. when all links were
   * removed, or the primary was unset). Avoids a stale source reappearing on the next edit.
   */
            ...
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Test case not found: " + tcId));
    if (!tc.getProjectId().equals(projectId)) {
      throw new ResponseStatusException(
          HttpStatus.NOT_FOUND, "Test case not found for project: " + projectId);
    }
    return tc;
  }
  ...
  private List<TestCaseStep> saveSteps(
      UUID tcId, List<CreateTestCaseRequest.StepRequest> stepRequests) {
