...
/**
 * Resolves a test suite to its concrete test-case set so suites can be reused when building runs.
 * STATIC suites read {@code test_suite_members}; SMART suites resolve a saved filter (Area/Team via
 * linked requirements + Iteration + status + tags) live, so newly-matching cases join
 * automatically. Also manages static membership.
 */
@Service
public class SuiteResolverService {
  ...
  public List<UUID> resolveMany(UUID projectId, List<UUID> suiteIds) {
    ...
    for (UUID sid : suiteIds) {
      TestSuite suite =
          suiteRepo
              .findByProjectIdAndId(projectId, sid)
              .orElseThrow(
                  () ->
                      new ResponseStatusException(
                          HttpStatus.BAD_REQUEST, "Suite not found: " + sid));
  ...
  public void replaceMembers(UUID projectId, UUID suiteId, List<UUID> caseIds) {
    TestSuite suite =
        suiteRepo
            .findByProjectIdAndId(projectId, suiteId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Suite not found: " + suiteId));
    if (suite.isSmart()) {
    ...
    njdbc.update(
        "DELETE FROM test_suite_members WHERE suite_id = :sid",
  ...
  public void setCaseSuites(UUID projectId, UUID caseId, List<UUID> suiteIds) {
    ...
    njdbc.update(
        "DELETE FROM test_suite_members WHERE test_case_id = :cid",
