...
/**
 * Parametrization properties (axes) on a test case. These define the values the
 * property matrix expands into per-combination executions at run creation.
 */
@RestController
...
public class TestCasePropertyController {
    ...
    @PutMapping
    public List<PropertyDto> replace(@PathVariable UUID projectId, @PathVariable UUID caseId,
        ...
        List<PropertyDto> replace(UUID caseId, List<PropertyDto> properties) {
            ...
            if (properties != null) {
                for (PropertyDto p : properties) {
                    if (p.name() != null && !p.name().isBlank() && p.value() != null && !p.value().isBlank()) {
