...
/**
 * Azure DevOps schema discovery + mapping suggestions for a project. Uses the
 * project's resolved AZURE_DEVOPS_BOARDS credential (Org→Team→Project cascade).
 */
@RestController
...
public class AdoDiscoveryController {
    ...
    public List<AdoDiscoveryService.TypeSummary> types(@PathVariable UUID projectId,
                                                       @RequestParam String adoProject) {
        return service.listTypes(projectId, adoProject);
    }

    @GetMapping("/work-item-types/{type}/schema")
    public AdoDiscoveryService.TypeSchema schema(@PathVariable UUID projectId,
    ...
    public SchemaDriftReport drift(@PathVariable UUID projectId,
                                   @PathVariable String type,
                                   @RequestParam String adoProject,
    ...
    public SchemaDriftReport captureBaseline(@PathVariable UUID projectId,
                                             @PathVariable String type,
                                             @RequestParam String adoProject,
                                             @RequestHeader(value = "X-Actor", required = false) String actor) {
