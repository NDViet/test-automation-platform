...
/**
 * Lightweight reachability check for an integration credential, used by the "Test connection" admin
 * action. Performs a single authenticated GET against a cheap endpoint per integration family.
 */
@Component
public class CredentialHealthChecker {
   ...
   * @param params merged non-secret params + decrypted secret (pat/token/email/owner/repo/…) Set
   *     {@code skipSslVerify=true} to bypass certificate validation for servers with self-signed or
   *     private-CA certificates.
   */
  public Result check(String type, String baseUrl, Map<String, String> params) {
    try {
      HttpClient client =
          "true".equalsIgnoreCase(params.get("skipSslVerify")) ? httpTrustAll : http;
      return doCheck(type, baseUrl, params, client);
    } catch (Exception e) {
      if (isSslCertError(e)) {
        log.warn(
            "SSL certificate validation failed, retrying without cert check: {}", e.getMessage());
        try {
  ...
  private Result doCheck(String type, String baseUrl, Map<String, String> params, HttpClient client)
      throws Exception {
    return switch (type) {
  ...
  private Result checkJira(String baseUrl, Map<String, String> p, HttpClient client)
      throws Exception {
    String email = p.get("email");
    String token =
        firstNonBlank(p.get("apiToken"), p.get("api_token"), p.get("token"), p.get("pat"));
  ...
  private Result checkAzure(String baseUrl, Map<String, String> p, HttpClient client)
      throws Exception {
  ...
  private Result checkGitHub(String baseUrl, Map<String, String> p, HttpClient client)
      throws Exception {
    String pat = firstNonBlank(p.get("pat"), p.get("token"));
    ...
    String url =
        (owner != null && repo != null) ? root + "/repos/" + owner + "/" + repo : root + "/user";
  ...
  private static HttpClient buildTrustAllClient() {
    try {
      ...
      ctx.init(
          ...
          new SecureRandom());
      return HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(8)).sslContext(ctx).build();
    } catch (Exception e) {
  ...
  private Result classify(int code, String name) {
    ...
    if (code == 401 || code == 403)
      return new Result(false, name + " authentication failed (HTTP " + code + ")");
