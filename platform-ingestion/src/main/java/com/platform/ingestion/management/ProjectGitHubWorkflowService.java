...
/**
 * Reads and triggers GitHub Actions workflows for TEST_AUTOMATION repos
 * assigned to a project. All calls are live (no local cache) since workflow
 * metadata and runs are small and change frequently.
 */
@Service
...
public class ProjectGitHubWorkflowService {
    ...
    public ProjectGitHubWorkflowService(ProjectRepoAssignmentRepository assignmentRepo,
    ...
    public List<RunDto> listRuns(UUID projectId, String repoFullName, long workflowId, int limit) {
        ...
        String url = ctx.baseUrl + "/repos/" + parts[0] + "/" + parts[1]
                + "/actions/workflows/" + workflowId
                + "/runs?per_page=" + Math.min(limit, 50);
        JsonNode root = githubGet(url, ctx.pat);
    ...
    public record DispatchRequest(String repoFullName, long workflowId, String ref,
    ...
    public DispatchResult triggerDispatch(UUID projectId, DispatchRequest req) {
        ...
        String url = ctx.baseUrl + "/repos/" + parts[0] + "/" + parts[1]
                + "/actions/workflows/" + req.workflowId() + "/dispatches";

        Map<String, Object> body = req.inputs() != null && !req.inputs().isEmpty()
        ...
        try {
            ...
            if (code == 422) return new DispatchResult(false,
                    "GitHub rejected dispatch (422) — check that workflow_dispatch trigger is configured and the ref exists");
            if (code == 403) return new DispatchResult(false,
                    "PAT lacks 'workflow' scope — re-generate with workflow permission");
            return new DispatchResult(false, "GitHub returned HTTP " + code);
        } catch (Exception e) {
            throw new ResponseStatusException(HttpStatus.BAD_GATEWAY,
                    "Dispatch request failed: " + e.getMessage());
        }
    }
    ...
    private List<ProjectRepoAssignment> testAutoAssignments(UUID projectId) {
        return assignmentRepo.findByProjectIdOrderByRepoFullName(projectId)
                .stream().filter(a -> "TEST_AUTOMATION".equals(a.getRole())).toList();
    }

    private record ApiCtx(String pat, String baseUrl) {}

    private ApiCtx ctxForRepo(UUID projectId, String repoFullName) {
        ProjectRepoAssignment a = assignmentRepo.findByProjectIdOrderByRepoFullName(projectId)
                .stream().filter(x -> repoFullName.equals(x.getRepoFullName())).findFirst()
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
                        "Repo not assigned to project: " + repoFullName));
        return ctx(a.getCredentialId());
    }

    private ApiCtx ctx(UUID credentialId) {
        IntegrationCredential cred = credRepo.findById(credentialId).orElseThrow(() ->
                new ResponseStatusException(HttpStatus.NOT_FOUND, "Credential not found: " + credentialId));
    ...
    private String decryptPat(IntegrationCredential c) {
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to read credential secret");
        }
    }
    ...
    private static String[] split(String fullName) {
        ...
        if (p.length < 2) throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                "Invalid repo name (expected owner/repo): " + fullName);
        return p;
    }
        ...
        try {
            ...
            if (code == 401 || code == 403)
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                        "GitHub auth failed (HTTP " + code + ") — check PAT scopes");
    ...
    private static HttpClient trustAllClient(Duration connectTimeout) {
        ...
        } catch (Exception e) { throw new IllegalStateException("Cannot create HTTP client", e); }
    }
}
