...
public class TestSuiteService {
  ...
  public TestSuiteService(
      TestSuiteRepository suiteRepo, AdoTeamRepository teamRepo, SuiteResolverService resolver) {
  ...
  public TestSuiteDto create(UUID projectId, CreateTestSuiteRequest req) {
    TestSuite suite =
        new TestSuite(
            projectId, req.name(), req.description(), parseParent(req.parentId()), req.planType());
  ...
  public void replaceMembers(UUID projectId, UUID suiteId, List<String> caseIds) {
    List<UUID> ids = caseIds == null ? List.of() : caseIds.stream().map(UUID::fromString).toList();
  ...
  private void applyScope(TestSuite suite, CreateTestSuiteRequest req) {
    UUID teamId =
        (req.teamId() != null && !req.teamId().isBlank())
            ? UUID.fromString(req.teamId().trim())
            : null;
    suite.setScopeAndMode(
        req.areaPath(),
        teamId,
        req.selectionMode(),
        req.filterIteration(),
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Test suite not found: " + suiteId));
  }
