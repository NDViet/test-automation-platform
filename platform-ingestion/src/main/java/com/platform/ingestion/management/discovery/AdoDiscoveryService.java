...
/**
 * Explores Azure DevOps Boards structure (projects, work-item types, fields, states) using the
 * resolved {@code AZURE_DEVOPS_BOARDS} credential for a project, and produces a suggested mapping
 * profile via {@link MappingSuggester}.
 */
@Service
public class AdoDiscoveryService {

  private static final String API = "api-version=7.0";
  private static final Set<String> STOCK_TYPES =
      Set.of(
          "bug",
          "task",
          "epic",
  ...
  public record TypeSummary(
      String name, boolean custom, String suggestedLane, String suggestedIssueType) {}

  public record FieldInfo(
      String referenceName, String name, String type, boolean custom, boolean required) {}
  ...
  public List<AdoProject> listProjects(java.util.UUID projectId) {
    ...
    for (JsonNode p : resp.path("value"))
      out.add(new AdoProject(p.path("id").asText(), p.path("name").asText()));
    return out;
  }
  ...
  public TypeSchema typeSchema(java.util.UUID projectId, String adoProject, String type) {
    ...
    JsonNode statesResp =
        get(
            ado,
            "/" + enc(adoProject) + "/_apis/wit/workitemtypes/" + enc(type) + "/states?" + API);
    ...
    Object suggested =
        suggester.suggest(
            rules, "AZURE_DEVOPS_BOARDS", type, suggesterFields, categories, hasBlocked);
  ...
  private Ado resolve(java.util.UUID projectId) {
    ResolvedCredential cred =
        credentialResolver
            .resolve(projectId, "AZURE_DEVOPS_BOARDS")
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "No Azure DevOps Boards credential resolved for this project. Configure one"
                            + " (Org/Team/Project) first."));
    ...
    if (org == null || pat == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Azure credential is missing organization or pat");
    }
  ...
  private JsonNode get(Ado ado, String path) {
    try {
      ...
      if (resp.statusCode() == 401 || resp.statusCode() == 403) {
        throw new ResponseStatusException(
            HttpStatus.BAD_GATEWAY, "Azure DevOps auth failed (HTTP " + resp.statusCode() + ")");
      }
      if (resp.statusCode() >= 300) {
        throw new ResponseStatusException(
            HttpStatus.BAD_GATEWAY, "Azure DevOps returned HTTP " + resp.statusCode());
      }
