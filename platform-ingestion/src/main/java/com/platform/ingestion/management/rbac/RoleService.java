...
/**
 * Manages RBAC role assignments ({@code team_members}). Authorization for the
 * grant/revoke operations themselves is enforced here via {@link RbacService}:
 * only ORG_ADMINs manage org-wide roles; TEAM_ADMINs (or ORG_ADMINs) manage their
 * team's roles. A first-admin bootstrap lets the very first ORG_ADMIN be created
...
public class RoleService {
    ...
    public List<TeamMemberDto> list(String scope, UUID scopeId) {
        List<TeamMember> rows = "ORG".equalsIgnoreCase(scope)
    ...
    public TeamMemberDto grant(String actor, GrantRoleRequest req) {
        ...
        if (!bootstrap) {
            ...
            if (!allowed) {
                throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                        "Actor '" + actor + "' may not grant " + role + " at this scope");
            }
        }
        ...
        log.info("[RBAC] {} granted {} to {} (team={}) bootstrap={}",
                actor, role, req.userId(), teamId, bootstrap);
        return TeamMemberDto.from(saved);
    }

    public void revoke(String actor, UUID id) {
        TeamMember m = memberRepo.findById(id).orElseThrow(() ->
                new ResponseStatusException(HttpStatus.NOT_FOUND, "Role assignment not found: " + id));

        boolean org = m.getTeamId() == null;
        boolean allowed = org ? rbac.canManageOrg(actor) : rbac.canManageTeam(actor, m.getTeamId());
        if (!allowed) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                    "Actor '" + actor + "' may not revoke this role");
        }
        // Don't strand the org: keep at least one ORG_ADMIN.
        if (Role.ORG_ADMIN.name().equals(m.getRole()) && memberRepo.countByRole(Role.ORG_ADMIN.name()) <= 1) {
    ...
    private TeamMember findExisting(String userId, UUID teamId, Role role) {
        List<TeamMember> candidates = teamId == null
                ? memberRepo.findByUserId(userId).stream().filter(x -> x.getTeamId() == null).toList()
                : memberRepo.findByUserIdAndTeamId(userId, teamId);
        return candidates.stream().filter(x -> role.name().equals(x.getRole())).findFirst().orElse(null);
    }
