 ...
 * Each check becomes one test case: passes=0 → FAILED, passes>0 && fails=0 → PASSED, passes>0 &&
 * fails>0 → FAILED (partial failure).
 *
 * <p>Generate the file with: {@code k6 run --summary-export=summary.json script.js}
 */
@Component
public class K6JsonParser implements ResultParser {
  ...
  public List<UnifiedTestResult> parse(List<byte[]> files, ParseContext ctx) throws ParseException {
    ...
    long totalMs =
        allCases.stream().mapToLong(t -> t.durationMs() != null ? t.durationMs() : 0L).sum();

    // Prefer perfMetrics duration (actual load-test wall-clock) over sum of check durations
    Long durationMs =
        (perfMetrics != null && perfMetrics.durationMs() != null)
            ? perfMetrics.durationMs()
            : (totalMs > 0 ? totalMs : null);

    return List.of(
        new UnifiedTestResult(
            ctx.runId(),
            ctx.teamId(),
            ctx.projectId(),
            ctx.branch(),
  ...
  /**
   * Extracts HTTP response time percentiles, throughput, error rate, and VU count from the
   * top-level {@code metrics} node of a k6 summary JSON.
   *
   * <p>K6 metric keys of interest:
   *
   * <ul>
   *   <li>{@code http_req_duration} — avg/min/med/max/p(90)/p(95)/p(99) in ms
   *   <li>{@code http_reqs} — count (total requests), rate (req/s)
   *   <li>{@code http_req_failed} — passes/fails used to compute error rate
   *   <li>{@code vus_max} — value (peak concurrent VUs)
   *   <li>{@code test_duration} — test wall-clock duration in ms (custom metric or inferred)
   * </ul>
   */
  ...
  private TestCaseResultDto parseCheck(JsonNode check, String groupPath) {
    ...
    if (fails == 0 && passes > 0) {
      ...
      failureMessage =
          String.format(
              "Check partially failed: %.1f%% failure rate (%d/%d failed)", failRate, fails, total);
    } else {
    ...
    return TestCaseResultDto.basic(
        testId,
        name,
        groupPath.isEmpty() ? "root" : groupPath,
        name,
        List.of(),
        status,
        0L,
