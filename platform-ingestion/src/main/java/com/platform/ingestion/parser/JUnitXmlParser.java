...
/**
 * Parses JUnit XML produced by Maven Surefire (one XML file per test class). Format: {@code
 * <testsuite><testcase><failure/><error/></testcase></testsuite>}
 */
@Component
    ...
    for (byte[] file : files) {
      try {
        ...
        throw new ParseException(
            "Failed to parse JUnit XML: " + e.getMessage(), SourceFormat.JUNIT_XML, e);
      }
    }
