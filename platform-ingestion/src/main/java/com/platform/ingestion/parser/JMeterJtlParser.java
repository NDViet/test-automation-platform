 ...
 * becomes one test case. Samples with {@code s="false"} are counted as failures. The test case is
 * FAILED if any sample failed; PASSED otherwise.
 *
 * <p>Generate with JMeter: {@code jmeter -n -t test.jmx -l results.jtl}
 */
@Component
public class JMeterJtlParser implements ResultParser {
    ...
    for (byte[] file : files) {
      try {
        ...
        throw new ParseException(
            "Failed to parse JMeter JTL: " + e.getMessage(), SourceFormat.JMETER, e);
      }
    }
  ...
  private void parseSamples(NodeList nodes, Map<String, LabelStats> statsMap) {
    for (int i = 0; i < nodes.getLength(); i++) {
      ...
      statsMap.computeIfAbsent(label, k -> new LabelStats()).add(elapsed, success, rc, rm);
    }
  }
    ...
    if (stats.failures == 0) {
      ...
      failureMessage =
          String.format(
              ...
              failRate, stats.failures, stats.total, stats.lastErrorRc, stats.lastErrorRm);
    }
