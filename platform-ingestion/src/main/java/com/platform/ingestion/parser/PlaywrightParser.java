...
/**
 * Parses Playwright JSON report (playwright-report/results.json). Structure: { suites[] → specs[] →
 * tests[] → results[] }
 */
@Component
public class PlaywrightParser implements ResultParser {
      ...
      try {
        ...
        if (!suites.isArray()) {
          throw new ParseException(
              "Invalid Playwright report: missing 'suites' array", SourceFormat.PLAYWRIGHT);
        }
        ...
        throw new ParseException(
            "Failed to parse Playwright JSON: " + e.getMessage(), SourceFormat.PLAYWRIGHT, e);
      }
    }
  ...
  private void parseSuites(JsonNode suites, String parentTitle, List<TestCaseResultDto> results) {
    for (JsonNode suite : suites) {
      ...
      String fullTitle = parentTitle.isBlank() ? suiteTitle : parentTitle + " > " + suiteTitle;
