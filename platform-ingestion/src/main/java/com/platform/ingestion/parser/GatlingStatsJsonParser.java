 ...
 * Nested {@code GROUP} contents are also traversed recursively. A request is FAILED if {@code
 * stats.numberOfRequests.ko > 0}.
 *
 * <p>File location: {@code target/gatling/<simulation>/js/stats.json}
 */
@Component
public class GatlingStatsJsonParser implements ResultParser {
  ...
  public List<UnifiedTestResult> parse(List<byte[]> files, ParseContext ctx) throws ParseException {
      ...
      try {
        ...
        if (contents.isMissingNode()) {
          throw new ParseException(
              "Invalid Gatling stats.json: missing 'contents'", SourceFormat.GATLING);
        }
        ...
        throw new ParseException(
            "Failed to parse Gatling JSON: " + e.getMessage(), SourceFormat.GATLING, e);
      }
    }
    ...
    double avgMs =
        allCases.isEmpty()
            ? 0
            : allCases.stream()
                .mapToLong(t -> t.durationMs() != null ? t.durationMs() : 0L)
                .average()
                .orElse(0);
    double errorRate = allCases.isEmpty() ? 0 : (double) failed / allCases.size();
    PerformanceMetricsDto perfMetrics =
        PerformanceMetricsDto.of(
            avgMs,
            null,
            null,
            null,
  ...
  private TestCaseResultDto parseRequest(JsonNode node) {
    ...
    return TestCaseResultDto.basic(
        path, name, path, name, List.of(), status, meanMs, failureMessage, null, 0, List.of());
  }
}
