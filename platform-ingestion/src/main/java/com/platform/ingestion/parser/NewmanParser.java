...
/**
 * Parses Postman/Newman JSON reporter output. Each execution item → each assertion = one test case.
 */
@Component
      ...
      try {
        ...
        if (!executions.isArray()) {
          throw new ParseException(
              "Invalid Newman report: missing run.executions", SourceFormat.NEWMAN);
        }
        ...
        throw new ParseException(
            "Failed to parse Newman JSON: " + e.getMessage(), SourceFormat.NEWMAN, e);
      }
    }
  ...
  private void parseExecution(JsonNode execution, List<TestCaseResultDto> results) {
    ...
    for (JsonNode assertion : assertions) {
      ...
      if (!error.isNull() && !error.isMissingNode() && error.isObject()) {
        ...
        failureMessage =
            error.path("message").asText(error.path("name").asText("Assertion failed"));
      } else {
