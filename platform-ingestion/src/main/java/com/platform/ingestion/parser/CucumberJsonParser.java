...
/**
 * Parses Cucumber JSON report (cucumber.json). Structure: Feature[] → Element[] (scenarios) →
 * Step[]
 */
@Component
public class CucumberJsonParser implements ResultParser {
      ...
      try {
        ...
        if (!features.isArray()) {
          throw new ParseException(
              "Cucumber JSON must be an array of features", SourceFormat.CUCUMBER_JSON);
        }
        ...
        throw new ParseException(
            "Failed to parse Cucumber JSON: " + e.getMessage(), SourceFormat.CUCUMBER_JSON, e);
      }
    }
  ...
  private TestCaseResultDto parseScenario(
      JsonNode scenario, String featureUri, String featureName) {
