...
/**
 * Parser for the {@code PLATFORM_NATIVE} format — JSON payloads produced directly by {@code
 * platform-testframework}.
 *
 * <p>No transformation is needed: the JSON is already a {@link UnifiedTestResult} with rich step,
 * trace, and environment data attached to each test case.
 */
@Component
  ...
  public List<UnifiedTestResult> parse(List<byte[]> files, ParseContext ctx) throws ParseException {
    if (files == null || files.isEmpty()) {
      throw new ParseException(
          "No PLATFORM_NATIVE JSON files provided", SourceFormat.PLATFORM_NATIVE);
    }
    ...
    for (byte[] file : files) {
      try {
        ...
        throw new ParseException(
            "Failed to parse PLATFORM_NATIVE JSON: " + e.getMessage(),
