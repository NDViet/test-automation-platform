 ...
 *
 * Ingestion owns the blob store (it wrote the trace), so it is the right service
 * to serve it — avoids cross-container filesystem access when storage type is
 * "filesystem" (different containers have different local disks).
 */
@RestController
...
public class TraceDownloadController {
    ...
    public TraceDownloadController(
            ...
            BlobStore blobStore,
            BlobStoreProperties storageProps) {
    ...
    public void downloadTrace(
        ...
        if (result.getTraceStorePath() == null) {
            log.warn("Trace download: resultId={} exists but traceStorePath is null (trace not uploaded)", resultId);
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No trace for this result");
        }

        BlobRef ref = new BlobRef(BlobStoreBuckets.TRACES, result.getTraceStorePath(),
                null, "application/zip", 0);

        log.debug("Serving trace resultId={} key={} storageType={}", resultId, result.getTraceStorePath(), storageProps.getType());

        if (storageProps.getType() != BlobStoreProperties.Type.filesystem) {
            URI presigned = blobStore.presignUrl(ref,
                    Duration.ofMinutes(storageProps.getPresignUrlTtlMinutes()));
            response.setStatus(HttpServletResponse.SC_FOUND);
            response.setHeader("Location", presigned.toString());
        } else {
            byte[] bytes = blobStore.fetchBytes(ref)
                    .orElseThrow(() -> new ResponseStatusException(
                            HttpStatus.NOT_FOUND, "Trace blob not found on disk: " + ref.key()));
            response.setContentType("application/zip");
            response.setHeader("Content-Disposition", "attachment; filename=\"trace-" + resultId + ".zip\"");
