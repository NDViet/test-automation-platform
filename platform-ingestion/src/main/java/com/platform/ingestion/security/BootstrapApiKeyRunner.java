 ...
 *
 * <p>Set {@code PLATFORM_PORTAL_KEY} to the raw {@code plat_...} key value.
 * The runner hashes it and upserts a single row in {@code api_keys} (no-op if
 * the row already exists). The same value must be set as
 * {@code portal.services.ingestion.service-key} on the portal service so it
 * sends the key on every request.</p>
 *
 * <p>To generate a suitable key:</p>
 * <pre>
 *   python3 -c "import secrets,base64; print('plat_'+base64.urlsafe_b64encode(secrets.token_bytes(32)).decode().rstrip('='))"
 * </pre>
 *
 * <p>The key has no expiry and no team — it is a system service-account key.</p>
 */
@Component
public class BootstrapApiKeyRunner implements ApplicationRunner {
    ...
    public BootstrapApiKeyRunner(
            ...
            @Value("${platform.portal.service-key:}") String rawKey) {
    ...
    public void run(ApplicationArguments args) {
        if (rawKey == null || rawKey.isBlank()) {
            log.info("[Bootstrap] PLATFORM_PORTAL_KEY not set — skipping portal service-account key bootstrap.");
            return;
        }
