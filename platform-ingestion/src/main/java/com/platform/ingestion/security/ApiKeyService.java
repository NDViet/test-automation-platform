 ...
 *
 * <p>Raw keys are <strong>never</strong> stored. Only the SHA-256 hash is persisted. The caller
 * must record the raw key at creation time — it cannot be retrieved later.
 */
@Service
public class ApiKeyService {
  ...
  public ApiKeyCreationResult create(String name, UUID teamId, Integer ttlDays) {
    ...
    Instant expiresAt = ttlDays != null ? Instant.now().plus(ttlDays, ChronoUnit.DAYS) : null;
  ...
  /**
   * Records usage timestamp. Called from the auth filter — runs in a short background-friendly
   * transaction.
   */
  @Transactional
  ...
  public record ApiKeyCreationResult(UUID id, String prefix, String rawKey, Instant expiresAt) {}
}
