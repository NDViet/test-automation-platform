...
public class ApiKeyController {
  ...
  public ResponseEntity<ApiKeyService.ApiKeyCreationResult> create(
    ...
    ApiKeyService.ApiKeyCreationResult result =
        keyService.create(request.name(), request.teamId(), request.ttlDays());
  ...
  public List<ApiKeySummary> list(@RequestParam(required = false) UUID teamId) {
    return keyService.list(teamId).stream().map(ApiKeySummary::from).toList();
  }
  ...
  /**
   * {@code teamId} and {@code ttlDays} are optional. Keys are not restricted to any
   * org/project/team.
   */
