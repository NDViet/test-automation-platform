 ...
 *
 * <p>Two ingestion paths:
 *
 * <ol>
 *   <li><b>Automatic</b> — called from {@link com.platform.ingestion.api.ResultIngestionService}
 *       when test cases carry {@code coveredClasses} (from {@code @AffectedBy} or JS SDK).
 *   <li><b>Standalone manifest</b> — called from {@link CoverageIngestionController} when a team
 *       POSTs a pre-built {@link CoverageManifest} (e.g. from a JaCoCo post-processor).
 * </ol>
 *
 * <p>Uses UPSERT semantics — existing mappings are touched (lastSeenAt updated), new ones are
 * inserted. This keeps the mapping table current without bloat.
 */
@Service
public class CoverageIngestionService {
  ...
  public CoverageIngestionService(
      TestCoverageMappingRepository coverageRepo, ProjectRepository projectRepo) {
    this.coverageRepo = coverageRepo;
    this.projectRepo = projectRepo;
  }

  /**
   * Extracts coverage data from an already-parsed test run and upserts mappings. No-op if no test
   * cases carry covered-class data.
   */
  @Transactional
  ...
  /** Ingests a standalone {@link CoverageManifest}. Returns the number of mappings upserted. */
  @Transactional
  ...
  private void upsert(UUID projectId, String testCaseId, String className, String methodName) {
    coverageRepo
        .findByProjectIdAndTestCaseIdAndClassName(projectId, testCaseId, className)
        .ifPresentOrElse(
            existing -> {
              existing.touch();
