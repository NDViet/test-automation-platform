...
/**
 * Unit tests for RequirementChangeProcessor.
 * Verifies SHA-256 hash change detection and TC status mutation contract.
 */
...
class RequirementChangeProcessorTest {
    ...
    @BeforeEach
    void setUp() {
        processor = new RequirementChangeProcessor(
    ...
    private PlatformTestCase makeTc(UUID id, String status) {
        PlatformTestCase tc = new PlatformTestCase(
                projectId, "Test case", List.of(), "AGENT", null);
        setField(tc, "id", id);
