...
/**
 * Unit tests for TestPlanGeneratorService. Verifies coverage score computation, risk level
 * derivation, and plan item creation.
 */
...
class TestPlanGeneratorServiceTest {
  ...
  void generateForRelease_allRequirementsCovered_scoreOneRiskLow() {
    ...
    when(releaseReqRepo.findRequirementIdsByReleaseId(releaseId))
        .thenReturn(List.of(req1Id, req2Id));
  ...
  void generateForRelease_existingPlanItems_areReplacedOnRegenerate() {
    ...
    SotTestPlanItem oldItem =
        new SotTestPlanItem(
            existingPlan.getId(), UUID.randomUUID(), List.of(req1Id), "MANUAL", "SHOULD_RUN");
  ...
  private PlatformTestCase makeTc(UUID id, boolean hasAutomation) {
    PlatformTestCase tc = new PlatformTestCase(projectId, "Test", List.of(), "AGENT", null);
    setField(tc, "id", id);
