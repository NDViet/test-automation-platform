...
/**
 * Unit tests for DefaultContextAssembler.
 * Verifies the Hub→Node ContextBundle assembly contract:
 * - requirementContext populated when trigger has an entityExternalId
 * - prDiff pre-fetched for GitHub webhook triggers
 * - executionContext always populated from live data
 */
...
class DefaultContextAssemblerTest {
    ...
    void setUp() {
        assembler = new DefaultContextAssembler(
                projectRepo, executionRepo, testCaseResultRepo,
                flakinessRepo, workflowRepo, requirementRepo,
                blobStore, graphService);
        ...
        // Default: empty executions — lenient so individual tests can override without UnnecessaryStubbing
        lenient().when(executionRepo.findByProjectIdOrderByExecutedAtDesc(eq(projectId), any()))
        ...
        lenient().when(executionRepo.findByProjectAndDateRange(eq(projectId), any(), any())).thenReturn(List.of());
    }
    ...
    void assemble_noEntityExternalId_requirementContextIsNull() {
        TriggerRef trigger = new TriggerRef(TriggerRef.TriggerType.MANUAL,
                IntegrationType.JIRA_CLOUD, "issue", null, null, "user", Instant.now());
    ...
    void assemble_entityExternalIdPresent_requirementContextPopulated() {
        ...
        RequirementContext reqCtx = new RequirementContext(null, List.of(), List.of(), List.of(), List.of(), TestGenMode.CREATE_ALL);
    ...
    void assemble_githubWebhookWithPrUrl_preDiffFetched() throws Exception {
        TriggerRef trigger = new TriggerRef(
                TriggerRef.TriggerType.WEBHOOK, IntegrationType.GITHUB,
                "pull_request", "42",
        ...
        when(blobStore.storeText(any(), eq(prFilesJson), eq(com.platform.common.storage.BlobRef.TYPE_JSON)))
                .thenReturn(stored);
    ...
    void assemble_githubWebhookNoPrUrl_preDiffNotFetched() {
        TriggerRef trigger = new TriggerRef(
                TriggerRef.TriggerType.WEBHOOK, IntegrationType.GITHUB,
                "push", null, null, "bot", Instant.now());
    ...
    void assemble_githubClientNull_preDiffSkipped() {
        ...
        ContextBundle bundle = assembler.assemble(workflowId, projectId, AgentGridFixtures.webhookTrigger());
        assertThat(bundle.prDiff()).isNull();
    }
    ...
    void assemble_withPassRate_executionContextReflectsIt() {
        ...
        ContextBundle bundle = assembler.assemble(workflowId, projectId, AgentGridFixtures.scheduleTrigger());
    ...
    void assemble_webhookTrigger_inferencesAnalyzeAndCoverage() {
        ...
        assertThat(bundle.taskTypes())
                .containsExactlyInAnyOrder(
                        AgentTaskType.ANALYZE_PR_DIFF,
                        AgentTaskType.DETECT_COVERAGE_GAPS);
    }
    ...
    void assemble_scheduleTrigger_inferencesDigestAndFlakiness() {
        ...
        assertThat(bundle.taskTypes())
                .containsExactlyInAnyOrder(
                        AgentTaskType.GENERATE_NIGHTLY_DIGEST,
                        AgentTaskType.INVESTIGATE_FLAKINESS);
    }
    ...
    void assemble_sessionIdIsUnique() {
        ContextBundle b1 = assembler.assemble(workflowId, projectId, AgentGridFixtures.scheduleTrigger());
        ContextBundle b2 = assembler.assemble(workflowId, projectId, AgentGridFixtures.scheduleTrigger());
    ...
    private PlatformRequirement makeRequirement(UUID id, String externalId) {
        PlatformRequirement req = new PlatformRequirement(
                projectId, null, externalId, "Req title", "Desc", "STORY");
        setField(req, "id", id);
        return req;
    }
