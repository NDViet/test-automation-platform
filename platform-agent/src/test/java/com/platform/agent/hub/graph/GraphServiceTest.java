...
/**
 * Unit tests for GraphService — the central traversal component. Verifies the graph→context
 * conversion contract and TestGenMode resolution.
 */
...
class GraphServiceTest {
  ...
  void getChildren_returnsDirectChildrenOnly() {
    ...
    // configId is null in makeRequirement → resolveSource returns GITHUB without querying
    // configRepo
  ...
  void hasAutomation_returnsTrueWhenAnyTcHasAutomation() {
    ...
    when(testCaseRepo.findCoveringTestCases(projectId, requirementId)).thenReturn(List.of(autoTc));
  ...
  void hasAutomation_returnsFalseWhenNoTcs() {
    when(testCaseRepo.findCoveringTestCases(projectId, requirementId)).thenReturn(List.of());
  ...
  void resolveTestGenMode_allTcsActive_returnsNoAction() {
    ...
    when(testCaseRepo.findCoveringTestCases(projectId, requirementId)).thenReturn(List.of(active));
  ...
  void upsertEdge_existingEdge_doesNotSaveDuplicate() {
    ...
    PlatformTraceabilityEdge existing =
        new PlatformTraceabilityEdge(
            projectId, fromId, "TEST_CASE", toId, "REQUIREMENT", "COVERED_BY");
    when(edgeRepo.findByProjectIdAndFromIdAndToIdAndEdgeType(projectId, fromId, toId, "COVERED_BY"))
        .thenReturn(Optional.of(existing));

    graphService.upsertEdge(
        projectId, fromId, "TEST_CASE", toId, "REQUIREMENT", EdgeType.COVERED_BY, null);
  ...
  void upsertEdge_newEdge_saves() {
    ...
    graphService.upsertEdge(
        projectId, fromId, "REQUIREMENT", toId, "REQUIREMENT", EdgeType.PARENT_OF, null);
  ...
  private PlatformTestCase makeTc(UUID id, boolean hasAutomation) {
    PlatformTestCase tc =
        new PlatformTestCase(projectId, "Test case " + id, List.of(), "AGENT", null);
    setField(tc, "id", id);
