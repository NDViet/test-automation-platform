...
/**
 * Unit tests for InsightNode tool dispatch.
 * Verifies tool routing, analytics client delegation, and Slack skip-when-unconfigured.
 */
...
class InsightNodeTest {
    ...
    void getTrends_delegatesToInsightTools() throws Exception {
        ...
        String inputJson = mapper.writeValueAsString(Map.of(
                "project_id", projectId.toString(), "days", 7));
    ...
    void getFlakinessLeaderboard_delegatesToInsightTools() throws Exception {
        UUID projectId = AgentGridFixtures.PROJECT_ID;
        when(insightTools.getFlakinessLeaderboard(projectId, 10)).thenReturn("[{\"test\":\"FooTest\",\"score\":0.55}]");

        String inputJson = mapper.writeValueAsString(Map.of(
                "project_id", projectId.toString(), "limit", 10));
        String result = nodeWithoutSlack.dispatchToolCall(
                "platform_get_flakiness_leaderboard", inputJson, bundle);
    ...
    void getQualityGate_delegatesToInsightTools() throws Exception {
        ...
        String inputJson = mapper.writeValueAsString(Map.of("project_id", projectId.toString()));
        String result = nodeWithoutSlack.dispatchToolCall(
                "platform_get_quality_gate", inputJson, bundle);
