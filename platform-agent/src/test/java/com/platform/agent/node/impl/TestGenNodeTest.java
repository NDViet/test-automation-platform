...
/**
 * Unit tests for TestGenNode tool dispatch — no Claude API, no DB. Verifies the Hub→Node tool
 * contract: correct entities created, correct sentinels returned.
 */
...
class TestGenNodeTest {
  ...
  void saveTestCases_createsTcAndCoveredByEdge() throws Exception {
    ...
    PlatformTestCase savedTc =
        new PlatformTestCase(
            AgentGridFixtures.PROJECT_ID,
            "Login with valid credentials",
            List.of("AC-1", "AC-2"),
            "AGENT",
            AgentGridFixtures.SESSION_ID);
  ...
  void updateTestCase_updatesTitle_acRefs_andMarksActive() throws Exception {
    ...
    PlatformTestCase existingTc =
        new PlatformTestCase(
            AgentGridFixtures.PROJECT_ID, "Old title", List.of("AC-1"), "AGENT", null);
  ...
  void updateTestCase_notFound_returnsMessage() throws Exception {
    ...
    String inputJson =
        mapper.writeValueAsString(
            java.util.Map.of("test_case_id", tcId.toString(), "new_title", "Some title"));
    String result = node.dispatchToolCall("update_test_case", inputJson, bundle);
  ...
  void updateTestCase_noChanges_returnsNoChangesMessage() throws Exception {
    UUID tcId = AgentGridFixtures.TEST_CASE_ID;
    PlatformTestCase existingTc =
        new PlatformTestCase(AgentGridFixtures.PROJECT_ID, "Title", List.of("AC-1"), "AGENT", null);
    when(testCaseRepo.findById(tcId)).thenReturn(Optional.of(existingTc));

    // send empty strings — no changes
    String inputJson = mapper.writeValueAsString(java.util.Map.of("test_case_id", tcId.toString()));
    String result = node.dispatchToolCall("update_test_case", inputJson, bundle);
  ...
  void linkTestCases_skipsDuplicateEdge() throws Exception {
    ...
    PlatformTraceabilityEdge existing =
        new PlatformTraceabilityEdge(
            AgentGridFixtures.PROJECT_ID, tcId, "TEST_CASE", reqId, "REQUIREMENT", "COVERED_BY");
    when(edgeRepo.findByProjectIdAndFromIdAndToIdAndEdgeType(
            any(), eq(tcId), eq(reqId), eq("COVERED_BY")))
  ...
  void requestReview_returnsSentinelPrefix() throws Exception {
    ...
    String inputJson =
        mapper.writeValueAsString(
            java.util.Map.of("summary", "Review these 1 proposed test cases", "payload", payload));
