 ...
 *
 * Each nested class exercises one complete slice of the pipeline:
 *   Hub assembles context → Node receives bundle → Tool dispatch → Side effects
 *
 * These are unit-level tests (no Spring context, no DB) verifying
 * that components speak the same language across their boundaries.
 */
...
class AgentGridContractTest {
    ...
    class AwaitingReviewSentinelContract {
        ...
        void testGenNode_requestReview_returnsExactSentinelPrefix() throws Exception {
            ...
            String inputJson = mapper.writeValueAsString(Map.of(
                    "summary", "2 test cases ready for review",
                    "payload", payload));
        ...
        void testGenNode_saveTestCases_doesNotReturnSentinel() throws Exception {
            ...
            PlatformTestCase tc = new PlatformTestCase(AgentGridFixtures.PROJECT_ID,
                    "TC", List.of(), "AGENT", null);
    ...
    class GraphToContextContract {
        ...
        void freshRequirement_noTcs_modeIsCreateAll() {
            GraphService gs = new GraphService(requirementRepo, testCaseRepo, edgeRepo, releaseRepo, configRepo);
    ...
    class ChangeProcessorToGraphContract {
        ...
        void changedRequirement_tcMarkedNeedsUpdate_graphReturnsUpdateChangedMode() {
            ...
            PlatformTestCase tc = new PlatformTestCase(projectId, "Old TC", List.of("AC-1"), "AGENT", null);
            setField(tc, "id", tcId);
            ...
            RequirementChangeProcessor processor = new RequirementChangeProcessor(
                    requirementRepo, testCaseRepo, releaseRepo, graphService);
            ...
            GraphService gs = new GraphService(requirementRepo, testCaseRepo, edgeRepo, releaseRepo2, configRepo);
    ...
    class BudgetGuardContract {
        ...
        void hardLimitExceeded_tokenUsageRecordedCorrectly() {
            ...
            when(budgetRepo.findByProjectIdAndBudgetMonth(projectId, month)).thenReturn(Optional.of(budget));
    ...
    class TestPlanCoverageContract {
        ...
        void coverageScore_isAlwaysBetweenZeroAndOne() {
            ...
            TestPlanGeneratorService svc = new TestPlanGeneratorService(
            ...
            assertThat(result.getCoverageScore().doubleValue())
                    .isBetween(0.0, 1.0);
        }
    }
    ...
    class InsightFallbackContract {
        ...
        void getTrends_serviceReturnsError_nodeReturnsFallbackJson() throws Exception {
            when(insightTools.getTrends(any(), anyInt())).thenReturn("{\"error\":\"analytics service unavailable\"}");

            InsightNode node = new InsightNode(orchestrator, insightTools, mapper, "");
            ContextBundle bundle = AgentGridFixtures.bundle();

            String input = mapper.writeValueAsString(Map.of("project_id", AgentGridFixtures.PROJECT_ID.toString()));
    ...
    private PlatformRequirement makeReqWithHash(UUID projectId, UUID id, String title, String description) {
        PlatformRequirement req = new PlatformRequirement(
                projectId, null, "EXT-1", title, description, "STORY");
        setField(req, "id", id);
