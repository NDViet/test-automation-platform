...
/**
 * Shared test fixtures for agent grid contract tests.
 * All builders produce valid, fully-populated objects so individual tests
 * only need to override the specific fields they care about.
 */
public final class AgentGridFixtures {
    ...
    public static ContextBundle bundle(TriggerRef trigger) {
        return new ContextBundle(
                SESSION_ID, WORKFLOW_ID, PROJECT_ID, "test-project",
                List.of(AgentTaskType.ANALYZE_PR_DIFF),
                trigger,
                null, null, null,
                executionContext(),
                null, null, null, null, null,
                ResumeStrategy.COMPRESSED, null,
                LlmTier.STANDARD, Instant.now()
        );
    }
    ...
    public static TriggerRef webhookTrigger() {
        return new TriggerRef(TriggerRef.TriggerType.WEBHOOK, IntegrationType.GITHUB,
                "pull_request", "123",
    ...
    public static TriggerRef manualTrigger(String entityExternalId) {
        return new TriggerRef(TriggerRef.TriggerType.MANUAL, IntegrationType.JIRA_CLOUD,
                "issue", entityExternalId,
                null, "user", Instant.now());
    }

    public static TriggerRef scheduleTrigger() {
        return new TriggerRef(TriggerRef.TriggerType.SCHEDULE, null,
                "schedule", "nightly",
                null, "scheduler", Instant.now());
    }

    public static ExecutionContext executionContext() {
        return new ExecutionContext(
                UUID.randomUUID(), Instant.now().minusSeconds(3600),
                0.85, 0.10, 0, List.of(), "default");
    }

    public static TokenUsage tokenUsage(int input, int output) {
        return new TokenUsage(input, 0, 0, output,
                java.math.BigDecimal.valueOf(input * 0.0003 + output * 0.0015));
    }
