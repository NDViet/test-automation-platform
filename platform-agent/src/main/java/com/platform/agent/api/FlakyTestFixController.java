...
/**
 * Triggers a PROPOSE_HEAL_FIX workflow for a specific flaky test.
 * Enriches the ContextBundle with targeted failure samples + AI analysis context
 * before routing to HealingNode.
 *
 * POST /hub/healing/{projectId}/trigger
 * Body: { "testId": "...", "flakyScoreId": "uuid", "githubConfigId": "uuid" }
 */
@RestController
...
public class FlakyTestFixController {
    ...
    public FlakyTestFixController(AgentWorkflowService workflowService,
    ...
    public ResponseEntity<Map<String, Object>> triggerFix(
        ...
        try {
            ...
            ContextBundle enriched = enrichWithFlakyContext(base, projectId, resolvedTestId, resolvedConfigId);

            workflowService.executeWorkflow(workflow.getId(), enriched);

            log.info("Flaky fix workflow {} started for test '{}' in project {}", workflow.getId(), resolvedTestId, projectId);
            ...
            log.error("Failed to start flaky fix for test '{}' in project {}: {}", resolvedTestId, projectId, e.getMessage(), e);
            return ResponseEntity.internalServerError().body(Map.of(
                    "error", "Failed to start fix workflow: " + e.getMessage()));
        }
    }

    private ContextBundle enrichWithFlakyContext(ContextBundle base, UUID projectId,
                                                  String testId, String githubConfigId) {
        Instant since30d = Instant.now().minus(30, ChronoUnit.DAYS);

        // Load targeted failure samples for this test
        List<TestCaseResult> failures = resultRepo.findByTestIdAndProjectIdSince(
                testId, projectId, since30d);

        List<ExecutionContext.FailureSample> samples = failures.stream()
                .limit(SAMPLE_LIMIT)
        ...
        analysisRepo.findTopByTestIdAndProjectIdOrderByAnalysedAtDesc(testId, projectId)
                .ifPresent(analysis -> {
                    String aiContext = buildAnalysisContext(analysis);
                    samples.add(new ExecutionContext.FailureSample(
                            testId,
                            "[AI Analysis] " + testId,
                            aiContext,
                            null,
        ...
        FlakinessScore flakinessScore = flakinessRepo.findByTestIdAndProjectId(testId, projectId)
                .orElse(null);
        ...
        ExecutionContext enrichedCtx = new ExecutionContext(
    ...
    private SessionCredentials resolveCredentials(UUID projectId, String githubConfigId) {
        ...
        if (githubConfigId != null && !githubConfigId.isBlank()) {
            ...
            config = configs.stream()
                    .filter(c -> c.getId().equals(configUuid))
        ...
        return new SessionCredentials(
                Map.of(IntegrationType.GITHUB.name(), token),
