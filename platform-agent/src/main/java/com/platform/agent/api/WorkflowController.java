...
public class WorkflowController {
  ...
  public ResponseEntity<UUID> trigger(@RequestBody TriggerWorkflowRequest request) {
    ...
    ContextBundle bundle =
        contextAssembler.assemble(workflow.getId(), request.projectId(), trigger);
  ...
  public ResponseEntity<List<PrAnalysisDto>> prAnalyses(
      @RequestParam UUID projectId, @RequestParam(defaultValue = "30") int limit) {

    List<PrAnalysisDto> results =
        workflowRepo.findByProjectIdOrderByCreatedAtDesc(projectId).stream()
            .filter(w -> "GITHUB".equals(w.getTriggerSource()))
            .limit(limit)
            .map(
                w -> {
                  String refUrl =
                      w.getTriggerRef() != null
                          ? String.valueOf(w.getTriggerRef().getOrDefault("refUrl", ""))
                          : null;
                  String summary =
                      stepRepo.findByWorkflowIdOrderBySequenceOrder(w.getId()).stream()
                  ...
                  return new PrAnalysisDto(
                      ...
                      w.getStatus(),
                      summary,
                      w.getTotalInputTokens(),
                      w.getTotalOutputTokens(),
                      w.getStartedAt(),
                      w.getCompletedAt(),
                      w.getCreatedAt());
                })
