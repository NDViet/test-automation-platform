...
/**
 * Async service that drives the Impact Analysis AI workflow: 1. Loads PR file diffs from GitHub for
 * each linked PR. 2. Fetches requirement details from the ingestion service. 3. Calls the Claude
 * API to generate test coverage suggestions. 4. Persists the result back to the ImpactAnalysis
 * entity.
 */
@Service
public class ImpactAnalysisService {
  ...
  private static final String SYSTEM_PROMPT =
      "You are an Impact Analysis AI. Given PR diffs, requirements, and existing test cases, you"
          + " identify what test coverage needs creating or updating.\n\n"
          + "For UPDATE_MANUAL_TEST suggestions, always set testCaseId to the ID of the specific"
          + " existing test case that needs updating (if one exists). Leave testCaseId null only"
          + " for new test cases.\n\n"
          ...
          + "  \"suggestions\": [\n"
          + "    {\n"
          + "      \"type\": \"UPDATE_MANUAL_TEST\" | \"CREATE_AUTOMATED_TEST\" |"
          + " \"UPDATE_AUTOMATION\",\n"
          + "      \"title\": \"short title\",\n"
  ...
  public void runAnalysis(UUID analysisId, UUID projectId) {
    ...
    try {
      ...
      for (Map<String, Object> prRef : linkedPrs) {
        ...
        try {
          ...
          List<Map<String, Object>> files =
              mapper.readValue(filesJson, new TypeReference<List<Map<String, Object>>>() {});
          ...
          prDiffs.add(
              new PrDiffContext(repoFullName, prNumber, asString(prRef.get("prTitle")), filenames));
        } catch (Exception e) {
          log.warn(
              "runAnalysis: failed to fetch files for {}/{}#{}: {}",
              owner,
              repo,
              prNumber,
              e.getMessage());
      ...
      log.error(
          "runAnalysis: failed for analysis {} project {}: {}",
          analysisId,
          projectId,
          e.getMessage(),
          e);
  ...
  private String resolveTokenForRepo(UUID projectId, String repoFullName) {
    ...
    return configs.stream()
        ...
        .orElseGet(
            () ->
                configRepo.findByProjectId(projectId).stream()
  ...
  private String buildUserMessage(
    ...
    if (prDiffs.isEmpty()) {
      ...
      for (PrDiffContext pr : prDiffs) {
        sb.append("### ").append(pr.repoFullName()).append(" PR #").append(pr.prNumber());
      ...
      for (PlatformTestCase tc : existingTestCases) {
        ...
        if (tc.getDescription() != null && !tc.getDescription().isBlank()) {
          sb.append("  Description: ")
              .append(tc.getDescription().substring(0, Math.min(200, tc.getDescription().length())))
              .append("…\n");
        }
        sb.append("\n");
      }
  ...
  /**
   * Extracts a JSON object from a Claude response that may be wrapped in a markdown code block
   * (```json ... ```) or returned as raw JSON.
   */
  ...
  private record PrDiffContext(
      String repoFullName, int prNumber, String prTitle, List<String> changedFiles) {}
}
