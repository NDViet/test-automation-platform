 ...
 *
 * <p>GET /hub/wip/{projectId} — list all WIP items POST
 * /hub/wip/review-requests/{requestId}/approve — approve an AgentReviewRequest POST
 * /hub/wip/review-requests/{requestId}/reject — reject an AgentReviewRequest
 */
@RestController
...
public class WorkInProgressController {
  ...
  public WorkInProgressController(
  ...
  public ResponseEntity<List<WipItem>> list(@PathVariable UUID projectId) {
    ...
    List<PlatformTestCase> underReview =
        testCaseRepo.findByProjectIdAndStatus(projectId, "UNDER_REVIEW");
    underReview.stream()
        .limit(50)
        .map(
            tc -> {
    ...
    List<PlatformTestCase> prRaised =
        testCaseRepo.findByProjectIdAndAutomationStatus(projectId, "PR_CREATED");
    prRaised.stream()
        .limit(50)
        .map(
            tc -> {
    ...
    reviewRequests.forEach(
        rr -> {
          ...
          String description =
              summary != null && !summary.isBlank()
                  ? summary
                  : "Agent is requesting a decision before continuing";
    ...
    for (String status : List.of("RUNNING", "PENDING", "AWAITING_REVIEW")) {
      List<AgentWorkflow> batch =
          workflowRepo.findByProjectIdAndStatusOrderByCreatedAtDesc(projectId, status);
    ...
    workflowMap.values().stream()
        .sorted(
            Comparator.comparing(
                AgentWorkflow::getCreatedAt, Comparator.nullsLast(Comparator.reverseOrder())))
        .limit(20)
        .forEach(
            w -> {
    ...
    items.sort(
        Comparator.comparing(WipItem::createdAt, Comparator.nullsLast(Comparator.reverseOrder())));
  ...
  public ResponseEntity<?> approve(@PathVariable UUID requestId, @RequestBody DecideRequest body) {
    return reviewRequestRepo
        .findById(requestId)
        .map(
            req -> {
  ...
  public ResponseEntity<?> reject(@PathVariable UUID requestId, @RequestBody DecideRequest body) {
    return reviewRequestRepo
        .findById(requestId)
        .map(
            req -> {
