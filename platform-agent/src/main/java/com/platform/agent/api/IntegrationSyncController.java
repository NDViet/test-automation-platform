...
/**
 * On-demand sync trigger for any configured integration type. GitHub: immediately polls for
 * new/updated PRs via GitHubPollingService. Jira/Linear: returns actionable guidance (API-client
...
public class IntegrationSyncController {
  ...
  public IntegrationSyncController(
  ...
  public ResponseEntity<Map<String, Object>> syncNow(@PathVariable UUID projectId) {
    ...
    if (configs.isEmpty()) {
      return ResponseEntity.badRequest()
          .body(Map.of("error", "No integration configs found for project " + projectId));
    }
    ...
    return switch (integrationType) {
      ...
      case "LINEAR" ->
          Map.of(
              ...
              false,
              "message",
              "Linear uses webhooks — trigger a test event from "
                  + "Settings → API → Webhooks in Linear to sync immediately.");
      default ->
          Map.of("success", false, "message", "Unknown integration type: " + integrationType);
    };
  }
}
