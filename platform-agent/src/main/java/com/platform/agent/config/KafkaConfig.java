 ...
 *
 * Spring Boot 4.x modularises its auto-configuration; KafkaAutoConfiguration
 * is not reliably activated from this service's module path, so we declare the
 * producer and consumer beans explicitly — matching the pattern used by every
 * other platform module.
 *
 * All agent payloads (workflow events, approval requests/decisions) are
 * serialised as JSON strings, so both key and value use StringSerializer /
 * StringDeserializer.
 */
@Configuration
