...
/**
 * Coordinates the full workflow lifecycle: trigger → plan → dispatch → review → complete.
 * Nodes are dispatched asynchronously; their results feed back via NodeResult.
 */
@Service
public class AgentWorkflowService {
    ...
    public AgentWorkflow createWorkflow(UUID projectId, TriggerRef trigger) {
        AgentWorkflow workflow = new AgentWorkflow(
                ...
                Map.of("entityType", String.valueOf(trigger.entityType()),
        ...
        try {
            for (TaskRouter.TaskAssignment assignment : plan) {
                ...
                AgentWorkflowStep step = new AgentWorkflowStep(
                        workflowId, assignment.nodeId(),
                        node.nodeType().name(), node.taskType().name(),
                ...
                workflow.addTokens(result.tokenUsage().totalInputTokens(),
                ...
                step.markCompleted(result.summary(),
    ...
    private void publishEvent(UUID workflowId, String eventType) {
        try {
            kafka.send(Topics.AGENT_WORKFLOW_EVENTS, workflowId.toString(),
