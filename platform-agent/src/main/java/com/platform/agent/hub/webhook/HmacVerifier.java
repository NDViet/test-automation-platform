...
/**
 * Verifies GitHub HMAC-SHA256 webhook signatures. If webhookSecret is blank (dev mode) verification
 * is skipped and always returns true.
 */
@Component
public class HmacVerifier {
  ...
  void warnIfUnsecured() {
    if (webhookSecret == null || webhookSecret.isBlank()) {
      log.warn(
          "[Security] platform.agent.github.webhook-secret is not set — "
  ...
  /**
   * Returns true if the signature header matches the expected "sha256=<hex>" of the payload, or if
   * webhookSecret is blank (dev mode — skip verification).
   */
