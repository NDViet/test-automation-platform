...
public class KafkaReviewGateway implements ReviewGateway {
    ...
    public KafkaReviewGateway(KafkaTemplate<String, String> kafka,
    ...
    public UUID requestReview(NodeResult result, ContextBundle bundle) {
        ...
        AgentReviewRequest req = new AgentReviewRequest(
                result.workflowId(),
        ...
        try {
            ...
            event.put("createdAt", saved.getCreatedAt() != null ? saved.getCreatedAt().toString() : Instant.now().toString());
            kafka.send(Topics.AGENT_APPROVAL_REQUESTS, result.workflowId().toString(),
    ...
    public void applyDecision(UUID reviewRequestId, ReviewDecision decision, String editedPayload) {
        reviewRepo.findById(reviewRequestId).ifPresent(req -> {
            switch (decision) {
    ...
    private UUID findOrCreateStep(NodeResult result) {
        return stepRepo.findByWorkflowIdAndStatus(result.workflowId(), "RUNNING")
                .stream().findFirst()
