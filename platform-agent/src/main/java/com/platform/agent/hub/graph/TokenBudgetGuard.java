...
/**
 * Enforces monthly per-project token budgets. Hard-limit projects are blocked when any budget
 * threshold is exceeded. Soft-limit projects log a warning but proceed.
 */
@Service
public class TokenBudgetGuard {
  ...
  /**
   * Returns true if the project is within budget for the current month. For hard-limit projects,
   * logs an error and returns false when exceeded.
   */
  public boolean isWithinBudget(UUID projectId) {
    String month = currentMonth();
    return budgetRepo
        .findByProjectIdAndBudgetMonth(projectId, month)
        .map(
            budget -> {
              if (!budget.isHardLimit()) return true;
              if (budget.isAnyLimitExceeded()) {
                log.error(
                    "Hard budget exceeded for project {} in {}: "
                        + "input={}/{}, output={}/{}, cost={}/{}",
                    projectId,
                    month,
                    budget.getUsedInputTokens(),
                    budget.getMaxInputTokens(),
                    budget.getUsedOutputTokens(),
                    budget.getMaxOutputTokens(),
                    budget.getUsedCostCents(),
                    budget.getMaxCostCents());
                return false;
              }
              return true;
            })
  ...
  /**
   * Records token usage after a node completes. Uses an atomic upsert to avoid lost updates under
   * concurrent sessions.
   */
  @Transactional
  public void recordUsage(UUID projectId, TokenUsage usage) {
    ...
    String month = currentMonth();
    BigDecimal cost =
        usage.effectiveCostCents() != null ? usage.effectiveCostCents() : BigDecimal.ZERO;
    try {
      budgetRepo.upsertUsage(
          projectId, month, usage.totalInputTokens(), usage.outputTokens(), cost);
