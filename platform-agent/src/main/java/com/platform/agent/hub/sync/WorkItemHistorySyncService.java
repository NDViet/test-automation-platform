...
/**
 * Extracts work-item change history from the Azure DevOps Work Item Updates API into {@code
 * work_item_events} — capturing who made each state transition / assignment change. Powers true
 * QE-involvement metrics (resolved-by, participated, reopened, timeline).
 *
 * <p>Incremental: only calls ADO for items whose current {@code System.Rev} is ahead of the
 * last-processed {@code history_rev}, so re-runs are cheap.
 */
@Service
public class WorkItemHistorySyncService {
  ...
  private final ExecutorService executor =
      Executors.newSingleThreadExecutor(
          r -> {
            Thread t = new Thread(r, "wi-history-sync");
            t.setDaemon(true);
            return t;
          });

  public WorkItemHistorySyncService(
  ...
  /**
   * Launches the history sync in the background (it makes one ADO call per work item, so a full
   * first run takes minutes). Returns {@code false} if a sync is already in flight.
   */
  public boolean startSync(UUID projectId, List<String> issueTypes) {
    ...
    executor.submit(
        () -> {
          try {
            runSync(projectId, issueTypes);
          } catch (Exception e) {
            log.error("history sync failed for project {}: {}", projectId, e.getMessage(), e);
          } finally {
  ...
  private void runSync(UUID projectId, List<String> issueTypes) {
    ...
    if (configs.isEmpty()) {
      log.warn("history sync: no ADO config for project {}", projectId);
      return;
    }
    String project = null;
    ...
    if (project == null) {
      log.warn("history sync: ADO config has no 'project' param for {}", projectId);
      return;
    }

    AzureBoardsPollClient.Ado ado = client.connect(projectId);
    if (ado == null) {
      log.warn("history sync: no resolvable credential for {}", projectId);
      return;
    }

    MappingRules rules = rulesProvider.effectiveForProject(projectId);
    List<PlatformRequirement> items =
        requirementRepo.findByProjectIdAndIssueTypeIn(projectId, issueTypes);
    ...
    for (PlatformRequirement r : items) {
      ...
      if (currentRev != null && currentRev.equals(r.getHistoryRev()))
        continue; // unchanged → skip ADO call

      try {
        JsonNode updates =
            client.getWorkItemUpdates(ado, project, externalId); // ADO call (no tx held)
        added +=
            ingest(
                projectId,
                externalId,
                r.getIssueType(),
                updates,
                rules); // per-event saves commit individually
        int maxRev = 0;
        for (JsonNode u : updates.path("value"))
          maxRev = Math.max(maxRev, u.path("rev").asInt(u.path("id").asInt(0)));
        // Watermark = latest revision seen (equals System.Rev), so future runs can skip unchanged
        // items.
    ...
    log.info(
        "Work-item history sync for project {}: {} considered, {} fetched, {} events added",
        projectId,
        items.size(),
        fetched,
        added);
  ...
  private int ingest(
      UUID projectId, String externalId, String issueType, JsonNode updates, MappingRules rules) {
    int added = 0;
    for (JsonNode u : updates.path("value")) {
