 ...
 *
 * JIRA Cloud webhooks do not sign payloads. Security is enforced via a shared
 * secret in the query parameter: POST /hub/webhooks/jira?secret={token}
 *
 * Required JIRA webhook events:
 *   - jira:issue_created
 *   - jira:issue_updated (description/summary changes only)
 */
@RestController
...
public class JiraWebhookController {
    ...
    private static final Set<String> TRIGGER_FIELDS = Set.of(
            "description", "summary", "Story Points", "Acceptance Criteria");
    ...
    public JiraWebhookController(ObjectMapper mapper,
    ...
    public ResponseEntity<Void> handle(
        ...
        try {
            ...
            log.info("JIRA webhook: {} for issue '{}' — triggering workflow for project {}",
                    webhookEvent, issueKey, projectId);
