...
/**
 * Derives a SotTestPlan from the requirements scoped to a release. Computes coverage score and risk
 * level, and creates SotTestPlanItems for each test case that covers an in-scope requirement.
 */
@Service
public class TestPlanGeneratorService {
  ...
  public TestPlanGeneratorService(
  ...
  /**
   * Generates (or regenerates) a test plan for the given release. Idempotent: replaces existing
   * plan items if a plan already exists.
   */
  @Transactional
  public SotTestPlan generateForRelease(UUID projectId, UUID releaseId) {
    ...
    SotTestPlan plan =
        planRepo
            .findByProjectIdAndReleaseId(projectId, releaseId)
    ...
    for (Map.Entry<UUID, List<UUID>> entry : tcToRequirements.entrySet()) {
      ...
      boolean hasAutomation =
          testCaseRepo.findById(tcId).map(PlatformTestCase::isHasAutomation).orElse(false);

      SotTestPlanItem item =
          new SotTestPlanItem(
              plan.getId(),
              tcId,
              reqIds,
    ...
    log.info(
        "Generated test plan for release {} (project {}): {} items, coverage={}, risk={}",
        release.getName(),
        projectId,
        tcToRequirements.size(),
        score,
        riskLevel);
