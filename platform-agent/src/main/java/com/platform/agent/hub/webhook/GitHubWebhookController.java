...
/**
 * Receives GitHub PR webhook events, verifies HMAC signature, finds the matching platform project
 * by repoFullName, and triggers an agent workflow asynchronously.
 */
@RestController
...
public class GitHubWebhookController {
  ...
  public ResponseEntity<Void> handle(
    ...
    try {
      ...
      log.info(
          "GitHub webhook: PR #{} action='{}' repo='{}' project={} — triggering workflow",
          prNumber,
          action,
          repoFullName,
          projectId);
  ...
  /**
   * Finds a GITHUB ProjectIntegrationConfig whose connectionParams["repoFullName"] matches the
   * given repoFullName. Loads all GITHUB configs and filters in Java (acceptable: small number of
   * configs per org).
   */
