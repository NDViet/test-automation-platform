...
/**
 * Registers local AgentNode beans into NodeRegistry on startup so CapabilityTaskRouter can route
 * tasks to them without requiring a remote HTTP registration call. Each local node gets a stable
 * deterministic UUID derived from its task type name.
 */
@Configuration
