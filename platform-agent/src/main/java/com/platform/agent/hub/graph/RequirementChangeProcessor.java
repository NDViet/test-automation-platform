...
/**
 * Detects what changed when a requirement is re-synced and calculates the downstream impact: which
 * test cases need updating, which are obsolete, which ACs need new coverage, and which releases are
 * affected.
 */
@Service
public class RequirementChangeProcessor {
  ...
  public RequirementChangeProcessor(
  ...
  /**
   * Computes the change impact for a requirement that was just re-synced. Returns null if the
   * requirement doesn't exist or nothing changed.
   */
  @Transactional
  public ChangeImpact computeImpact(
      UUID projectId, UUID requirementId, String newTitle, String newDescription) {
    ...
    List<UUID> obsoleteCandidates =
        existingTcs.stream()
            .filter(
                tc ->
                    "NEEDS_UPDATE".equals(tc.getCoverageStatus())
    ...
    log.info(
        "Requirement {} changed: {} TCs marked NEEDS_UPDATE, {} release(s) affected",
        requirementId,
        toUpdate.size(),
        affectedReleases.size());
  ...
  public record ChangeImpact(
    ...
    public boolean hasImpact() {
      return !testCasesToUpdate.isEmpty()
          || !obsoleteCandidates.isEmpty()
