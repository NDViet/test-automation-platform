 ...
 *
 * <p>Linear signs payloads with HMAC-SHA256; the signature is in the {@code linear-signature}
 * header as a plain hex digest (no prefix).
 */
@RestController
...
public class LinearWebhookController {
  ...
  public LinearWebhookController(
      ObjectMapper mapper,
  ...
  public ResponseEntity<Void> handle(
    ...
    try {
      ...
      if (projectIdOpt.isEmpty()) {
        log.debug(
            "Linear webhook: no platform project tracks identifier '{}', ignoring", identifier);
        return ResponseEntity.ok().build();
      }
      ...
      log.info(
          "Linear webhook: {} action='{}' for '{}' — triggering workflow for project {}",
          type,
          action,
          identifier,
          projectId);
