 ...
 *
 * Slack sends POST /hub/webhooks/slack/interactions with body:
 *   payload=<urlencoded-json>
 *
 * Signature verification uses HMAC-SHA256 over "v0:{timestamp}:{rawBody}" with the
 * signing secret. The header is: X-Slack-Signature: v0=<hex>
 */
@RestController
...
public class SlackInteractionController {
    ...
    public ResponseEntity<Map<String, Object>> interactions(
        ...
        try {
            ...
            String jsonPayload = URLDecoder.decode(
            ...
            ReviewDecision decision = switch (actionId) {
            ...
            // Return a Slack-compatible acknowledgement (replaces the message buttons with confirmation text)
            return ResponseEntity.ok(Map.of(
                    "ok", true,
                    "text", "Decision recorded: " + decision.name() + " by @" + actor
            ));
    ...
    private boolean verifySlackSignature(String rawBody, String timestamp, String slackSig) {
        ...
        try {
            ...
            String computed = "v0=" + HexFormat.of().formatHex(
