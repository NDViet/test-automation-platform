 ...
 * Configure the ADO Service Hook HTTP request with a custom header of that name. If the property
 * {@code platform.agent.azure-boards.webhook-secret} is blank, verification is skipped (dev mode
 * only — always set it in production).
 *
 * <p>Configure two subscriptions in Azure DevOps: "Work item created" and "Work item updated".
 */
@RestController
...
public class AzureBoardsWebhookController {
  ...
  private static final Set<String> TRIGGER_EVENTS = Set.of("workitem.created", "workitem.updated");
  ...
  public AzureBoardsWebhookController(
      ObjectMapper mapper,
  ...
  public ResponseEntity<Void> handle(
      @RequestHeader(value = "X-ADO-Webhook-Secret", required = false, defaultValue = "")
          String secret,
    ...
    try {
      ...
      String url =
          resource.path("_links").path("html").path("href").asText(resource.path("url").asText(""));
      String actor =
          root.path("resource").path("revisedBy").path("displayName").asText("azure-boards");
      ...
      log.info(
          "Azure Boards webhook: {} for work item '{}' — triggering workflow for project {}",
          eventType,
          workItemId,
          projectId);
