 ...
 *
 * <p>Converts platform-core JPA entities into platform-common model records (RequirementRecord,
 * RequirementLink, etc.) that can be packaged into a ContextBundle without leaking persistence
 * types into platform-common.
 */
@Service
...
public class GraphService {
  ...
  /**
   * Returns the full ancestor chain from this requirement up to the root Epic. The list is ordered
   * from immediate parent → root.
   */
  public List<RequirementRecord> getAncestors(UUID projectId, UUID requirementId) {
    ...
    while (current != null && visited.add(current)) {
      UUID parentId =
          requirementRepo.findById(current).map(PlatformRequirement::getParentId).orElse(null);
      // Null parent = root; cyclic parent = already visited — stop before adding either.
      if (parentId == null || visited.contains(parentId)) break;
      requirementRepo.findById(parentId).ifPresent(r -> chain.add(toRecord(r, projectId)));
      current = parentId;
    }
    return chain;
  }
  ...
  public List<RequirementRecord> getChildren(UUID projectId, UUID requirementId) {
    return requirementRepo.findByProjectIdAndParentId(projectId, requirementId).stream()
  ...
  public List<RequirementLink> getLinks(UUID projectId, UUID requirementId) {
    ...
    return edges.stream()
        ...
        .flatMap(
            e ->
                requirementRepo.findById(e.getToId()).stream()
                    .map(
                        r ->
                            new RequirementLink(
                                r.getId(),
                                ...
                                e.getLinkSubtype() != null
                                    ? LinkSubtype.valueOf(e.getLinkSubtype())
                                    : null)))
        .collect(Collectors.toList());
  }
  ...
  /**
   * Candidate test cases from requirements linked to this one via CLONED_FROM or RELATES_TO. Used
   * to detect REUSE_FROM_RELATED test-gen mode.
   */
  public List<PlatformTestCase> getReusableCandidates(UUID projectId, UUID requirementId) {
    List<PlatformTraceabilityEdge> linkedEdges =
        edgeRepo
            .findByProjectIdAndFromIdAndFromTier(projectId, requirementId, "REQUIREMENT")
            .stream()
            .filter(e -> EdgeType.LINKED_TO.name().equals(e.getEdgeType()))
            .filter(
                e ->
      ...
      requirementRepo
          ...
          .ifPresent(
              r -> {
                ...
                queue.addAll(
                    requirementRepo.findByProjectIdAndParentId(projectId, r.getId()).stream()
  ...
  public boolean hasCoverageFromOtherRequirements(UUID projectId, UUID requirementId) {
    ...
    for (PlatformTestCase tc : tcs) {
      ...
      boolean otherExists =
          coverageEdges.stream().anyMatch(e -> !requirementId.equals(e.getToId()));
  ...
  /**
   * Builds a complete RequirementContext for the given requirement, including graph neighbourhood
   * (ancestors, children, links, release scope) and resolving the TestGenMode.
   */
  ...
  /**
   * Determines the most appropriate generation strategy for a requirement: 1. No existing TCs →
   * CREATE_ALL 2. Candidates from related reqs cover ≥ 80% of ACs → REUSE_FROM_RELATED 3. Existing
   * TCs are NEEDS_UPDATE → UPDATE_CHANGED 4. Some ACs are uncovered → CREATE_FOR_NEW_ACS 5. All
   * covered and current → NO_ACTION
   */
  public TestGenMode resolveTestGenMode(UUID projectId, UUID requirementId) {
    ...
    boolean anyNeedsUpdate =
        existingTcs.stream().anyMatch(tc -> "NEEDS_UPDATE".equals(tc.getCoverageStatus()));
    if (anyNeedsUpdate) return TestGenMode.UPDATE_CHANGED;

    boolean allActive =
        existingTcs.stream().allMatch(tc -> "ACTIVE".equals(tc.getCoverageStatus()));
  ...
  public void upsertEdge(
      ...
      LinkSubtype subtype) {
    edgeRepo
        .findByProjectIdAndFromIdAndToIdAndEdgeType(projectId, fromId, toId, edgeType.name())
        .orElseGet(
            () -> {
  ...
  private RequirementRecord.IssueType toIssueType(String raw) {
    ...
    try {
      return RequirementRecord.IssueType.valueOf(raw);
  ...
  private IntegrationType resolveSource(UUID configId, UUID projectId) {
    ...
    return configRepo
        ...
        .map(
            c -> {
              try {
                return IntegrationType.valueOf(c.getIntegrationType());
              } catch (Exception e) {
