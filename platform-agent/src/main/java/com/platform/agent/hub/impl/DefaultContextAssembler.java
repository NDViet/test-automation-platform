...
/**
 * Builds a ContextBundle from live platform-core data.
 * Populates execution tier (pass rate, flakiness) and leaves requirement/test-case
 * tiers null until requirement-tracking entities are persisted in a later phase.
 */
@Component
public class DefaultContextAssembler implements ContextAssembler {
    ...
    public DefaultContextAssembler(ProjectRepository projectRepo,
    ...
    public ContextBundle resume(UUID workflowId, String checkpointId) {
        ...
        Project project = projectRepo.findById(workflow.getProjectId())
                .orElseThrow(() -> new IllegalArgumentException("project not found: " + workflow.getProjectId()));
        ...
        TriggerRef trigger = new TriggerRef(
                ...
                null, null, null, null, null, workflow.getCreatedAt());
    ...
    private ExecutionContext buildExecutionContext(UUID projectId) {
        ...
        List<TestExecution> recent = executionRepo
        ...
        return new ExecutionContext(lastRunId, lastRunAt, pass7d, flakinessAvg,
                consecutiveFails, failures, "default");
    }
    ...
    private double computeProjectFlakinessAvg(UUID projectId) {
        List<FlakinessScore> scores = flakinessRepo.findTopFlakyByProject(
    ...
    private List<ExecutionContext.FailureSample> buildFailureSamples(UUID projectId, Instant since) {
        List<TestExecution> executions = executionRepo.findByProjectAndDateRange(
                projectId, since, Instant.now());

        return executions.stream()
                .flatMap(exec -> testCaseResultRepo
                        .findByExecutionIdAndStatus(exec.getId(), TestStatus.FAILED)
                        .stream()
                        .limit(3))
                .limit(RECENT_FAILURE_LIMIT)
                .map(r -> new ExecutionContext.FailureSample(
                        r.getTestId(),
                        r.getDisplayName(),
                        r.getFailureMessage(),
                        r.getStackTrace(),
     ...
     * PlatformRequirement and delegates to GraphService for full context.
     * Returns null when no requirement is available (e.g. pure PR triggers).
     */
    private RequirementContext buildRequirementContext(UUID projectId, TriggerRef trigger) {
        ...
        try {
            return requirementRepo.findByProjectIdAndExternalId(projectId, trigger.entityExternalId())
                    .map(req -> graphService.buildRequirementContext(projectId, req.getId()))
                    .orElse(null);
        } catch (Exception e) {
            log.warn("Failed to build requirement context for trigger entity {}: {}",
                    trigger.entityExternalId(), e.getMessage());
            return null;
        }
    }

    /**
     * For GitHub webhook triggers, fetches changed files from the GitHub API and stores
     * the result in the DIFFS bucket. Returns null if not applicable or fetch fails.
     * The blob is referenced in ContextBundle so AnalysisNode can read it without an extra tool call.
     */
    private BlobRef prefetchPrDiff(TriggerRef trigger) {
        ...
        try {
            ...
            log.warn("Failed to pre-fetch PR diff for {}/{}#{}: {}", owner, repo, prNumber, e.getMessage());
            return null;
        }
    }
    ...
    private List<AgentTaskType> inferTaskTypes(TriggerRef trigger) {
        ...
        return switch (trigger.triggerType()) {
            ...
            case SCHEDULE  -> List.of(AgentTaskType.GENERATE_NIGHTLY_DIGEST, AgentTaskType.INVESTIGATE_FLAKINESS);
            case MANUAL    -> List.of(AgentTaskType.EXTRACT_ACCEPTANCE_CRITERIA,
                                      AgentTaskType.GENERATE_MANUAL_TEST_CASES);
