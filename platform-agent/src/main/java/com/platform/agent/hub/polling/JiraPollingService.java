 ...
 *
 * Auth: Basic base64(email:apiToken) — same scheme as JiraClient in platform-integration.
 * Jira Cloud description fields are ADF (Atlassian Document Format); plain text is extracted
 * by walking the content[] tree. Jira Server sends plain strings — both are handled.
 *
 * Config keys in ProjectIntegrationConfig.connectionParams:
 *   baseUrl    — https://yourorg.atlassian.net (Cloud) or https://jira.company.com (Server)
 *   email      — Atlassian account email (Cloud); use "username" key for Server
 *   username   — Basic auth username for Server (used when "email" is absent)
 *   apiToken   — API token (Cloud) or password (Server)
 *   projectKey — Jira project key, e.g. PROJ
 *
 * filterConfig keys (optional):
 *   jql        — JQL override; defaults to: project = {projectKey} ORDER BY created DESC
 */
@Service
public class JiraPollingService {
    ...
    public JiraPollingService(ProjectIntegrationConfigRepository configRepo,
    ...
    /**
     * Fetches Jira issues for the project and upserts them as platform requirements.
     * Returns the total number of issues synced (created or updated).
     */
    @Transactional
    public int syncNow(UUID projectId, String integrationType) {
        ProjectIntegrationConfig config = configRepo
        ...
        if (user == null || user.isBlank())
            throw new IllegalArgumentException("Jira config missing 'email'/'username' for project " + projectId);
        if (apiToken == null || apiToken.isBlank())
            throw new IllegalArgumentException("Jira config missing 'apiToken' for project " + projectId);
        if (projectKey == null || projectKey.isBlank())
            throw new IllegalArgumentException("Jira config missing 'projectKey' for project " + projectId);

        String jql = config.getFilterConfig() != null
        ...
        try {
            ...
            while (total < MAX_ISSUES) {
                StringBuilder body = new StringBuilder()
                        .append("{\"jql\":\"").append(jql.replace("\"", "\\\"")).append("\"")
                        .append(",\"maxResults\":").append(MAX_RESULTS)
                        .append(",\"fields\":[\"key\",\"summary\",\"description\",\"issuetype\",\"status\",\"priority\",\"parent\"]");
            ...
            log.info("Jira sync: {} issue(s) synced for project {} ({} parent links resolved)",
                    total, projectId, parentKeyMap.size());
    ...
    /**
     * For each child→parent external-ID pair collected during sync, looks up both
     * rows in the DB and calls updateParent() to wire the parentId + depth.
     * Depth is computed from the parent's depth + 1; if the parent's depth is still 0
     * (not yet resolved in this pass), it defaults to issueType-based depth.
     */
