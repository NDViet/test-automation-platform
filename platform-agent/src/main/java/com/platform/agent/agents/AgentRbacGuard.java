...
/**
 * Authorization for agent/task-agent writes, delegating to the platform-wide capability model. Per
 * the RBAC spec, agent management is a Tester capability: PROJECT-scoped writes require {@code
 * OPERATE_QUALITY}; ORG-scoped writes require {@code MANAGE_ORG}. A no-op while {@code
 * platform.security.enabled} is false (handled by {@link CapabilityEnforcer}). Reads are not gated
 * here.
 */
@Component
