...
/**
 * Client for platform-analytics APIs used by agent nodes. Provides Test Impact Analysis queries and
 * other platform data lookups.
 */
@Component
public class PlatformQueryTools {
  ...
  private static final String ANALYTICS_UNAVAILABLE_RESPONSE =
      "{\"risk\":\"UNKNOWN\",\"selectedTests\":[],\"reduction\":0,\"message\":\"Analytics service"
          + " unavailable\"}";

  private final String analyticsUrl;
  private final RestClient restClient;

  public PlatformQueryTools(
   ...
   *
   * <p>GET
   * {analyticsUrl}/api/v1/analytics/{projectId}/impact?changedFiles=file1&changedFiles=file2...
   *
  ...
  public String getTiaImpact(UUID projectId, List<String> changedFiles) {
    try {
      UriComponentsBuilder uriBuilder =
          UriComponentsBuilder.fromPath("/api/v1/analytics/{projectId}/impact");
      ...
      log.debug(
          "PlatformQueryTools: TIA request project={} changedFiles={}",
          projectId,
      ...
      log.warn(
          "PlatformQueryTools: analytics service unavailable at '{}', returning fallback. Cause:"
              + " {}",
          analyticsUrl,
          e.getMessage());
