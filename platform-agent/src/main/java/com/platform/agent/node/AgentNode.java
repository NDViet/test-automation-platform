...
/**
 * Stateless task executor contract for all Node types.
 * Each implementation handles a specific AgentTaskType using its LLM + tool set.
 * Nodes are identified by their NodeCapabilities and registered with the Hub on startup.
 */
public interface AgentNode {
    ...
    /**
     * Execute the task described by the ContextBundle.
     * Returns a NodeResult — never throws; errors are captured in NodeResult.status/errorMessage.
     * When human review is needed, returns status AWAITING_REVIEW with a non-null checkpointId.
     */
    ...
    /**
     * Dispatch a tool call from the Claude tool-use loop.
     * Returns the tool result as a plain string.
     * Return a string starting with "__AWAITING_REVIEW__" to pause and request human review.
     * Default implementation returns an acknowledgement for unknown tools.
     */
