...
/**
 * Generates nightly digest reports by querying platform-analytics and optionally posting a summary
 * to Slack.
 *
 * <p>Tools: platform_get_trends, platform_get_flakiness_leaderboard, platform_get_quality_gate,
 * slack_post_message
 */
@Component
public class InsightNode implements AgentNode {
  ...
  public AgentTaskType taskType() {
    return AgentTaskType.GENERATE_NIGHTLY_DIGEST;
  }
  ...
  private String handleGetTrends(String inputJson, ContextBundle bundle) {
    try {
      ...
      UUID projectId =
          UUID.fromString(input.path("project_id").asText(bundle.projectId().toString()));
  ...
  private String handleSlackPost(String inputJson, ContextBundle bundle) {
    ...
    try {
      ...
      log.info(
          "InsightNode posted digest to Slack channel {} for project {}",
          channel,
          bundle.projectId());
