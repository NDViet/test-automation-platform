 ...
 *
 * CREATE_ALL        — requirement has no coverage; generate new TCs from scratch.
 * UPDATE_CHANGED    — existing TCs are NEEDS_UPDATE; patch them to match new ACs.
 * REUSE_FROM_RELATED — candidates from a related req cover ≥ 80% of ACs; link them.
 *
 * Tools: save_test_cases, update_test_case, link_test_cases, request_review
 */
@Component
public class TestGenNode implements AgentNode {
    ...
    public TestGenNode(AgentOrchestrator orchestrator,
    ...
    @Override public AgentTaskType taskType() { return AgentTaskType.GENERATE_AUTOMATED_TESTS; }
    @Override public NodeType      nodeType() { return NodeType.TEST_GEN; }
    ...
    public List<Tool> tools() {
        return List.of(
                Tool.builder()
                        ...
                        .description("Persist new test cases for a requirement. " +
                                "Use for CREATE_ALL and CREATE_FOR_NEW_ACS modes.")
                        .inputSchema(Tool.InputSchema.builder()
                                .type(JsonValue.from("object"))
                                .putAdditionalProperty("properties", JsonValue.from(Map.of(
                                        "requirement_id", Map.of("type", "string",
                                                "description", "UUID of the requirement these TCs cover"),
                                        "test_cases", Map.of(
                                                "type", "array",
                                                ...
                                                "items", Map.of(
                                                        ...
                                                        "properties", Map.of(
                                                                ...
                                                                "ac_refs", Map.of("type", "array",
                                                                        "items", Map.of("type", "string")),
                                                                "has_automation", Map.of("type", "boolean")
                                                        ),
                ...
                Tool.builder()
                        ...
                        .description("Update an existing test case that is NEEDS_UPDATE. " +
                                "Use for UPDATE_CHANGED mode.")
                        .inputSchema(Tool.InputSchema.builder()
                                .type(JsonValue.from("object"))
                                .putAdditionalProperty("properties", JsonValue.from(Map.of(
                                        "test_case_id", Map.of("type", "string",
                                                "description", "UUID of the test case to update"),
                                        "new_title",   Map.of("type", "string"),
                                        "new_ac_refs", Map.of("type", "array",
                                                "items", Map.of("type", "string"))
                                )))
                ...
                Tool.builder()
                        ...
                        .description("Link existing test cases to a requirement via COVERED_BY edges. " +
                                "Use for REUSE_FROM_RELATED mode.")
                        .inputSchema(Tool.InputSchema.builder()
                                .type(JsonValue.from("object"))
                                .putAdditionalProperty("properties", JsonValue.from(Map.of(
                                        "requirement_id",  Map.of("type", "string"),
                                        "test_case_ids",   Map.of("type", "array",
                                                "items", Map.of("type", "string"),
                ...
                Tool.builder()
                        .name("request_review")
                        .description("Pause and request human review of the proposed test cases " +
    ...
    private String handleSaveTestCases(String inputJson, ContextBundle bundle) {
        try {
            ...
            for (JsonNode tc : testCases) {
                ...
                List<String> acRefs = mapper.convertValue(tc.path("ac_refs"),
                        mapper.getTypeFactory().constructCollectionType(List.class, String.class));

                PlatformTestCase entity = new PlatformTestCase(
                        bundle.projectId(), title, acRefs, "AGENT", bundle.sessionId());
                ...
                PlatformTraceabilityEdge edge = new PlatformTraceabilityEdge(
                        bundle.projectId(),
                        saved.getId(), "TEST_CASE",
                        requirementId, "REQUIREMENT",
                        "COVERED_BY");
                edgeRepo.save(edge);
                count++;
            }
            log.info("Saved {} test cases for requirement {} in project {}",
                    count, reqIdStr, bundle.projectId());
    ...
    private String handleUpdateTestCase(String inputJson, ContextBundle bundle) {
        try {
            ...
            return testCaseRepo.findById(tcId).map(tc -> {
                ...
                if (acRefsNode != null && acRefsNode.isArray()) {
                    List<String> refs = mapper.convertValue(acRefsNode,
                            mapper.getTypeFactory().constructCollectionType(List.class, String.class));
                    tc.setAcRefs(refs);
                    changed = true;
                }
    ...
    private String handleLinkTestCases(String inputJson, ContextBundle bundle) {
        try {
            ...
            for (JsonNode idNode : idsNode) {
                ...
                if (edgeRepo.findByProjectIdAndFromIdAndToIdAndEdgeType(
                        bundle.projectId(), tcId, reqId, "COVERED_BY").isEmpty()) {
                    PlatformTraceabilityEdge edge = new PlatformTraceabilityEdge(
                            bundle.projectId(), tcId, "TEST_CASE",
