...
/**
 * AUTOMATION_GEN node that generates automated test code from an approved manual test case and
 * creates a GitHub pull request with the generated test file.
 *
 * <p>Tools available to Claude: - github_create_file: creates a new test file on a branch -
...
public class AutomationCodeGenerationNode implements AgentNode {
  ...
  public AgentTaskType taskType() {
    return AgentTaskType.GENERATE_AUTOMATION_CODE;
  }

  @Override
  public NodeType nodeType() {
    return NodeType.AUTOMATION_GEN;
  }
  ...
  public NodeResult execute(ContextBundle bundle) {
    ...
    if (trigger == null || trigger.entityExternalId() == null) {
      return NodeResult.failed(
          ...
          "No testCaseId in trigger.entityExternalId",
          TokenUsage.zero());
    }
    ...
    if ("GENERATING".equals(reloaded.getAutomationStatus())) {
      log.warn(
          "AutomationCodeGenerationNode: PR was not created for test case {}, marking FAILED",
          testCaseId);
    ...
    String summary =
        prUrl != null
            ...
            : "Automation code generation attempted for test case '"
                + tc.getTitle()
  ...
  String handleCreateFile(
      String inputJson, ContextBundle bundle, ProjectIntegrationConfig githubConfig) {
    try {
      ...
      log.info(
          "AutomationCodeGenerationNode: creating file {}/{}/{}@{}", owner, repo, path, branch);
      return gitHubApiClient.createOrUpdateFile(
          owner, repo, path, message, content, null, branch, token);
    } catch (Exception e) {
  ...
  String handleCreatePr(
    ...
    try {
      ...
      try {
        ...
        log.warn(
            "AutomationCodeGenerationNode: could not parse PR response: {}", parseEx.getMessage());
      }
  ...
  private String buildTestCaseMessage(
      PlatformTestCase tc,
    ...
    sb.append(
        "Generate automated test code and create a GitHub PR for the following manual test"
            + " case.\n\n");
    sb.append("## Test Case\n");
    ...
    if (tc.getPreconditions() != null)
      sb.append("Preconditions: ").append(tc.getPreconditions()).append("\n");
              ...
              .ifPresent(
                  req -> {
                    ...
                    if (req.getAcceptanceCriteria() != null
                        && !req.getAcceptanceCriteria().isEmpty()) {
  ...
  private ProjectIntegrationConfig resolveGithubConfig(
      List<ProjectIntegrationConfig> configs, PlatformTestCase tc) {
    if (configs.isEmpty()) return null;

    // Prefer the config referenced by the test case's automationGithubConfigId
    if (tc.getAutomationGithubConfigId() != null) {
      Optional<ProjectIntegrationConfig> preferred =
          configs.stream()
  ...
  private String resolveToken(ContextBundle bundle, ProjectIntegrationConfig config) {
    ...
    if (bundle.projectId() != null) {
      Optional<String> cascade =
          tokenResolver.resolveToken(bundle.projectId(), IntegrationType.GITHUB);
  ...
  /**
   * Wraps AutomationCodeGenerationNode but carries test case and GitHub config so dispatchToolCall
   * can pass them to the handler methods.
   */
