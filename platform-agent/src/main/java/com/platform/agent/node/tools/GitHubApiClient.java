 ...
 * Uses RestClient (Spring 6.1+). All methods return raw JSON strings.
 * If a per-call token is blank, falls back to the configured default token.
 */
@Component
public class GitHubApiClient {
    ...
    public GitHubApiClient(@Value("${platform.agent.github.token:}") String defaultToken,
                           ObjectMapper mapper) {
     ...
     * Returns a JSON array of changed files: [{filename, status, additions, deletions, changes}].
     *
    ...
    public String getPrFiles(String owner, String repo, int prNumber, String token) {
        ...
        try {
            ...
            String msg = String.format("Failed to get PR files for %s/%s#%d: %s", owner, repo, prNumber, e.getMessage());
            log.error(msg, e);
     ...
     * Posts a comment on the PR (GitHub uses the issues endpoint for PR comments).
     *
     ...
     * @throws RuntimeException on HTTP error
     */
    public String postIssueComment(String owner, String repo, int prNumber, String commentBody, String token) {
        String bearerToken = resolveToken(token);
        ...
        try {
            ...
            String msg = String.format("Failed to post comment on %s/%s#%d: %s", owner, repo, prNumber, e.getMessage());
            log.error(msg, e);
            throw new RuntimeException(msg, e);
        }
    }
    ...
    /**
     * GET /repos/{owner}/{repo}/pulls?state=open&sort=updated&direction=desc&per_page=100
     * Returns a JSON array of open PRs sorted by most-recently-updated first.
     * Callers filter by updated_at client-side against their lastPolledAt timestamp.
     *
     * @param owner repo owner
     * @param repo  repo name
     * @param token GitHub token; if blank, falls back to defaultToken
    ...
    public String getOpenPRs(String owner, String repo, String token) {
        ...
        try {
            ...
            return restClient.get()
                    .uri("/repos/{owner}/{repo}/pulls?state=open&sort=updated&direction=desc&per_page=100",
                            owner, repo)
                    .header("Authorization", "Bearer " + bearerToken)
                    .retrieve()
                    .body(String.class);
        } catch (Exception e) {
            String msg = String.format("Failed to list open PRs for %s/%s: %s", owner, repo, e.getMessage());
            log.error(msg, e);
            throw new RuntimeException(msg, e);
        }
    }
     ...
     * Returns the raw JSON from GitHub, which includes a base64-encoded {@code content} field
     * and the {@code sha} required for subsequent update calls.
     *
     * @param owner repo owner
     * @param repo  repo name
    ...
    public String getFileContent(String owner, String repo, String path, String ref, String token) {
        ...
        try {
            ...
            RestClient.RequestHeadersSpec<?> spec = (ref == null || ref.isBlank())
            ...
            String msg = String.format("Failed to get file content for %s/%s/%s@%s: %s",
     ...
     */
    public String createOrUpdateFile(String owner, String repo, String path,
                                     String message, String contentBase64,
        ...
        try {
            ...
            String msg = String.format("Failed to create/update file %s/%s/%s: %s",
                    owner, repo, path, e.getMessage());
            log.error(msg, e);
            throw new RuntimeException(msg, e);
        }
    }
     ...
     * Creates a pull request (always as a draft for agent-generated PRs).
     *
     * @param owner  repo owner
     * @param repo   repo name
    ...
    public String createPullRequest(String owner, String repo, String title,
        ...
        try {
            ...
            String msg = String.format("Failed to create PR for %s/%s (%s→%s): %s",
                    owner, repo, head, base, e.getMessage());
            log.error(msg, e);
            throw new RuntimeException(msg, e);
        }
    }
    ...
    /**
     * Minimal JSON string escape — wraps value in quotes and escapes special chars.
     * Using this avoids pulling in a full JSON serializer for a single string field.
     */
