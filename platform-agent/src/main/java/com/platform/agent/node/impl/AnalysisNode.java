...
/**
 * ANALYSIS node that performs PR coverage gap analysis. Uses GitHub to fetch changed files,
 * platform TIA to identify impacted tests, and posts a structured review comment back to the PR.
 *
 * <p>Tools available to Claude: - github_get_pr_files: fetch changed files for a PR -
 * platform_get_tia_impact: get test impact analysis for changed files - github_post_pr_comment:
...
public class AnalysisNode implements AgentNode {
  ...
  public AgentTaskType taskType() {
    return AgentTaskType.ANALYZE_PR_DIFF;
  }
  ...
  private String handleGetTiaImpact(String inputJson, ContextBundle bundle) {
    try {
      ...
      log.debug(
          "AnalysisNode: TIA impact query for project {} with {} files",
          bundle.projectId(),
          changedFiles.size());
