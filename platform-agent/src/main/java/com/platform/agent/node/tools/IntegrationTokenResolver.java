...
/**
 * Resolves a decrypted access token for an integration from the Org→Team→Project credential cascade
 * ({@link CredentialResolver}). Used by agent nodes that call external systems (e.g. GitHub PR
 * creation) so they use the centralized, encrypted, per-project credentials rather than plaintext
 * config params.
 */
@Component
