 ...
 *
 * This node bypasses the Claude tool-use loop and instead:
 *   1. Loads requirements from the DB
 *   2. Calls Claude once via the orchestrator to get a JSON array of test case specs
 *   3. Persists each generated test case + its steps directly
 *
 * The orchestrator is invoked through a thin shim node that overrides execute()
 * so Claude gets a structured prompt and returns pure JSON (no tool calls needed).
 */
@Component
public class TestCaseGenerationNode implements AgentNode {
    ...
    public TestCaseGenerationNode(AgentOrchestrator orchestrator,
    ...
    @Override
    public AgentTaskType taskType() { return AgentTaskType.GENERATE_TEST_CASES; }

    @Override
    public NodeType nodeType() { return NodeType.TEST_GENERATION; }
    ...
    public NodeResult execute(ContextBundle bundle) {
        ...
        List<PlatformRequirement> allRequirements = requirementRepo.findByProjectIdOrderByUpdatedAtDesc(projectId);
        ...
        if (generatedCases.isEmpty()) {
            log.warn("TestCaseGenerationNode: Claude returned no parseable test cases for project {}", projectId);
            return NodeResult.completed(bundle.sessionId(), bundle.workflowId(),
                    nodeType(), taskType(),
                    ArtifactManifest.empty(),
                    "Generated 0 test cases from " + requirements.size() + " requirements (parse error or empty response).",
        ...
        for (GeneratedTestCase gen : generatedCases) {
            try {
                PlatformTestCase tc = new PlatformTestCase(
                        projectId, gen.title(), List.of(), "AGENT", bundle.workflowId());
                ...
                if (gen.steps() != null) {
                    ...
                    for (GeneratedStep step : gen.steps()) {
                        TestCaseStep stepEntity = new TestCaseStep(
                                saved.getId(), stepNum++,
                                step.action(),
                                step.expectedResult(),
                                step.notes());
                ...
                log.error("TestCaseGenerationNode: failed to save test case '{}': {}",
                        gen.title(), e.getMessage(), e);
            }
        }

        String summary = "Generated " + savedCount + " test cases from " + requirements.size() + " requirements.";
    ...
    /**
     * Filter requirements to only those whose IDs are listed in the trigger.
     * If the trigger's entityExternalId equals the project ID (all-requirements case)
     * or is null/blank, return all requirements unfiltered.
     */
    private List<PlatformRequirement> filterRequirements(List<PlatformRequirement> all,
    ...
    private String buildRequirementsMessage(List<PlatformRequirement> requirements) {
        ...
        sb.append("---\n");
        sb.append("Return ONLY a valid JSON array of test case objects. No prose before or after the JSON.");
        return sb.toString();
    }
    ...
    private List<GeneratedTestCase> parseGeneratedTestCases(String responseText) {
        ...
        if (json == null) {
            log.warn("TestCaseGenerationNode: could not extract JSON array from response: {}",
        ...
        try {
            ...
            for (JsonNode node : array) {
                try {
                    ...
                    log.warn("TestCaseGenerationNode: skipping unparseable test case node: {}", e.getMessage());
                }
            }
            return result;
        } catch (Exception e) {
    ...
    private GeneratedTestCase parseTestCaseNode(JsonNode node) {
        ...
        String expectedResult    = node.path("expectedResult").asText(
                                       node.path("expected_result").asText(null));
        String priority          = node.path("priority").asText("MEDIUM");
        String sourceReqId       = node.path("sourceRequirementId").asText(
                                       node.path("source_requirement_id").asText(null));
        ...
        if (stepsNode.isArray()) {
            for (JsonNode stepNode : stepsNode) {
                ...
                String expResult = stepNode.path("expectedResult").asText(
                                       stepNode.path("expected_result").asText(null));
                String notes     = stepNode.path("notes").isNull() ? null
                                                                    : stepNode.path("notes").asText(null);
                if (action != null) {
        ...
        return new GeneratedTestCase(title, description, preconditions, expectedResult,
                priority, sourceReqId, steps);
    }

    /**
     * Extracts the first JSON array from the response text,
     * handling markdown code fences (```json ... ```) and bare arrays.
     */
    ...
    /**
     * A thin wrapper AgentNode that replaces the user message seen by the orchestrator.
     * The orchestrator calls buildUserPrompt(bundle) internally; we can't override that
     * directly, so we supply a custom systemPrompt that embeds the requirements and
     * instructs Claude to respond with pure JSON — the orchestrator's generic user
     * prompt ("Task: GENERATE_TEST_CASES") becomes harmless.
     */
    private static class TextOnlyNode implements AgentNode {
        ...
        @Override
        public AgentTaskType taskType() { return parent.taskType(); }

        @Override
        public NodeType nodeType() { return parent.nodeType(); }
