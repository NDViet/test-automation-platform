...
/**
 * Node that extracts structured acceptance criteria from a requirement description.
 * Tools available to Claude:
 *   - store_acceptance_criteria: persist the extracted ACs as structured JSON
...
public class ExtractAcceptanceCriteriaNode implements AgentNode {
    ...
    @Override
    public AgentTaskType taskType() { return AgentTaskType.EXTRACT_ACCEPTANCE_CRITERIA; }

    @Override
    public NodeType nodeType() { return NodeType.REQUIREMENT; }
    ...
    public List<Tool> tools() {
        return List.of(
                Tool.builder()
                        ...
                        .description("Store the extracted acceptance criteria for the requirement. " +
                                                ...
                                                "items", Map.of(
                                                        ...
                                                        "properties", Map.of(
                                                                ...
                                                                "priority", Map.of("type", "string",
                                                                        "enum", List.of("MUST", "SHOULD", "COULD")))
                                                )
                                        )
                                )))
                ...
                Tool.builder()
                        .name("request_review")
                        .description("Pause execution and send the AC draft to a human reviewer " +
                                "when you need QA engineer validation before proceeding.")
                        .inputSchema(Tool.InputSchema.builder()
                                .type(JsonValue.from("object"))
                                .putAdditionalProperty("properties", JsonValue.from(Map.of(
                                        "summary", Map.of("type", "string",
                                                "description", "Brief description of what is being reviewed"),
                                        "payload", Map.of("type", "string",
                                                "description", "The full draft content for review")
                                )))
                                .addRequired("summary")
                                .addRequired("payload")
                                .build())
