 ...
 *
 * <p>Flow: 1. Build system prompt from ContextBundle 2. Maintain messages list across turns 3. Call
 * Claude API; if stop_reason=tool_use → execute tools → append results → repeat 4. If
 * stop_reason=end_turn → extract final output, return NodeResult.completed() 5. Nodes can signal
 * AWAITING_REVIEW by including a sentinel in their tool output
 */
@Component
public class ClaudeAgentOrchestrator implements AgentOrchestrator {
  ...
  public ClaudeAgentOrchestrator(
  ...
  public NodeResult run(ContextBundle bundle, AgentNode node) {
    ...
    messages.add(MessageParam.builder().role(MessageParam.Role.USER).content(userPrompt).build());
    ...
    try {
      ...
      while (iterations++ < MAX_TOOL_ITERATIONS) {
        MessageCreateParams.Builder reqBuilder =
        ...
        int cacheWrite = response.usage().cacheCreationInputTokens().map(Long::intValue).orElse(0);
        int cacheRead = response.usage().cacheReadInputTokens().map(Long::intValue).orElse(0);
        totalUsage =
            totalUsage.add(
                new TokenUsage(
                    inputFresh,
        ...
        List<ContentBlockParam> assistantBlocks =
            response.content().stream().map(ContentBlock::toParam).toList();
        messages.add(
        ...
        if (stopReason == StopReason.TOOL_USE) {
          ...
          for (ContentBlock block : response.content()) {
            if (block.isToolUse()) {
              ...
              String toolResult = dispatchToolCall(tu.name(), tu._input().toString(), bundle, node);
        ...
        .orElseGet(
            () ->
                NodeResult.failed(
                    ...
                    "Checkpoint " + checkpointId + " not found or expired",
                    TokenUsage.zero()));
  }

  // -------------------------------------------------------------------------

  protected String dispatchToolCall(
      String toolName, String inputJson, ContextBundle bundle, AgentNode node) {
  ...
  private String saveCheckpoint(
      ContextBundle bundle, List<MessageParam> messages, TokenUsage usage) {
    try {
  ...
  private BigDecimal estimateCost(
      int inputFresh, int cacheWrite, int cacheRead, int output, Model model) {
