...
/**
 * Scheduled job that runs nightly (default: 02:00 UTC) to classify any FAILED test results from the
 * previous 24 hours that have not yet been analysed by Claude.
 *
 * <p>Configure via {@code ai.batch.cron} in {@code application.yml}.
 */
@Component
public class NightlyAnalysisBatchJob {
  ...
  public NightlyAnalysisBatchJob(
  ...
  public void run() {
    ...
    for (TestCaseResult failure : failures) {
      try {
        ...
        log.warn(
            "[AI Batch] Failed to classify testId={}: {}", failure.getTestId(), e.getMessage());
        errors++;
      }
    }
