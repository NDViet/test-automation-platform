...
/**
 * Consumes {@link UnifiedTestResult} events from {@code test.results.raw} and, when real-time AI
 * analysis is enabled, immediately classifies any FAILED test cases using Claude.
 *
 * <p>Enable via {@code ai.realtime.enabled=true} (default: false) to avoid incurring Claude API
 * costs on every run during initial rollout.
 */
@Component
public class AnalysisEventConsumer {
  ...
  void logMode() {
    log.info(
        "[AI Consumer] Started — real-time analysis is controlled via AI Settings in the portal"
            + " (currently: {})",
  ...
  public void consume(ConsumerRecord<String, UnifiedTestResult> record) {
    ...
    // Look up the persisted execution by its stable runId (written by ingestion before Kafka
    // publish)
    Optional<TestExecution> execution = executionRepo.findByRunId(run.runId());
    if (execution.isEmpty()) {
      log.warn(
          "[AI Consumer] Execution not found in DB for runId={} — ingestion may not have committed"
              + " yet",
          run.runId());
      return;
    }
    ...
    List<TestCaseResult> failures =
        resultRepo.findByExecutionIdAndStatusIn(execution.get().getId(), List.of(FAILED, BROKEN));
    ...
    for (TestCaseResult failure : failures) {
      try {
        ...
        log.warn(
            "[AI Consumer] Classification failed for testId={}: {}",
            failure.getTestId(),
            e.getMessage());
      }
    }
