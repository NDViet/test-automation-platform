 ...
 *
 * <p>Active when {@code ai.provider=claude} (the default). Wraps the Anthropic Java SDK; parses the
 * JSON response after stripping any optional markdown fences. Reports real token usage from the API
 * response.
 */
...
public class ClaudeApiClient implements AiClient {
  ...
  /**
   * Resolves the API key to use, in priority order: 1. Key saved via Portal (platform_settings
   * table) — updated at runtime 2. Key from application.yml / ANTHROPIC_API_KEY env var — set at
   * startup
   */
