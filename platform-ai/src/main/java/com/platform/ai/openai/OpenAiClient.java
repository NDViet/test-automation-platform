 ...
 *
 * <p>Active when {@code ai.provider=openai}. Uses Spring {@link RestClient} to call {@code POST
 * /v1/chat/completions}. Requests {@code response_format: {type: "json_object"}} to guarantee valid
 * JSON output, eliminating the need for markdown-fence stripping on the happy path. Reports real
...
public class OpenAiClient implements AiClient {
  ...
  private AiAnalysisResponse parseResponse(String rawBody) {
    try {
      ...
      // Extract content
      String content =
          response.choices() != null && !response.choices().isEmpty()
