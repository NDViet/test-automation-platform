 ...
 *
 * <p>GET /api/v1/ai/settings — read current settings (api-key masked) PUT /api/v1/ai/settings —
 * update settings POST /api/v1/ai/settings/test — test API key connectivity
 */
@RestController
...
public class AiSettingsController {
  ...
  public ResponseEntity<TestConnectionResult> testConnection(
    ...
    try {
      boolean ok =
          "openai".equalsIgnoreCase(provider) ? testOpenAi(key, model) : testClaude(key, model);
      return ResponseEntity.ok(
          new TestConnectionResult(ok, ok ? "Connection successful" : "API returned an error"));
    } catch (Exception e) {
      return ResponseEntity.ok(
          new TestConnectionResult(false, "Connection failed: " + e.getMessage()));
    }
  }
  ...
  private String resolveKey(String primaryKey, String legacyKey) {
    String v =
        repo.findById(primaryKey)
            .map(PlatformSetting::getValue)
            .filter(s -> s != null && !s.isBlank())
            .orElse(null);
    if (v != null) return v;
    v =
        repo.findById(legacyKey)
            .map(PlatformSetting::getValue)
            .filter(s -> s != null && !s.isBlank())
            .orElse(null);
    return v;
