 ...
 *
 * <ul>
 *   <li>Compact system prompt — one-line category definitions instead of verbose paragraphs
 *   <li>Stack trace capped at {@value #MAX_STACK_LINES} lines (key frames only)
 *   <li>History limited to {@value #MAX_HISTORY_RUNS} recent runs in a compact format
 *   <li>No padding or filler text — every token carries signal
 * </ul>
 */
@Component
public class PromptBuilder {
  ...
  /**
   * Compact system prompt. Explicit JSON schema + one-line category descriptions. ~200 tokens vs
   * ~350 for the original.
   */
  public static final String SYSTEM_PROMPT =
      "You are a test automation expert. Analyse the test failure and reply with ONLY valid JSON:\n"
          + "{\"category\":\"...\",\"confidence\":0.0,\"rootCause\":\"...\",\"detailedAnalysis\":\"...\",\"suggestedFix\":\"...\",\"isFlakyCandidate\":false,\"affectedComponent\":\"...\"}\n"
          + "Categories (pick one):\n"
          ...
          + "When page HTML is provided, inspect the DOM and suggest correct alternative selectors"
          + " in suggestedFix.";

  public String buildUserPrompt(
      String testId, String failureMessage, String stackTrace, List<TestCaseResult> history) {
   ...
   *
   * @param diagnostics may contain {@code platform.diagnostic.dom} (page HTML) and {@code
   *     platform.diagnostic.selector} (failed CSS/XPath selector)
   */
  public String buildUserPrompt(
      String testId,
    ...
    if (!history.isEmpty()) {
      ...
      for (int i = 0; i < limit; i++) {
        ...
        String env =
            r.getExecution() != null && r.getExecution().getEnvironment() != null
  ...
  private String truncate(String text, int maxLines) {
    ...
    return Arrays.stream(lines, 0, maxLines).collect(Collectors.joining("\n")) + "\n...(truncated)";
  }
}
