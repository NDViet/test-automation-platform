...
/**
 * Orchestrates failure analysis: builds the prompt, calls the AI provider, persists the resulting
 * {@link FailureAnalysis}, and emits Micrometer metrics.
 *
 * <p>Idempotent: if a {@link FailureAnalysis} already exists for the given {@code
 * testCaseResultId}, the existing record is returned without another API call.
 */
@Service
public class FailureClassificationService {
  ...
  /**
   * Classify with additional diagnostic context (DOM snapshot, selector) forwarded from the testkit
   * via the test result environment map.
   *
   * @param diagnostics key-value pairs from {@code TestCaseResultDto.environment()} — may contain
   *     {@code platform.diagnostic.dom} and {@code platform.diagnostic.selector}
   */
  @Transactional
  public FailureAnalysis classify(
      TestCaseResult failure, UUID projectId, Map<String, String> diagnostics) {
    ...
    List<TestCaseResult> history =
        resultRepo.findLatestByTestIdAndProjectId(
    ...
    log.info(
        "[AI] Classifying failure testId={} projectId={} provider={} hasDom={}",
        failure.getTestId(),
        projectId,
        provider,
    ...
    log.info(
        "[AI] Saved analysis id={} category={} confidence={} tokens=in:{} out:{}",
        saved.getId(),
        saved.getCategory(),
        saved.getConfidence(),
