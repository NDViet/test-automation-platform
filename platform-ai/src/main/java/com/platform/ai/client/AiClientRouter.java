...
/**
 * Routes every AI analysis call to the provider that is currently configured in platform_settings
 * ({@code ai.provider = "claude" | "openai"}).
 *
 * <p>Marked {@code @Primary} so Spring injects this bean everywhere {@link AiClient} is autowired.
 * Both concrete clients are always registered; the active one is selected at call time, so changing
 * the provider in the Portal takes effect immediately without a service restart.
 */
@Primary
@Component
public class AiClientRouter implements AiClient {
  ...
  /**
   * Selects the provider for a project via the Org→Team→Project settings cascade. Falls back to the
   * global {@code ai.provider} setting, then to claude.
   */
