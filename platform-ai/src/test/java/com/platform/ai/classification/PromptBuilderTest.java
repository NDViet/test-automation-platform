...
class PromptBuilderTest {
  ...
  void includesTestIdAndFailureMessage() {
    String prompt =
        builder.buildUserPrompt(
            "com.example.LoginTest#login", "Expected 200 but got 500", null, List.of());
  ...
  void truncatesStackTraceAt20Lines() {
    ...
    long lineCount = prompt.lines().filter(l -> l.equals("line")).count();
  ...
  void includesRunHistory() {
    List<TestCaseResult> history =
        List.of(makeResult(TestStatus.FAILED, "staging"), makeResult(TestStatus.PASSED, "staging"));
  ...
  private TestCaseResult makeResult(TestStatus status, String env) {
    ...
    return TestCaseResult.builder().execution(exec).testId("test#m").status(status).build();
  }
}
