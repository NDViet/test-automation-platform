 ...
 *
 * <p>Fetches automated-test references from the repo item tree, and supports
 * create-PR / PR-thread-comment push commands. {@code COMMIT_FILE} via the Azure
 * Pushes API is not implemented (requires the latest commit objectId); use the
 * GitHub adapter for agent commit flows, or extend here. Credentials:
 * {@code connectionParams.pat} (+ {@code organization}, {@code project},
 * {@code repository}, optional {@code base_url}).</p>
 */
@Component
public class AzureReposAdapter
    ...
    public PagedRecords<AutomatedTestRef> fetchPage(SourceIntegrationConfig config, SyncCursor cursor) {
        try {
            JsonNode items = client(config).get()
                    .uri("/{p}/_apis/git/repositories/{r}/items?recursionLevel=Full&" + API,
                            project(config), repo(config))
        ...
        return switch (command.op()) {
            case CREATE_PR -> {
                JsonNode resp = client.post()
                        .uri("/{p}/_apis/git/repositories/{r}/pullrequests?" + API,
                                project(config), repo(config))
                        .header("Content-Type", "application/json")
                        .body(Map.of(
                                "sourceRefName", "refs/heads/" + command.branch(),
                                "targetRefName", "refs/heads/main",
                                "title", command.title(),
                                "description", command.body() == null ? "" : command.body()))
                        .retrieve().body(JsonNode.class);
                yield resp == null ? null : resp.path("pullRequestId").asText(null);
            }
            case POST_PR_COMMENT -> {
                JsonNode resp = client.post()
                        .uri("/{p}/_apis/git/repositories/{r}/pullRequests/{id}/threads?" + API,
                                project(config), repo(config), command.prId())
                        .header("Content-Type", "application/json")
                        .body(Map.of("status", "active",
                                "comments", List.of(Map.of("content",
                                        command.body() == null ? "" : command.body()))))
                        .retrieve().body(JsonNode.class);
                yield resp == null ? null : resp.path("id").asText(null);
            }
            case COMMIT_FILE -> {
                log.warn("[AzureReposAdapter] COMMIT_FILE not implemented (Pushes API requires latest objectId)");
                yield null;
            }
        };
    }
    ...
    private boolean isTestFile(String path) {
        ...
        return p.endsWith("test.java") || p.endsWith("tests.java")
                || p.endsWith(".spec.ts") || p.endsWith(".test.ts")
                || p.endsWith(".feature")
                || p.contains("/test/") || p.contains("/tests/");
    }

    private AutomatedTestRef toRef(String path) {
        String fileName = path.contains("/") ? path.substring(path.lastIndexOf('/') + 1) : path;
        String className = fileName.contains(".") ? fileName.substring(0, fileName.indexOf('.')) : fileName;
        String fw = path.toLowerCase().endsWith(".feature") ? "CUCUMBER"
    ...
    public record AzureRepoCommand(Op op, String branch, String title, String body, String prId) {
        public enum Op { CREATE_PR, POST_PR_COMMENT, COMMIT_FILE }
    }
}
