 ...
 *
 * <p>Distinct from {@link GitHubAdapter} (Tier 3 = source repos). Maps issues to {@link
 * RequirementRecord}s and handles {@code issues} webhook payloads. Credentials: {@code
 * connectionParams.pat} (+ {@code owner}, {@code repo} or {@code repository}, optional {@code
 * base_url} for GHE).
 */
@Component
public class GitHubIssuesAdapter
  ...
  @Override
  public PagedRecords<RequirementRecord> fetchPage(
  ...
  public String push(SourceIntegrationConfig config, GitHubIssueCommand command) {
    ...
    JsonNode resp =
        client(config)
            ...
            .body(
                Map.of(
                    "title", command.title(), "body", command.body() == null ? "" : command.body()))
            .retrieve()
            .body(JsonNode.class);
