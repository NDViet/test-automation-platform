 ...
 *
 * <p>Fetches automated-test references from the repo tree, handles
 * commit/PR/comment push commands, and maps push-webhook payloads to changed
 * test files. Credentials: {@code connectionParams.pat} (+ {@code owner},
 * {@code repo} or {@code repository}, optional {@code base_url} for GHE).</p>
 */
@Component
public class GitHubAdapter implements IntegrationAdapter<AutomatedTestRef, GitHubAdapter.GitHubCommand> {
    ...
    public PagedRecords<AutomatedTestRef> fetchPage(SourceIntegrationConfig config, SyncCursor cursor) {
        try {
            ...
            JsonNode repo = client.get().uri("/repos/{o}/{r}", or[0], or[1])
        ...
        return switch (command.op()) {
            ...
            case CREATE_PR -> {
                JsonNode resp = client.post()
                        ...
                        .header("Content-Type", "application/json")
                        .body(Map.of("title", command.commitMessage(), "head", command.branch(),
                                "base", "main", "body", command.reviewBody() == null ? "" : command.reviewBody(),
                                "draft", true))
                        .retrieve().body(JsonNode.class);
    ...
    private String framework(String path) {
        ...
        if (p.endsWith(".spec.ts") || p.endsWith(".spec.js")
                || p.endsWith(".test.ts") || p.endsWith(".test.js")) return "PLAYWRIGHT";
        return "JUNIT5";
    }
