  ...
  void fromWebhook_mapsAddedAndModifiedTestFiles() throws Exception {
    ...
    WebhookEvent event =
        new WebhookEvent(
            ...
            WebhookEvent.EventAction.UPDATED,
            "push",
            "abc",
            raw,
            Instant.now());
    ...
    assertThat(refs)
        .extracting(AutomatedTestRef::filePath)
        .containsExactlyInAnyOrder(
            "src/test/java/com/example/LoginTest.java", "e2e/checkout.spec.ts");
    assertThat(refs)
        .extracting(AutomatedTestRef::framework)
