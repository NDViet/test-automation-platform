...
class AlertRuleEngineTest {
  ...
  void setUp() {
    ...
    props.setRules(
        List.of(
            new AlertRule(
                "High Failure Rate",
                AlertRule.Metric.PASS_RATE_BELOW,
                0.80,
                AlertRule.Severity.HIGH,
                true),
            new AlertRule(
                "Failure Spike",
                AlertRule.Metric.NEW_FAILURES_ABOVE,
                5,
                AlertRule.Severity.MEDIUM,
                true),
            new AlertRule(
                "Broken Tests",
                AlertRule.Metric.BROKEN_TESTS_ABOVE,
                3,
                AlertRule.Severity.HIGH,
                true),
            new AlertRule(
  ...
  void alertEventContainsCorrectMetadata() {
    ...
    List<AlertEvent> events = engine.evaluate(result);
    AlertEvent failureSpike =
        events.stream().filter(e -> e.ruleName().equals("Failure Spike")).findFirst().orElseThrow();
