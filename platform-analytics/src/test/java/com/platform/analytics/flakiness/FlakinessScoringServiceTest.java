...
class FlakinessScoringServiceTest {
  ...
  @Test
  void classifyStable() {
    assertThat(FlakinessScoringService.classify(0.05))
        .isEqualTo(FlakinessScore.Classification.STABLE);
    assertThat(FlakinessScoringService.classify(0.0))
        .isEqualTo(FlakinessScore.Classification.STABLE);
  }

  @Test
  void classifyWatch() {
    assertThat(FlakinessScoringService.classify(0.10))
        .isEqualTo(FlakinessScore.Classification.WATCH);
    assertThat(FlakinessScoringService.classify(0.25))
        .isEqualTo(FlakinessScore.Classification.WATCH);
  }

  @Test
  void classifyFlaky() {
    assertThat(FlakinessScoringService.classify(0.30))
        .isEqualTo(FlakinessScore.Classification.FLAKY);
    assertThat(FlakinessScoringService.classify(0.50))
        .isEqualTo(FlakinessScoringService.classify(0.50));
  }

  @Test
  void classifyCriticalFlaky() {
    assertThat(FlakinessScoringService.classify(0.60))
        .isEqualTo(FlakinessScore.Classification.CRITICAL_FLAKY);
    assertThat(FlakinessScoringService.classify(1.0))
        .isEqualTo(FlakinessScore.Classification.CRITICAL_FLAKY);
  }
  ...
  void recencyWeightHigherWhenRecentFailures() {
    ...
    List<TestCaseResult> recentFailures =
        List.of(
    ...
    List<TestCaseResult> oldFailures =
        List.of(
  ...
  void computeAndPersistSkipsWhenNoHistory() {
    ...
    verify(scoreRepo, never())
        .upsert(any(), any(), any(), any(), anyInt(), anyInt(), any(), any(), any());
  }
  ...
  void computeAndPersistCallsUpsertWithCorrectClassification() {
    ...
    verify(scoreRepo)
        .upsert(
            any(), any(), any(), classCaptor.capture(), anyInt(), anyInt(), any(), any(), any());
