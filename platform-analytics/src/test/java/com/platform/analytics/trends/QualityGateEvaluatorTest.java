...
class QualityGateEvaluatorTest {
  ...
  void passesAt80PercentExactly() {
    ...
    List<TestCaseResultDto> cases =
        List.of(testCase(TestStatus.FAILED), testCase(TestStatus.FAILED));
  ...
  void failsWhenPassRateBelowThreshold() {
    ...
    List<TestCaseResultDto> cases =
        List.of(
            testCase(TestStatus.FAILED), testCase(TestStatus.FAILED), testCase(TestStatus.FAILED));
  ...
  void failsWhenTooManyNewFailures() {
    ...
    QualityGateResult gate = evaluator.evaluate(result, projectId);
    assertThat(gate.passed()).isFalse();
    assertThat(gate.violations().stream().anyMatch(v -> v.contains("failure count"))).isTrue();
  }
  ...
  private UnifiedTestResult buildResult(
      int total, int passed, int failed, int broken, List<TestCaseResultDto> cases) {
