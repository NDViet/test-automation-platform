...
/**
 * Evaluates configured alert rules against an incoming {@link UnifiedTestResult} and fires {@link
 * AlertEvent}s for any breached rules.
 *
 * <p>Use {@link #evaluate(UnifiedTestResult, UUID)} when a project UUID is available (e.g. from the
 * Kafka consumer) to also evaluate DB-backed rules such as {@link
 * AlertRule.Metric#CRITICAL_FLAKY_COUNT_ABOVE} and persist alert history.
 */
@Service
public class AlertRuleEngine {
  ...
  /**
   * Evaluates rules that do not require a project UUID (pass-rate, failure count). DB-backed rules
   * (CRITICAL_FLAKY_COUNT_ABOVE) are skipped.
   */
  ...
  /**
   * Evaluates all enabled rules including DB-backed ones, persists history, and dispatches
   * notifications.
   *
  ...
  public List<AlertEvent> evaluate(UnifiedTestResult result, UUID projectId) {
    ...
    if (!fired.isEmpty()) {
      log.info(
          "[Alerts] Fired {} alert(s) for run={} project={}",
          fired.size(),
          result.runId(),
          result.projectId());
    }
  ...
  private AlertEvent evaluatePassRate(AlertRule rule, UnifiedTestResult result) {
    ...
    if (passRate < rule.threshold()) {
      return AlertEvent.of(
          rule,
          String.format(
              "Pass rate %.1f%% breached threshold %.1f%% in project=%s branch=%s",
              passRate * 100, rule.threshold() * 100, result.projectId(), result.branch()),
          result.teamId(),
          result.projectId(),
          result.runId());
    }
  ...
  private AlertEvent evaluateNewFailures(AlertRule rule, UnifiedTestResult result) {
    long failures =
        result.testCases().stream().filter(tc -> tc.status() == TestStatus.FAILED).count();
    if (failures > rule.threshold()) {
      return AlertEvent.of(
          rule,
          String.format(
              "%d new failures exceeded threshold %.0f in project=%s branch=%s",
  ...
  private AlertEvent evaluateBrokenTests(AlertRule rule, UnifiedTestResult result) {
    long broken =
        result.testCases().stream().filter(tc -> tc.status() == TestStatus.BROKEN).count();
    if (broken > rule.threshold()) {
      return AlertEvent.of(
          rule,
          String.format(
              "%d broken tests exceeded threshold %.0f in project=%s",
  ...
  private AlertEvent evaluateCriticalFlaky(
      AlertRule rule, UnifiedTestResult result, UUID projectId) {
    if (projectId == null) return null; // requires DB access — skip if no project UUID
    long criticalCount =
        scoreRepo.findTopFlakyByProject(projectId, PageRequest.of(0, 10_000)).stream()
    ...
    if (criticalCount > rule.threshold()) {
      return AlertEvent.of(
          rule,
          String.format(
              "%d CRITICAL_FLAKY tests exceeded threshold %.0f in project=%s",
