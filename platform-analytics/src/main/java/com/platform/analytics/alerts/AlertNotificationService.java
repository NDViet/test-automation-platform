...
/**
 * Dispatches alert notifications. Currently supports Slack incoming webhooks. All failures are
 * logged at WARN and swallowed — alerts must never affect the main flow.
 */
@Service
public class AlertNotificationService {
  ...
  /**
   * Sends the alert to all configured channels. Never throws. Returns a comma-separated string of
   * channels that were attempted.
   */
  public String send(AlertEvent event) {
    log.warn(
        "[Alert] severity={} rule='{}' — {}", event.severity(), event.ruleName(), event.message());
  ...
  private void postWebhook(AlertEvent event, String webhookUrl) {
    try {
      String body =
          ("{\"id\":\"%s\",\"ruleName\":\"%s\",\"severity\":\"%s\","
                  + "\"message\":\"%s\",\"teamId\":\"%s\",\"projectId\":\"%s\","
                  + "\"runId\":\"%s\",\"firedAt\":\"%s\"}")
              .formatted(
                  event.id(),
                  escapeJson(event.ruleName()),
                  event.severity(),
                  escapeJson(event.message()),
                  event.teamId(),
                  event.projectId(),
      ...
      log.warn(
          "[Alert] Failed to send generic webhook notification (non-fatal): {}", e.getMessage());
    }
  }
