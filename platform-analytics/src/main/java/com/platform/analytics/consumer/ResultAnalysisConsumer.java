...
/**
 * Consumes {@link UnifiedTestResult} events from the {@code test.results.raw} topic and triggers
 * flakiness scoring, quality gate evaluation, and alert rule evaluation.
 */
@Component
public class ResultAnalysisConsumer {
  ...
  public void consume(ConsumerRecord<String, UnifiedTestResult> record) {
    ...
    log.info(
        "[Analytics] Received run={} team={} project={} tests={}",
        result.runId(),
        result.teamId(),
        result.projectId(),
        result.total());
    ...
    if (projectId == null) {
      log.warn(
          "[Analytics] Could not resolve project for team={} project={} — skipping",
          result.teamId(),
          result.projectId());
      return;
    }
    ...
    if (!gate.passed()) {
      log.warn(
          "[Analytics] Quality gate FAILED for run={} violations={}",
          result.runId(),
          gate.violations());
    } else {
      log.info(
          "[Analytics] Quality gate PASSED for run={} passRate={}%",
