 ...
 *
 * <ul>
 *   <li>Release gate: called by CI just before a deployment tag is created
 *   <li>Weekly digest: auto-generated every Monday morning
 * </ul>
 */
@Service
...
public class ReleaseReportService {
  ...
  public ReleaseReportDto generate(UUID projectId, Instant from, Instant to, String releaseTag) {
    ...
    // Flakiness summary
    List<FlakinessScore> allScores =
        scoreRepo.findTopFlakyByProject(projectId, PageRequest.of(0, 10_000));
    int flakyCount =
        (int)
            allScores.stream()
                .filter(
                ...
                .count();
    List<FlakinessScore> criticalScores =
        allScores.stream()
            .filter(s -> s.getClassification() == FlakinessScore.Classification.CRITICAL_FLAKY)
            .limit(TOP_FLAKY_LIMIT)
            .toList();
    List<String> topCritical = criticalScores.stream().map(FlakinessScore::getTestId).toList();
  ...
  private List<ReleaseReportDto.TopFailingTest> buildTopFailing(
    ...
    for (TestExecution exec : executions) {
      List<TestCaseResult> results =
          resultRepo.findByExecutionIdAndStatus(exec.getId(), TestStatus.FAILED);
    ...
    return failCounts.entrySet().stream()
        ...
        .map(
            e ->
                new ReleaseReportDto.TopFailingTest(
                    e.getKey(),
                    e.getValue(),
  ...
  private QualityGateResult evaluateGate(List<TestExecution> executions, UUID projectId) {
    ...
    String orgSlug =
        latest.getProject().getOrganization() != null
            ? latest.getProject().getOrganization().getSlug()
            : "unknown";
    UnifiedTestResult stub =
        new UnifiedTestResult(
            latest.getRunId(),
            orgSlug,
            latest.getProject().getSlug(),
            latest.getBranch(),
            latest.getEnvironment(),
            latest.getCommitSha(),
            latest.getTriggerType(),
            latest.getCiProvider(),
            latest.getCiRunUrl(),
            latest.getExecutedAt(),
            latest.getTotalTests(),
            latest.getPassed(),
            latest.getFailed(),
            latest.getSkipped(),
            latest.getBroken(),
            latest.getDurationMs(),
            latest.getSourceFormat(),
            List.of(),
            latest.getExecutionMode(),
            latest.getParallelism(),
            latest.getSuiteName(),
            null,
            null,
            null,
