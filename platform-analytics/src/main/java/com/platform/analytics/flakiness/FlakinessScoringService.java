...
public class FlakinessScoringService {
  ...
  public FlakinessScoringService(
    ...
    Gauge.builder(
            "platform.flakiness.critical.count",
            scoreRepo,
    ...
    Gauge.builder(
            "platform.flakiness.watch.count",
            scoreRepo,
  ...
  /**
   * Computes the flakiness score for {@code testId} in {@code projectId} and upserts the result.
   * No-op if there is no run history.
   */
  @Transactional
  ...
  ScoringInput buildInput(List<TestCaseResult> history) {
    ...
    Instant lastFailed =
        history.stream()
            ...
            .map(TestCaseResult::getCreatedAt)
    ...
    return new ScoringInput(
        total, failures, failureRate, recencyWeight, envVariance, lastFailed, lastPassed);
  }
  ...
  private double computeEnvVariance(List<TestCaseResult> history) {
    ...
    double[] rates = envStats.values().stream().mapToDouble(s -> (double) s[1] / s[0]).toArray();
    double mean = DoubleStream.of(rates).average().orElse(0.0);
    double variance =
        DoubleStream.of(rates).map(r -> (r - mean) * (r - mean)).average().orElse(0.0);
