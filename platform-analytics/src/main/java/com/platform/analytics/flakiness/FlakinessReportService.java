...
public class FlakinessReportService {
  ...
  public FlakinessReportService(FlakinessScoreRepository scoreRepo, ProjectRepository projectRepo) {
    this.scoreRepo = scoreRepo;
    this.projectRepo = projectRepo;
  }

  public List<FlakinessScore> getTopFlakyForProject(UUID projectId, int limit) {
    return scoreRepo.findTopFlakyByProject(
        projectId, PageRequest.of(0, limit > 0 ? limit : DEFAULT_LIMIT));
  }

  public List<FlakinessScore> getTopFlakyByClassification(
      UUID projectId, FlakinessScore.Classification classification, int limit) {
    return scoreRepo
        .findTopFlakyByProject(projectId, PageRequest.of(0, limit > 0 ? limit : DEFAULT_LIMIT))
        .stream()
  ...
  public List<FlakinessScore> getOrgWideTopFlaky(int limit) {
    return scoreRepo.findTopFlakyAcrossOrg(PageRequest.of(0, limit > 0 ? limit : DEFAULT_LIMIT));
  }
}
