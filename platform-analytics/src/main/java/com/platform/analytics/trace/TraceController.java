...
public class TraceController {
    ...
    public void downloadTrace(
        ...
        if (result.getTraceStorePath() == null) {
            log.warn("Trace request for resultId={} but traceStorePath is null (trace was never uploaded)", resultId);
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No trace for this result");
        }

        log.debug("Serving trace resultId={} storePath={} storageType={}", resultId, result.getTraceStorePath(), storageProps.getType());
        ...
        if (storageProps.getType() != BlobStoreProperties.Type.filesystem) {
            ...
            byte[] bytes = blobStore.fetchBytes(ref)
                    .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Trace not found in store"));
