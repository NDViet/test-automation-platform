...
/**
 * Queries the {@code test_execution_logs-*} OpenSearch index to retrieve log events by {@code
 * run_id} or {@code test_id}.
 *
 * <p>Uses Spring's {@link RestClient} over HTTP — no separate OpenSearch client library required.
 * OpenSearch's REST API is standard JSON.
 *
 * <p>On startup, applies the {@code test_execution_logs} index template if it does not already
 * exist, so the index is always created with the correct field mappings.
 */
@Service
public class LogSearchService {
  ...
  public LogSearchService(
      ...
      ObjectMapper mapper) {
    this.restClient = RestClient.builder().baseUrl("http://" + host + ":" + port).build();
    this.mapper = mapper;
  }
  ...
  void ensureIndexTemplate() {
    try {
      String existing =
          restClient.get().uri("/_index_template/" + TEMPLATE_NAME).retrieve().body(String.class);
    ...
    try {
      String templateJson =
          new ClassPathResource(TEMPLATE_PATH).getContentAsString(StandardCharsets.UTF_8);
      restClient
          .put()
      ...
      log.warn(
          "[LogSearch] Could not apply index template '{}' (non-fatal): {}",
          TEMPLATE_NAME,
          e.getMessage());
    }
  }
  ...
  /**
   * Returns up to {@code size} log entries for a given {@code run_id}, sorted oldest-first so the
  ...
  /**
   * Returns up to {@code size} log entries for a specific test case, identified by its {@code
   * test_id} (e.g. {@code login.feature#Login_with_valid_credentials}).
   *
  ...
  /**
   * Returns all distinct {@code run_id} values seen in the last {@code days} days, for the given
   * {@code teamId} and {@code projectId}. Useful for building a "recent runs" list in the portal.
   */
  ...
  /**
   * Builds a term-query JSON body, optionally filtered by log level. Results are sorted by
   * {@code @timestamp} ascending (chronological order).
   */
