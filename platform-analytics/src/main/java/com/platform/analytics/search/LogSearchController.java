 ...
 *
 * <p>All logs produced during a test JVM run are tagged with {@code run_id} (shared across the
 * entire suite) and {@code test_id} (per scenario/method). Use these endpoints to retrieve the
 * exact log stream for any run or test case.
 *
...
public class LogSearchController {
   ...
   *
   * <p>The {@code run_id} is the value of {@code RunContext.getRunId()} in the test JVM — a UUID
   * that is the same for all tests in one {@code mvn test} invocation. It is printed at the start
   * of each test run by PlatformExtension / PlatformCucumberPlugin.
   */
  @GetMapping("/runs/{runId}")
  @Operation(
      summary = "Get all logs for a test run",
      description =
          "Returns chronologically sorted log lines tagged with the given run_id. "
  ...
  /**
   * Returns log entries for a specific test case (scenario or JUnit method), identified by its
   * {@code test_id}.
   *
   * <p>The {@code test_id} format is {@code ClassName#methodName} for JUnit5 tests and {@code
   * featureFile#Scenario_name} for Cucumber scenarios.
   */
  @GetMapping("/tests/{testId}")
  @Operation(
      summary = "Get logs for a specific test case",
      description =
          "Returns log lines emitted during a single scenario or test method. "
  ...
  /**
   * Returns the distinct {@code run_id} values seen in the last N days for a given team + project.
   * Use this to build a "recent runs" picker in the portal.
   */
  @GetMapping("/runs")
  @Operation(
      summary = "List recent run IDs for a project",
      description =
          "Returns distinct run_id values from the last N days, "
              + "newest first. Useful for building a run selector in the UI.")
  public ResponseEntity<List<String>> listRecentRuns(
      @Parameter(description = "Team slug", required = true) @RequestParam String teamId,
      @Parameter(description = "Project slug", required = true) @RequestParam String projectId,
  ...
  public record LogSearchResponse(String runId, String testId, int total, List<LogEntry> logs) {}
}
