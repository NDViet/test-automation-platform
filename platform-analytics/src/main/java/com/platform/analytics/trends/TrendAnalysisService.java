...
public class TrendAnalysisService {
  ...
  public List<PassRatePoint> dailyPassRates(UUID projectId, int days) {
    ...
    Map<LocalDate, List<TestExecution>> byDay =
        executions.stream()
            .collect(
                Collectors.groupingBy(e -> e.getExecutedAt().atZone(ZoneOffset.UTC).toLocalDate()));
  ...
  /**
   * Mean Time to Recovery: average duration (in minutes) from first failure in a streak to the
   * first subsequent pass, measured across the lookback window.
   */
  public OptionalDouble meanTimeToRecoveryMinutes(UUID projectId, int days) {
    Instant from = Instant.now().minus(days, ChronoUnit.DAYS);
    List<TestExecution> executions =
        executionRepo.findByProjectAndDateRange(projectId, from, Instant.now());

    // Sort chronologically
    List<TestExecution> sorted =
        executions.stream().sorted(Comparator.comparing(TestExecution::getExecutedAt)).toList();
