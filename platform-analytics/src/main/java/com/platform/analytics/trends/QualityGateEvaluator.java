 ...
 *
 * <p>Gates are configured via application properties:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Used in CI pipelines to block deployments when quality drops below threshold.
 */
@Service
public class QualityGateEvaluator {
  ...
  /**
   * Evaluates quality gates for an incoming result. {@code projectId} is the resolved UUID of the
   * project this result belongs to.
   */
  ...
  public QualityGateResult evaluate(UnifiedTestResult result, UUID projectId) {
    ...
    double passRate = result.total() > 0 ? (double) result.passed() / result.total() : 1.0;
    if (passRate < minPassRate) {
      violations.add(
          String.format(
              "Pass rate %.1f%% is below minimum %.1f%%", passRate * 100, minPassRate * 100));
    }
    ...
    if (newFailures > maxNewFailures) {
      violations.add(
          String.format("New failure count %d exceeds maximum %d", newFailures, maxNewFailures));
    }
  ...
  private void recordPassRateGauge(UnifiedTestResult result, double passRate) {
    ...
    AtomicLong backing =
        passRateGauges.computeIfAbsent(
            key,
            k -> {
