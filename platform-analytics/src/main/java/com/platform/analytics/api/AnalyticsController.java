...
public class AnalyticsController {
  ...
  public AnalyticsController(
      FlakinessReportService flakinessService,
  ...
  public List<FlakinessDto> topFlaky(
    ...
    if (classification != null) {
      ...
      return flakinessService.getTopFlakyByClassification(projectId, cls, limit).stream()
          .map(FlakinessDto::from)
          .toList();
    }
    return flakinessService.getTopFlakyForProject(projectId, limit).stream()
        .map(FlakinessDto::from)
        .toList();
  }
  ...
  public Map<String, Object> recomputeFlakiness(@PathVariable UUID projectId) {
    ...
    Set<String> testIds =
        existing.stream().map(FlakinessScore::getTestId).collect(Collectors.toSet());

    // Also include tests seen in the last 30 days but not yet scored
    java.time.Instant since30d =
        java.time.Instant.now().minus(30, java.time.temporal.ChronoUnit.DAYS);
    List<TestCaseResult> recent = testCaseResultRepo.findByStatusSince(TestStatus.FAILED, since30d);
    recent.stream()
        .filter(
            r ->
                r.getExecution() != null
  ...
  public ResponseEntity<QualityGateResult> qualityGate(@PathVariable UUID projectId) {
    return executionRepo
        .findTopByProjectIdOrderByExecutedAtDesc(projectId)
        .map(
            exec -> {
  ...
  public OrgSummaryDto orgSummary(
      @RequestParam(required = false) String orgSlug, @RequestParam(defaultValue = "7") int days) {
    ...
    // Group by project
    Map<UUID, List<TestExecution>> byProject =
        executions.stream().collect(Collectors.groupingBy(e -> e.getProject().getId()));
    ...
    int criticalFlaky =
        (int)
            scoreRepo.findTopFlakyAcrossOrg(PageRequest.of(0, 10_000)).stream()
                .filter(
                    s ->
    ...
    List<OrgSummaryDto.ProjectSummary> summaries =
        projects.stream()
            .map(
                p -> {
                  ...
                  int flakyCount =
                      (int)
                          scoreRepo
                              ...
                              .filter(
                                  s ->
                                      s.getClassification() == FlakinessScore.Classification.FLAKY
                                          || s.getClassification()
                                              == FlakinessScore.Classification.CRITICAL_FLAKY)
                              .count();
                  String orgSlugVal =
                      p.getOrganization() != null ? p.getOrganization().getSlug() : "unknown";
                  return new OrgSummaryDto.ProjectSummary(
                      p.getSlug(), orgSlugVal, pRate, pExecs.size(), flakyCount);
                })
            .toList();

    return new OrgSummaryDto(
        projects.size(), executions.size(), overallRate, criticalFlaky, summaries);
  }
  ...
  private UnifiedTestResult buildStubResult(TestExecution exec) {
    String orgSlugVal =
        exec.getProject().getOrganization() != null
            ? exec.getProject().getOrganization().getSlug()
            : "unknown";
    return new UnifiedTestResult(
        ...
        exec.getBroken(),
        exec.getDurationMs(),
        exec.getSourceFormat(),
        List.of(),
        exec.getExecutionMode(),
        exec.getParallelism(),
        exec.getSuiteName(),
        null,
        null,
