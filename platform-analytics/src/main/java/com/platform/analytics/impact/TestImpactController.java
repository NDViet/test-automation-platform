 ...
 *
 * <p>Given a set of changed source files (from a git diff), returns the minimal subset of tests
 * that cover those changes plus metadata for CI decision-making.
 *
...
public class TestImpactController {
  ...
  public TestImpactController(
      TestImpactService impactService,
  ...
  @Operation(
      ...
      description =
          "Returns the recommended tests to run given a list of changed source files. "
  ...
  public ResponseEntity<TestImpactResult> analyse(
      ...
      @Parameter(
              description =
                  "Comma-separated or repeated list of changed file paths "
                      + "(e.g. src/main/java/com/example/Foo.java)")
          @RequestParam(required = false)
          List<String> changedFiles,
      @Parameter(
              description =
                  "Comma-separated or repeated list of fully qualified class names "
      ...
      @Parameter(description = "Who triggered the query: api | ci | portal")
          @RequestParam(required = false, defaultValue = "api")
          String triggeredBy) {
    long totalTests = coverageRepo.countMappedTestsByProject(projectId);

    TestImpactResult result =
        impactService.analyse(
            projectId, changedFiles, changedClasses, (int) totalTests, branch, triggeredBy);
