...
/**
 * Analyses a set of changed source files and returns the minimal subset of tests that cover those
 * changes — enabling selective ("smart") test execution in CI.
 *
 * <h2>Algorithm</h2>
 *
 * <ol>
 *   <li>Convert each changed file path to a fully qualified class name (e.g. {@code
 *       src/main/java/com/example/Foo.java} → {@code com.example.Foo}).
 *   <li>Query coverage mappings for matching class names.
 *   <li>Collect distinct test IDs that cover any matched class.
 *   <li>Assess risk: are all changed classes covered? What % is uncovered?
 * </ol>
 *
 * <p>Every call is recorded as a {@link TiaEvent} row for dashboard trending.
 */
@Service
public class TestImpactService {
  ...
  public TestImpactService(
    ...
    this.queriesTotal =
        Counter.builder("tia.queries.total")
            .description("Total number of TIA analyse() calls")
            .register(meterRegistry);
    this.queryTimer =
        Timer.builder("tia.query.duration")
            .description("Duration of TIA analyse() calls")
            .register(meterRegistry);
  }

  @Transactional
  public TestImpactResult analyse(
      UUID projectId, List<String> changedFiles, List<String> changedClasses, int totalTestCount) {
  ...
  private TestImpactResult doAnalyse(
      UUID projectId,
      List<String> changedFiles,
      List<String> changedClasses,
      int totalTestCount,
    ...
    if (changedClasses != null) {
      changedClasses.stream().filter(c -> c != null && !c.isBlank()).forEach(classesToCheck::add);
    }
    ...
    for (var wm : wildcardMappings) {
      ...
      for (String cls : classesToCheck) {
        if (cls.startsWith(prefix)) {
          classToTests.computeIfAbsent(cls, k -> new LinkedHashSet<>()).add(wm.getTestCaseId());
        }
      }
    }

    Set<String> recommendedTests =
        classToTests.values().stream()
    ...
    double coveredRatio =
        classesToCheck.isEmpty() ? 1.0 : (double) coveredClasses.size() / classesToCheck.size();

    RiskLevel risk = computeRisk(coveredRatio, uncoveredClasses.size());

    int selected = recommendedTests.size();
    double reduction =
        totalTestCount > 0 ? (1.0 - (double) selected / totalTestCount) * 100.0 : 0.0;

    log.info(
        "TIA projectId={} changedClasses={} recommended={}/{} risk={}",
        projectId,
        classesToCheck.size(),
        selected,
        totalTestCount,
  ...
  /**
   * Converts a source file path to a fully qualified class name. Handles standard Java/Kotlin/Scala
   * conventions. Returns null if the file is not a recognisable source file.
   *
   * <p>Examples: src/main/java/com/example/PaymentService.java → com.example.PaymentService
   * src/main/kotlin/com/example/CartService.kt → com.example.CartService
   * src/app/services/payment.service.ts → payment.service (module path) lib/checkout/checkout.js →
   * lib/checkout/checkout (module path)
   */
