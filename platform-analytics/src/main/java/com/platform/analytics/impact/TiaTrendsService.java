 ...
 *
 * <p>All queries are read-only projections over {@code tia_events} and {@code
...
public class TiaTrendsService {
  ...
  public TiaTrendsService(
      TiaEventRepository tiaEventRepo, TestCoverageMappingRepository coverageRepo) {
  ...
  public TiaSummary orgSummary(int days) {
    ...
    return new TiaSummary(
        totalRequests, avgReduction, totalMappedTests, totalMappedClasses, (long) breadth.size());
  }
  ...
  public List<TiaEventDto> recentEvents(UUID projectId, int limit) {
    return tiaEventRepo.findByProjectIdOrderByQueriedAtDesc(projectId).stream()
        .limit(limit)
        .map(
            e ->
                new TiaEventDto(
  ...
  public record DailyCoverageBreadthPoint(
      LocalDate day, long distinctClasses, long distinctTests) {}
