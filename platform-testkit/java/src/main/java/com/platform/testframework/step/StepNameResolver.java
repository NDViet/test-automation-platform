 ...
 *
 * <p>Two resolution strategies:
 *
 * <ol>
 *   <li><b>Method name derivation</b> — when the annotation value is empty, the method name is
 *       split on camelCase boundaries and title-cased: {@code fillShippingAddress} → {@code "Fill
 *       Shipping Address"}
 *   <li><b>Template interpolation</b> — when the annotation value contains {@code {paramName}}
 *       placeholders, each placeholder is replaced with the actual argument value at runtime:
 *       {@code "Submit payment with {cardType}"} + arg {@code "Visa"} → {@code "Submit payment with
 *       Visa"}
 * </ol>
 */
public final class StepNameResolver {
  ...
  public static String resolve(
      String template, String methodName, String[] paramNames, Object[] args) {
   ...
   *
   * <ul>
   *   <li>{@code fillShippingAddress} → {@code "Fill Shipping Address"}
   *   <li>{@code submitPaymentWithVisa} → {@code "Submit Payment With Visa"}
   *   <li>{@code clickLoginButton} → {@code "Click Login Button"}
   *   <li>{@code openURL} → {@code "Open Url"}
   * </ul>
   */
  static String camelToTitle(String methodName) {
    ...
    String[] words = methodName.split("(?<=[a-z0-9])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])");
   ...
   *
   * <p>Unmatched placeholders are left as-is. Null arguments are rendered as {@code "null"}.
   *
