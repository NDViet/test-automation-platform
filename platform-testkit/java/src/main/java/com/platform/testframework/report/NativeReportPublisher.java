...
/**
 * Converts accumulated {@link TestContext.Snapshot} instances into a {@link UnifiedTestResult} and
 * publishes to the platform via {@link PlatformReporter#publishNative}.
 *
 * <p>Never throws — publication failures are non-fatal.
 */
public final class NativeReportPublisher {
  ...
  /**
   * Publishes a single test result immediately after the test completes. Called by {@link
   * com.platform.testframework.extension.PlatformExtension} in {@code afterEach}.
   */
  public void publish(
      TestContext.Snapshot snap,
      TestStatus status,
    ...
    List<TestStepDto> stepDtos = snap.rootSteps().stream().map(TestStep::toDto).toList();

    TestCaseResultDto tc =
  ...
  private static String resolveEnv(String key1, String key2, String key3, String fallback) {
    ...
    if (key2 != null) {
      v = System.getenv(key2);
      if (v != null && !v.isBlank()) return v;
    }
    if (key3 != null) {
      v = System.getenv(key3);
      if (v != null && !v.isBlank()) return v;
    }
    return fallback;
  }
