 ...
 *
 * <p>Safe for parallel test execution — each thread (test worker) gets its own context. Always call
 * {@link #clear()} in the {@code afterEach} lifecycle hook.
 */
  ...
  public static TestContext require() {
    ...
    if (ctx == null) {
      throw new IllegalStateException(
          "No active TestContext — ensure @ExtendWith(PlatformExtension.class) is on your test"
              + " class");
    }
    return ctx;
  }
