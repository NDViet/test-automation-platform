 ...
 * com.platform.testframework.step.TestStep}s — giving the platform per-step timing and status for
 * every BDD scenario without any test code changes.
 *
...
public class PlatformCucumberPlugin implements ConcurrentEventListener {
  ...
  private void onTestCaseStarted(TestCaseStarted event) {
    ...
    String className =
        featureName.contains("/")
            ? featureName.substring(featureName.lastIndexOf('/') + 1)
            : featureName;
    ...
    List<String> tags =
        scenario.getTags().stream().map(t -> t.startsWith("@") ? t.substring(1) : t).toList();
  ...
  private void onTestCaseFinished(TestCaseFinished event) {
    ...
    log.info(
        "[Platform/Cucumber] Scenario finished — '{}' status={} duration={}ms",
        snap.displayName(),
        status,
        durationMs);
