 ...
 *
 * <p>Each test thread registers its own provider during setup so that the platform listener can
 * capture browser state on failure without holding a reference to any Selenium/Playwright type.
 *
...
public final class DiagnosticsRegistry {
  ...
  public static void register(
      PageSourceSupplier pageSourceSupplier, ScreenshotSupplier screenshotSupplier) {
    CURRENT.set(
        new DiagnosticsProvider() {
          @Override
          public String capturePageSource() {
            try {
              return pageSourceSupplier.get();
            } catch (Exception e) {
              return null;
            }
          }
