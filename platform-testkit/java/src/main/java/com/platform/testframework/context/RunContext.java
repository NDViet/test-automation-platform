...
/**
 * Holds stable, JVM-scoped context for the duration of one {@code mvn test} / {@code gradle test}
 * invocation.
 *
 * <p>All test cases in the same execution share {@code run_id}, {@code hostname}, {@code
 * ci_provider}, and {@code build_id}. These are set as MDC fields by {@link
 * com.platform.testframework.extension.PlatformExtension} and {@link
 * com.platform.testframework.cucumber.PlatformCucumberPlugin} before each test so every log line
 * produced during a run is tagged and queryable in OpenSearch:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Override the generated run ID by setting the {@code PLATFORM_RUN_ID} environment variable or
 * the {@code platform.runId} system property.
 */
