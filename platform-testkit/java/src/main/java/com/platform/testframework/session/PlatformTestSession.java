 ...
 *
 * <p>Use this when you are not using JUnit5 {@code @ExtendWith}, TestNG {@code @Listeners}, or
 * Cucumber plugins — for example, in a bespoke keyword-driven harness, a RestAssured API test suite
 * with its own runner, or a custom Playwright wrapper that doesn't use JUnit5.
 *
 * <h3>Usage:</h3>
 *
 * <pre>{@code
...
public final class PlatformTestSession implements AutoCloseable {
  ...
  /**
   * Start a session for {@code testId} (format: {@code ClassName#methodName}). Tags and display
   * name are derived from the testId.
   */
   ...
   *
   * <p>The step is pushed to the platform context, executed, then popped. If the action throws, the
   * step is marked as failed and the exception is re-thrown.
   */
  ...
  /**
   * Run {@code action} inside a named step, checked exceptions wrapped in {@link RuntimeException}.
   */
  ...
  public void failure(Throwable t) {
    ...
    this.resultStatus = (t instanceof AssertionError) ? TestStatus.FAILED : TestStatus.BROKEN;
  }
   ...
   *
   * <p>If neither {@link #success()} nor {@link #failure(Throwable)} was called before {@code
   * close()}, the session is treated as broken.
   *
  ...
  public void close() {
    ...
    if (resultThrowable != null) {
      ...
      FailureHint hint = FailureClassifier.classify(resultThrowable, lastStep, snap.capturedLog());
    ...
    TestContext.Snapshot snap = ctx.snapshot();
    log.info(
        "[Platform/Session] Test finished — {} status={} duration={}ms",
        snap.testId(),
        resultStatus,
        durationMs);
