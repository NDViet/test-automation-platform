 ...
 *
 * <p>This class uses reflection to write step events to the Allure lifecycle when {@code
 * allure-java-commons} is on the classpath. If Allure is absent, every method is a no-op — no
 * exception is thrown and no dependency is required.
 *
 * <p>Teams that want local Allure HTML reports simply add the Allure dependency to their project.
 * The platform testkit detects it automatically and writes to both the platform context and the
 * Allure lifecycle from the same {@code @Step} call.
 *
...
public final class AllureStepBridge {
   ...
   * @return UUID for this step (pass to {@link #passStep} / {@link #failStep} / {@link
   *     #brokenStep}), or {@code null} if Allure is not present
   */
  ...
  public static void passStep(String uuid) {
    ...
    try {
      Object lifecycle = getLifecycle.invoke(null);
      updateStep.invoke(
          lifecycle, uuid, (Consumer<Object>) r -> setStatusSilently(r, statusPassed));
  ...
  public static void failStep(String uuid) {
    ...
    try {
      Object lifecycle = getLifecycle.invoke(null);
      updateStep.invoke(
          lifecycle, uuid, (Consumer<Object>) r -> setStatusSilently(r, statusFailed));
      stopStep.invoke(lifecycle, uuid);
    } catch (Exception e) {
  ...
  public static void brokenStep(String uuid) {
    ...
    try {
      Object lifecycle = getLifecycle.invoke(null);
      updateStep.invoke(
          lifecycle, uuid, (Consumer<Object>) r -> setStatusSilently(r, statusBroken));
      stopStep.invoke(lifecycle, uuid);
    } catch (Exception e) {
