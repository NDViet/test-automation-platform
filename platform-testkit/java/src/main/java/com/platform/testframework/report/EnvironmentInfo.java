 ...
 *
 * <p>Attached to every test result so the platform knows exactly which environment a failure came
 * from — critical for environment-specific vs application bugs. Also detects execution mode
 * (parallel/sequential) and CI provider.
 */
public final class EnvironmentInfo {
  ...
  /**
   * Returns a map of standard environment properties. Teams can augment this with their own keys
   * (APP_URL, BROWSER, DEVICE, etc.)
   */
  public static Map<String, String> collect() {
    ...
    if (testEnv != null && !testEnv.isBlank()) env.put("test.env", testEnv);
    String appUrl = System.getenv("APP_URL");
    if (appUrl != null && !appUrl.isBlank()) env.put("app.url", appUrl);

    return env;
  }

  /**
   * Returns one of: github-actions, gitlab-ci, jenkins, circleci, azure-devops, bitbucket,
   * teamcity, travis-ci, ci (generic), local
   */
  ...
  public static String detectExecutionMode() {
    ...
    int forkCount =
        parseForkCount(
            System.getProperty("surefire.forkCount", System.getProperty("forkCount", "0")));
  ...
  public static int detectParallelism() {
    ...
    String dynamicFactor =
        System.getProperty("junit.jupiter.execution.parallel.config.dynamic.factor");
  ...
  /**
   * Parses Maven Surefire forkCount — supports plain integers and the {@code 2C} (CPU multiplier)
   * format.
   */
