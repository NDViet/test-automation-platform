...
/**
 * Wrapper around AssertJ {@link SoftAssertions} that also logs each assertion check to the active
 * {@link com.platform.testframework.context.TestContext}.
 *
 * <p>Collects all assertion failures and reports them together at the end — so you see
 * <em>every</em> failure in one test run, not just the first.
 *
...
public final class SoftAssert {
  ...
  /**
   * Returns an AssertJ {@code AbstractAssert} chain for the given object. All failures are deferred
   * until {@link #assertAll()} is called.
   */
  ...
  /**
   * Throws an {@link AssertionError} listing all failures collected so far. No-op if all assertions
   * passed.
   */
