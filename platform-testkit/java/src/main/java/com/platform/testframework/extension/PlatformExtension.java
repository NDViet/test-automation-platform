 ...
 *
 * <ul>
 *   <li><b>BeforeEach</b> — initialises {@link TestContext}, sets MDC fields (testId, testClass,
 *       traceId), opens an OTel span.
 *   <li><b>AfterEach</b> — captures outcome, closes the span, publishes to the platform via {@link
 *       NativeReportPublisher}.
 * </ul>
 *
 * <p>Register via {@code @ExtendWith(PlatformExtension.class)} or use {@link
 * com.platform.testframework.base.PlatformBaseTest} which pre-wires it.
 */
public class PlatformExtension implements BeforeEachCallback, AfterEachCallback, TestWatcher {
  ...
  public void afterEach(ExtensionContext ctx) {
    long startMs =
        ctx.getStore(NS).getOrDefault(KEY_START, Long.class, Instant.now().toEpochMilli());
    ...
    log.info(
        "[Platform] Test finished — {} status={} duration={}ms traceId={}",
        snap.testId(),
        status,
        durationMs,
        snap.traceId());
    ...
    if (testCtx != null) {
      ...
      try {
        ...
        new NativeReportPublisher(config)
            .publish(snap, TestStatus.SKIPPED, reason.orElse(null), null, 0);
      } catch (Exception e) {
  ...
  private String extractSelector(String msg) {
    ...
    for (String prefix :
        List.of("By.cssSelector:", "By.xpath:", "By.id:", "By.name:", "By.className:")) {
