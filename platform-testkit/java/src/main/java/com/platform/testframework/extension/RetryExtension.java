 ...
 * com.platform.testframework.context.TestContext} so the platform tracks flaky behaviour. Only the
 * final attempt outcome is published as the test result.
 *
 * <p>Register alongside {@link PlatformExtension}:
 *
 * <pre>{@code
...
public class RetryExtension implements TestExecutionExceptionHandler {
  ...
  public void handleTestExecutionException(ExtensionContext ctx, Throwable throwable)
    ...
    if (attempt < maxAttempts) {
      log.warn(
          "[Retry] Test failed on attempt {}/{} — retrying: {}",
          attempt,
          maxAttempts,
          ctx.getDisplayName());
      ...
      log.error(
          "[Retry] Test failed after {}/{} attempts: {}",
          attempt,
          maxAttempts,
          ctx.getDisplayName());
      throw throwable;
