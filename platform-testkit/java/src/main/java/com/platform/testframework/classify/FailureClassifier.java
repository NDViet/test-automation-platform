...
/**
 * Rule-based failure classifier that analyzes a {@link Throwable} and test context to produce a
 * {@link FailureHint} before the result is sent to the platform.
 *
 * <h3>Design principles:</h3>
 *
 * <ul>
 *   <li><b>No hard dependencies</b> — matches Selenium/Playwright/RestAssured exception types by
 *       class name string so this compiles without those libraries on the classpath.
 *   <li><b>Layered rules</b> — most-specific exception types win; ambiguous timeouts are
 *       disambiguated by message content and step name.
 *   <li><b>Transparent</b> — confidence score reflects certainty; low-confidence results still
 *       surface to Claude AI for deeper analysis.
 * </ul>
 *
 * <h3>Usage:</h3>
 *
...
public final class FailureClassifier {
  ...
  public static FailureHint classify(Throwable throwable, String currentStep, String capturedLog) {
    ...
    // ── 1. Bad locator ───────────────────────────────────────────────
    if (is(exName, "NoSuchElementException", "ElementNotFoundException", "ElementNotFoundError")) {
      return FailureHint.of(
          FailureCategory.BAD_LOCATOR,
          0.95,
          "Element not found. The selector '"
              + extractSelector(exMsg)
    ...
    if (contains(
      ...
      return FailureHint.of(
          FailureCategory.FLAKY_TIMING,
          0.88,
          "Element is obscured: " + firstLine(exMsg) + ". Wait for animations/overlays to finish.");
    }
  ...
  private static FailureHint classifyPlaywrightTimeout(String msg, String step) {
    ...
    if (contains(msg, "locator", "waiting for locator", "no element found", "not found in page")) {
      return FailureHint.of(
          FailureCategory.BAD_LOCATOR,
          0.80,
  ...
  private static FailureHint classifySeleniumTimeout(String msg, String step) {
    if (contains(msg, "element", "locator", "find")) {
      return FailureHint.of(
          FailureCategory.BAD_LOCATOR,
          0.72,
          "Selenium timed out waiting for an element: "
              + firstLine(msg)
    ...
    return FailureHint.of(
        FailureCategory.TIMEOUT, 0.65, "Selenium TimeoutException: " + firstLine(msg));
  }
  ...
  private static FailureHint classifyWebDriverException(String msg) {
    if (contains(
        ...
        "driver executable does not exist")) {
      return FailureHint.of(
          FailureCategory.INFRASTRUCTURE, 0.95, "Browser/driver setup failure: " + firstLine(msg));
    }
    if (contains(msg, "chrome not reachable", "unable to connect", "connection refused")) {
      return FailureHint.of(
          FailureCategory.INFRASTRUCTURE,
          0.90,
          "Browser process is not reachable: "
              + firstLine(msg)
  ...
  private static FailureHint classifyAssertionError(String msg, String step) {
    ...
    if (contains(step, "verify", "assert", "check", "validate", "confirm")) {
      return FailureHint.of(
          FailureCategory.APPLICATION_BUG,
          0.80,
          "Assertion failed during verification step '" + step + "': " + firstLine(msg));
    }
    return FailureHint.of(
        FailureCategory.APPLICATION_BUG,
        0.72,
  ...
  /**
   * Heuristic: is the exception likely thrown from test/framework code rather than the application
   * or browser library?
   */
