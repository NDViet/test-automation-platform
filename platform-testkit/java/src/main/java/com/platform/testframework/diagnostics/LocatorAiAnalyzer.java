...
/**
 * Captures DOM snapshot and failed selector from the browser and stores them in the {@link
 * TestContext} environment so the platform backend can perform AI analysis server-side.
 *
 * <p>No AI API calls are made from the test client. The diagnostic data travels with the test
 * result payload to the platform ingestion service and is picked up by platform-ai via Kafka for
 * classification.
 *
 * <p>Keys written to {@link TestContext#putEnvironment}:
 *
 * <ul>
 *   <li>{@code platform.diagnostic.selector} — the CSS/XPath selector that failed
 *   <li>{@code platform.diagnostic.dom} — page HTML truncated to {@value #MAX_DOM_CHARS} chars
 * </ul>
 */
public final class LocatorAiAnalyzer {
  ...
  /**
   * Captures browser state via the registered {@link DiagnosticsProvider} and stores it in the
   * given {@link TestContext} for forwarding to the platform.
   *
  ...
  public static void attach(String selector, DiagnosticsProvider provider, TestContext ctx) {
    ...
    try {
      ...
      if (dom != null && !dom.isBlank()) {
        ...
        log.debug(
            "[Diagnostics] DOM snapshot attached ({} chars) — platform will analyse",
            truncated.length());
      }
    } catch (Exception e) {
