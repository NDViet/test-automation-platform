 ...
 *
 * <p>Every call writes to the SLF4J logger (with MDC context for structured log aggregation)
 * <em>and</em> appends to the active {@link TestContext} so the output is attached to the platform
 * result for AI-assisted debugging.
 *
 * <h3>Usage:</h3>
 *
 * <pre>{@code
...
public final class TestLogger {
  ...
  /**
   * Opens a named step. Must be balanced with {@link #endStep()}. Steps can be nested — use for
   * keyword-driven or page-object style testing.
   *
  ...
  /**
   * Attaches binary data (e.g. screenshots, HAR files) to the current test. The bytes are saved to
   * a temp file and the path is recorded in the context.
   */
    ...
    try {
      String suffix =
          mimeType.contains("png")
              ? ".png"
              : mimeType.contains("json") ? ".json" : mimeType.contains("html") ? ".html" : ".bin";
