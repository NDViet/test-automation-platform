 ...
 *
 * <p>For Selenium/Playwright exception types (not on classpath), inner static exception classes are
 * used — their names contain the target substring so the classifier's {@code
 * getClass().getName().contains(...)} logic fires.
 */
class FailureClassifierTest {
  ...
  void unknownHostExceptionIsInfrastructure() {
    FailureHint hint =
        FailureClassifier.classify(new UnknownHostException("api.example.internal"), null, null);
    assertThat(hint.category()).isEqualTo(FailureCategory.INFRASTRUCTURE);
    assertThat(hint.confidence()).isGreaterThanOrEqualTo(0.90);
  ...
  void httpStatusAssertionHasHigherConfidence() {
    FailureHint hint =
        FailureClassifier.classify(new AssertionError("expected 200 but got 500"), null, null);
    assertThat(hint.category()).isEqualTo(FailureCategory.APPLICATION_BUG);
  ...
  void noSuchElementExceptionIsBadLocator() {
    ...
    FailureHint hint =
        FailureClassifier.classify(
            new NoSuchElementException("Unable to locate element: By.cssSelector: .login-btn"),
            null,
            null);
  ...
  void unableToLocateElementMessageIsBadLocator() {
    FailureHint hint =
        FailureClassifier.classify(
            new RuntimeException("unable to locate element: #submit-button"), null, null);
    assertThat(hint.category()).isEqualTo(FailureCategory.BAD_LOCATOR);
  }
    ...
    FailureHint hint =
        FailureClassifier.classify(
            new RuntimeException(
                "Element is not clickable at point (123, 456) "
  ...
  void elementNotInteractableIsFlaky() {
    FailureHint hint =
        FailureClassifier.classify(
            new ElementNotInteractableException("element not interactable"), null, null);
    assertThat(hint.category()).isEqualTo(FailureCategory.FLAKY_TIMING);
    assertThat(hint.confidence()).isGreaterThanOrEqualTo(0.80);
  }
  ...
  void webDriverStaleMessageIsFlaky() {
    FailureHint hint =
        FailureClassifier.classify(
            new WebDriverException("stale element in WebDriver response"), null, null);
    assertThat(hint.category()).isEqualTo(FailureCategory.FLAKY_TIMING);
  }
  ...
  void genericWebDriverIsInfrastructure() {
    FailureHint hint =
        FailureClassifier.classify(
            new WebDriverException("unknown error: cannot determine loading status"), null, null);
    assertThat(hint.category()).isEqualTo(FailureCategory.INFRASTRUCTURE);
  }
  ...
  void npeInAppCodeIsApplicationBug() {
    ...
    StackTraceElement[] frames = {
      ...
      new StackTraceElement(
          "com.app.controller.UserController", "getUser", "UserController.java", 20)
    };
    npe.setStackTrace(frames);
    FailureHint hint = FailureClassifier.classify(npe, null, null);
    assertThat(hint.category()).isEqualTo(FailureCategory.APPLICATION_BUG);
