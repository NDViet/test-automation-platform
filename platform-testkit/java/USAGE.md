# platform-testkit-java — Usage Guide

`platform-testkit-java` is the full observability layer for new test suites. On top of
`platform-adapter-java`'s auto-publish, it adds:
...
> No need to also add `platform-adapter-java` — it is a transitive dependency.

Configure `src/test/resources/platform.properties` exactly as described in
[platform-adapter-java/USAGE.md](../platform-adapter-java/USAGE.md#2-configure-the-sdk).
...
```
platform-testkit-java
  ├── platform-adapter-java          (HTTP publisher, PlatformConfig, PlatformReporter)
