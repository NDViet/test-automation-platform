...
class RbacServiceTest {
  ...
  void orgAdmin_canManageAnyTeamAndOrg() {
    when(repo.findByUserId("admin"))
        .thenReturn(List.of(new TeamMember("admin", null, Role.ORG_ADMIN, "system")));
  ...
  void teamAdmin_managesOwnTeamOnly() {
    when(repo.findByUserId("lead"))
        .thenReturn(List.of(new TeamMember("lead", teamA, Role.TEAM_ADMIN, "admin")));
    when(repo.findByUserIdAndTeamId("lead", teamA))
        .thenReturn(List.of(new TeamMember("lead", teamA, Role.TEAM_ADMIN, "admin")));
  ...
  void teamMember_canWriteButNotManage() {
    when(repo.findByUserId("dev"))
        .thenReturn(List.of(new TeamMember("dev", teamA, Role.TEAM_MEMBER, "lead")));
    when(repo.findByUserIdAndTeamId("dev", teamA))
        .thenReturn(List.of(new TeamMember("dev", teamA, Role.TEAM_MEMBER, "lead")));
