...
class CredentialResolverTest {
  ...
  private IntegrationCredential cred(
      Scope scope,
      UUID scopeId,
      String baseUrl,
    ...
    if (secret != null && !secret.isEmpty()) {
      try {
        ciphertext = cipher.encrypt(new ObjectMapper().writeValueAsString(secret));
      } catch (Exception e) {
        ...
        .thenReturn(
            Optional.of(
                cred(
                    ...
                    orgId,
                    "https://dev.azure.com/contoso",
                    Map.of("organization", "contoso", "area_path", "Default"),
                    Map.of("pat", "ORG_PAT"))));
        ...
        .thenReturn(
            Optional.of(
                cred(
                    ...
                    projectId,
                    null,
                    Map.of("area_path", "Product House\\Checkout"),
                    Map.of())));
  ...
  void teamOverridesProjectAndOrg() {
    ...
    when(credRepo.findByScopeAndScopeIdAndIntegrationType(Scope.PROJECT.name(), projectId, TYPE))
        .thenReturn(
            Optional.of(cred(Scope.PROJECT, projectId, "u", Map.of(), Map.of("pat", "PROJECT"))));
