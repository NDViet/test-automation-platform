...
class PropertyMatrixServiceTest {
    ...
    void full_isCartesianProduct() {
        var result = service.expand(axes(
                "browser", List.of("Chrome", "Firefox"),
                "os", List.of("Linux", "Windows", "Mac")), MatrixType.FULL);
    ...
    private Set<String> uncoveredPairs(LinkedHashMap<String, List<String>> axes,
                                       List<LinkedHashMap<String, String>> combos) {
        ...
        for (var combo : combos) {
            for (int i = 0; i < names.size(); i++) {
                for (int j = i + 1; j < names.size(); j++) {
                    pairs.remove(i + "=" + combo.get(names.get(i))
                            + "|" + j + "=" + combo.get(names.get(j)));
                }
            }
        }
        return pairs;
