...
/**
 * Resolves the {@link MappingRules} used by the Mapping Suggester (ingestion) and the
 * sync-time {@link MappingProfileApplier} (agent), deep by scope:
 * <b>PROJECT override → ORG override → built-in default</b>.
 *
 * <p>The built-in default ships as the classpath resource {@code mapping-rules.json}
 * and can be replaced at deploy time via {@code platform.mapping.rules-path}. ORG and
 * PROJECT overrides are persisted ({@link MappingRuleset}) and edited via the portal;
 * deleting an override "resets to default" (falls back to the parent scope).</p>
 */
@Component
public class MappingRulesProvider {
    ...
    public MappingRulesProvider(ObjectMapper mapper,
        ...
        log.info("[MappingRules] Built-in default loaded: {} lane rule(s), {} field heuristic(s){}",
                size(loaded.laneRules()), size(loaded.fieldHeuristics()),
                (overridePath != null && !overridePath.isBlank()) ? " (file override: " + overridePath + ")" : "");
    }
    ...
    public MappingRules effectiveForProject(UUID projectId) {
        ...
        UUID orgId = projectRepo.findById(projectId)
                .map(Project::getOrganization).map(o -> o.getId()).orElse(null);
        if (orgId != null) {
    ...
    private MappingRules stored(MappingRuleset.Scope scope, UUID scopeId) {
        return rulesetRepo.findByScopeAndScopeId(scope.name(), scopeId)
                .map(rs -> {
                    try {
                        return parse(rs.getRulesJson());
                    } catch (Exception e) {
                        log.error("[MappingRules] Stored {} {} override is invalid — falling back: {}",
    ...
    public static MappingRules loadDefault(ObjectMapper mapper) {
        try (InputStream in = new ClassPathResource(DEFAULT_RESOURCE).getInputStream()) {
            ...
            throw new IllegalStateException("Failed to load default mapping rules (" + DEFAULT_RESOURCE + ")", e);
        }
    }

    private static MappingRules loadFileOverride(ObjectMapper mapper, String path, MappingRules fallback) {
        try {
            Path p = Path.of(path);
            if (!Files.exists(p)) {
            ...
            log.error("[MappingRules] Failed to read override {} — using bundled default: {}", path, e.getMessage());
            return fallback;
        }
    }
