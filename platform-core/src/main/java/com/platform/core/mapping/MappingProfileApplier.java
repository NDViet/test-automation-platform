...
/**
 * Applies a resolved {@link MappingRules} profile to an upstream work item at sync time, producing
 * the canonical values the platform persists (status, priority). Pure / deterministic. Status maps
 * via the configured state-name → category → canonical chain, which is drift-resilient (state
 * renames within a category don't change the result).
 *
 * <p>Unmapped/custom fields are intentionally NOT collapsed here — the sync stores the full raw
 * upstream payload, so nothing is lost when the schema drifts.
 */
@Component
public class MappingProfileApplier {

  /**
   * A canonical value plus whether the profile could resolve it (so callers can avoid
   * null-overwrite).
   */
  public record Resolved(String value, boolean resolved) {
    public static Resolved unresolved() {
      return new Resolved(null, false);
    }

    public static Resolved of(String v) {
      return new Resolved(v, v != null);
    }
  }

  /**
   * Canonical status for an upstream state name (e.g. ADO {@code System.State}). Chain: blocked
   * override → stateName→category → category→canonical. Unresolved if the state is unknown, so the
   * caller keeps the last-known status (no clobber).
   */
  public Resolved status(MappingRules rules, String stateName) {
    ...
    if (blockedName != null
        && blockedName.equalsIgnoreCase(s)
        && rules.blockedStateOverride() != null) {
      return Resolved.of(rules.blockedStateOverride());
    }
    String category = lookupIgnoreCase(rules.stateCategoryByName(), s);
    if (category == null)
      category = rules.defaultStateCategory(); // exclude strategy: unknown → in-progress
  ...
  /**
   * Canonical priority from the configured priority value-rule applied to the upstream field value
   * (e.g. {@code Microsoft.VSTS.Common.Priority} = "2" → "P2").
   */
  public Resolved priority(MappingRules rules, Map<String, String> upstreamFields) {
    if (rules == null || rules.priority() == null || upstreamFields == null)
  ...
  /**
   * Canonical issue type for an upstream work-item-type name, derived from the profile's lane rules
   * (e.g. "Acme Bug" → DEFECT, "Acme Team Epic" → EPIC, "Acme Capability" → CAPABILITY). Returns
   * null for non-tracked types (caller falls back to its own normalization). DEFECT lane →
   * "DEFECT"; requirement lanes → their configured issueType.
   */
