...
public interface PlatformRequirementRepository extends JpaRepository<PlatformRequirement, UUID> {
  ...
  List<PlatformRequirement> findByProjectIdAndStatusOrderByUpdatedAtDesc(
      UUID projectId, String status);

  List<PlatformRequirement> findByProjectIdAndIssueTypeOrderByUpdatedAtDesc(
      UUID projectId, String issueType);

  List<PlatformRequirement> findByProjectIdAndIssueTypeIn(
      UUID projectId, java.util.Collection<String> issueTypes);

  @Query(
      "SELECT r FROM PlatformRequirement r WHERE r.projectId = :pid "
          + "AND (LOWER(r.title) LIKE LOWER(CONCAT('%', :q, '%')) "
          + "OR LOWER(r.externalId) LIKE LOWER(CONCAT('%', :q, '%'))) "
          + "ORDER BY r.updatedAt DESC")
  List<PlatformRequirement> searchByProjectId(
      @Param("pid") UUID projectId, @Param("q") String query);

  /**
   * Paged, combinable filter+search (each filter optional via "" sentinel — empty means "no
   * filter"). Empty strings are used instead of nulls so Postgres types the bind parameters as
   * varchar (a null bind makes LOWER() resolve to lower(bytea)).
   */
  @Query(
      "SELECT r FROM PlatformRequirement r WHERE r.projectId = :pid "
          ...
          + "OR LOWER(r.externalId) LIKE LOWER(CONCAT('%', :q, '%')))")
  Page<PlatformRequirement> searchPage(
   ...
   * Iteration exact) plus status/issueType/search. Each filter is optional via "" sentinel. Native
   * so {@code starts_with} handles backslash area paths without LIKE-escape issues.
   */
  @Query(
      nativeQuery = true,
      value =
          ...
          """)
  List<PlatformRequirement> filterList(
      @Param("pid") UUID projectId,
      @Param("status") String status,
      @Param("issueType") String issueType,
  ...
  @Query(
      ...
      countQuery =
          ...
          SELECT count(*) FROM platform_requirements r WHERE r.project_id = :pid
            ...
            AND (:q = '' OR LOWER(r.title) LIKE LOWER('%'||:q||'%') OR LOWER(r.external_id) LIKE LOWER('%'||:q||'%'))
          """)
  Page<PlatformRequirement> filterPage(
      @Param("pid") UUID projectId,
      @Param("status") String status,
      @Param("issueType") String issueType,
      @Param("area") String area,
  ...
  /**
   * Upsert a requirement from an external source. Only updates title/description/syncedAt when they
   * actually changed (version check via title+desc hash). Uses the unique partial index on
