...
public interface FailureAnalysisRepository extends JpaRepository<FailureAnalysis, UUID> {
  ...
  List<FailureAnalysis> findByProjectIdAndTestIdOrderByAnalysedAtDesc(
      UUID projectId, String testId);

  @Query(
      "SELECT f FROM FailureAnalysis f WHERE f.projectId = :projectId "
  ...
  List<FailureAnalysis> findByProjectIdAndCategory(
      @Param("projectId") UUID projectId, @Param("category") String category, Pageable pageable);
