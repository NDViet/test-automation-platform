...
public interface TestExecutionRepository extends JpaRepository<TestExecution, UUID> {
  ...
  @Query(
      "SELECT e FROM TestExecution e WHERE e.project.id = :projectId "
  ...
  @Query(
      "SELECT AVG(CAST(e.passed AS double) / NULLIF(e.totalTests, 0)) "
  ...
  @Query(
      "SELECT e FROM TestExecution e WHERE e.project.id IN :projectIds "
          + "AND e.executedAt >= :since ORDER BY e.executedAt DESC")
  List<TestExecution> findByProjectIdsAndSince(
      @Param("projectIds") Set<UUID> projectIds, @Param("since") Instant since);

  /**
   * Fetch all executions in a given lifecycle status — used to detect zombie RUNNING executions.
   */
  @Query(
      "SELECT e FROM TestExecution e JOIN FETCH e.project p JOIN FETCH p.organization WHERE"
          + " e.status = :status")
