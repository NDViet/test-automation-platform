...
public interface AlertHistoryRepository extends JpaRepository<AlertHistory, UUID> {
  ...
  @Query(
      "SELECT a FROM AlertHistory a WHERE a.projectId = :projectId "
          + "AND a.firedAt >= :since ORDER BY a.firedAt DESC")
  List<AlertHistory> findByProjectIdSince(
      @Param("projectId") String projectId, @Param("since") Instant since);
  ...
  long countByProjectIdAndSeverityAndFiredAtAfter(String projectId, String severity, Instant after);
}
