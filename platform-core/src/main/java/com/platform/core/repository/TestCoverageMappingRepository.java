...
public interface TestCoverageMappingRepository extends JpaRepository<TestCoverageMapping, UUID> {
  ...
  List<TestCoverageMapping> findByProjectIdAndClassNameIn(
      UUID projectId, Collection<String> classNames);
  ...
  /**
   * Returns all wildcard coverage entries for a project — entries where className ends with {@code
   * .*} (e.g. {@code com.example.payment.*}). Used by TestImpactService to resolve
   * wildcard @AffectedBy annotations.
   */
  @Query(
      "SELECT m FROM TestCoverageMapping m WHERE m.projectId = :projectId AND m.className LIKE"
          + " '%.*'")
  ...
  @Query(
      "SELECT COUNT(DISTINCT m.className) FROM TestCoverageMapping m WHERE m.projectId ="
          + " :projectId")
  ...
  @Query(
      "SELECT COUNT(DISTINCT m.testCaseId) FROM TestCoverageMapping m WHERE m.projectId ="
          + " :projectId")
  long countMappedTestsByProject(@Param("projectId") UUID projectId);

  /**
   * Daily coverage breadth snapshots — distinct classes and tests seen per day, derived from
   * last_seen_at. Used by Grafana to plot coverage growth over time.
   */
  @Query(
      value =
          """
          ...
          """,
      nativeQuery = true)
  List<Object[]> dailyCoverageBreadth(
      @Param("projectId") UUID projectId, @Param("from") Instant from);
