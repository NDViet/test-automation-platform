...
public interface PlatformTestCaseRepository extends JpaRepository<PlatformTestCase, UUID> {
  ...
  @Query(
      "SELECT tc FROM PlatformTestCase tc WHERE tc.projectId = :projectId AND tc.hasAutomation ="
          + " true")
  ...
  @Query(
      value =
          ...
          """,
      nativeQuery = true)
  List<PlatformTestCase> findCoveringTestCases(
      @Param("projectId") UUID projectId, @Param("requirementId") UUID requirementId);

  @Query(
      "SELECT COUNT(tc) FROM PlatformTestCase tc WHERE tc.projectId = :projectId AND"
          + " tc.coverageStatus = 'ACTIVE'")
  ...
  List<PlatformTestCase> findByProjectIdAndAutomationStatus(
      UUID projectId, String automationStatus);

  List<PlatformTestCase> findByProjectIdAndStatusAndCreatedBy(
      UUID projectId, String status, String createdBy);

  List<PlatformTestCase> findByProjectIdAndSuiteId(UUID projectId, UUID suiteId);

  List<PlatformTestCase> findByProjectIdAndStatusAndSuiteId(
      UUID projectId, String status, UUID suiteId);
  ...
  @Query(
      "SELECT tc FROM PlatformTestCase tc WHERE tc.projectId = :projectId AND "
          + "(:search IS NULL OR LOWER(tc.title) LIKE LOWER(CONCAT('%', :search, '%')))")
  List<PlatformTestCase> searchByProjectId(
      @Param("projectId") UUID projectId, @Param("search") String search);
}
