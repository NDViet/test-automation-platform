...
public class ExecutionPersistenceService {
    ...
    /**
     * Creates a {@link TestExecution} immediately when a streaming run opens.
     * Counts are all zero; status is "RUNNING". Call {@link #finalizeStreamingExecution}
     * when the run closes to write final totals and flip the status to "COMPLETED".
     */
    @Transactional
    public TestExecution createStreamingExecution(
            String runId, String orgSlug, String projectSlug,
            String teamSlug, String areaSlug, String iterationPath,
            String branch, String environment, String commitSha, String ciRunUrl,
            String ciProvider, TriggerType triggerType, String workflow,
            Instant executedAt) {
        ...
        var execution = TestExecution.builder()
                .runId(runId)
                .project(project)
                .team(team)
                .areaSlug(areaSlug)
    ...
    /**
     * Updates count fields and marks a streaming execution as "COMPLETED".
     * Called by {@code StreamingRunService.finishRun()} after all test events are persisted.
     */
    @Transactional
    public void finalizeStreamingExecution(
        ...
        executionRepository.findByRunId(runId).ifPresent(e -> {
    ...
    public TestExecution persist(UnifiedTestResult result) {
        ...
        var team    = result.teamSlug() != null
                      ? findOrCreateTeam(project, result.teamSlug())
                      : null;
        ...
        log.info("Persisted execution runId={} org={} project={} team={} testType={} total={} failed={}",
                result.runId(), result.teamId(), result.projectId(), result.teamSlug(),
    ...
    private Team findOrCreateTeam(Project project, String slug) {
        return teamRepository.findByProjectIdAndSlug(project.getId(), slug).orElseGet(() -> {
            log.info("[Platform] Auto-registering new team slug={} projectId={}", slug, project.getId());
            try {
