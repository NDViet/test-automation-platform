...
/**
 * Expands a test case's properties into value combinations — either the full
 * Cartesian product or a reduced pairwise (all-pairs) set, mirroring Kiwi TCMS's
 * {@code TestRun.property_matrix}.
 *
 * <p>The pairwise generator is a deterministic greedy (AETG-style) algorithm:
 * every pair of values drawn from any two distinct properties appears in at least
 * one returned combination, typically with far fewer rows than the full product.</p>
 */
@Service
public class PropertyMatrixService {
     ...
     * @return ordered list of combinations (each an ordered map name → value);
     *         empty list when there are no axes
     */
    public List<LinkedHashMap<String, String>> expand(LinkedHashMap<String, List<String>> axes,
                                                      MatrixType type) {
        ...
        axes.forEach((k, v) -> { if (v != null && !v.isEmpty()) clean.put(k, v); });
