...
/**
 * Minimal Azure DevOps Boards REST client for the pull-based poller. Resolves the org/PAT/base-URL
 * from the project's {@code AZURE_DEVOPS_BOARDS} credential cascade (Org→Project→Team) and runs
 * WIQL queries + batch work-item fetches (with relations, for hierarchy/dependency links).
 */
@Component
public class AzureBoardsPollClient {
  ...
  /**
   * Resolved connection (org-rooted base URL + Basic auth) for a project, or null if not
   * configured.
   */
  ...
  public Ado connect(UUID projectId) {
    ResolvedCredential cred =
        credentialResolver.resolve(projectId, "AZURE_DEVOPS_BOARDS").orElse(null);
  ...
  /**
   * Returns work-item ids in {@code project} (optionally filtered by area path) that changed
   * on/after {@code sinceDate} (yyyy-MM-dd; null = all), oldest-changed first.
   */
  public List<String> queryWorkItemIds(Ado ado, String project, String areaPath, String sinceDate) {
    StringBuilder wiql =
        new StringBuilder("SELECT [System.Id] FROM WorkItems WHERE [System.TeamProject] = '")
            .append(esc(project))
            .append("'");
    ...
    JsonNode resp =
        send(
            "POST",
            "/" + enc(project) + "/_apis/wit/wiql?" + API,
            json(Map.of("query", wiql.toString())),
            ado);
  ...
  public List<JsonNode> getWorkItems(Ado ado, List<String> ids) {
    ...
    for (int i = 0; i < ids.size(); i += BATCH) {
      ...
      JsonNode resp =
          send(
              "POST",
              "/_apis/wit/workitemsbatch?" + API,
              json(Map.of("ids", chunk, "$expand", "Relations")),
              ado);
  ...
  public JsonNode getTeamMembers(Ado ado, String project, String teamId) {
    return send(
        "GET",
        "/_apis/projects/" + enc(project) + "/teams/" + enc(teamId) + "/members?" + API,
        null,
        ado);
  }
  ...
  public JsonNode getTeamFieldValues(Ado ado, String project, String teamId) {
    return send(
        "GET",
        "/" + enc(project) + "/" + enc(teamId) + "/_apis/work/teamsettings/teamfieldvalues?" + API,
        null,
        ado);
  }
  ...
  public JsonNode getWorkItemUpdates(Ado ado, String project, String workItemId) {
    return send(
        "GET",
        "/" + enc(project) + "/_apis/wit/workItems/" + enc(workItemId) + "/updates?" + API,
        null,
        ado);
  }

  /**
   * Returns the {@code authenticatedUser} node from {@code /_apis/connectionData}. Contains {@code
    ...
    try {
      ...
      if (resp.statusCode() >= 300) {
        throw new IllegalStateException(
            "Azure DevOps " + method + " " + path + " returned HTTP " + resp.statusCode());
      }
