...
/**
 * One-time, idempotent backfill of the legacy plaintext configs
...
public class CredentialBackfillRunner implements ApplicationRunner {
    ...
    private static final Set<String> SECRET_KEYS = Set.of(
            "apitoken", "api_token", "token", "pat", "password", "secret",
            "clientsecret", "client_secret", "apikey", "api_key", "accesstoken", "access_token");

    /** Legacy tracker_type -> new IntegrationType name for lifecycle trackers. */
    private static final Map<String, String> TRACKER_TYPE_MAP = Map.of(
            "JIRA", "JIRA_CLOUD",
    ...
    private int backfillTeamConfigs() {
        ...
        for (IntegrationConfig cfg : teamConfigRepo.findByEnabledTrue()) {
            String type = TRACKER_TYPE_MAP.getOrDefault(
                    cfg.getTrackerType(), cfg.getTrackerType());
            if (credRepo.findByScopeAndScopeIdAndIntegrationType(
                    Scope.TEAM.name(), cfg.getTeamId(), type).isPresent()) {
                continue; // already migrated
            }
            Split split = split(cfg.getConfigJson());
            if (cfg.getProjectKey() != null) split.nonSecret.putIfAbsent("project_key", cfg.getProjectKey());

            IntegrationCredential cred = new IntegrationCredential(
                    Scope.TEAM, cfg.getTeamId(), type,
                    "Migrated " + cfg.getTrackerType(), cfg.getBaseUrl(),
                    split.nonSecret, encryptSecret(split.secret));
    ...
    private int backfillProjectConfigs() {
        ...
        for (ProjectIntegrationConfig cfg : projectConfigRepo.findAll()) {
            ...
            if (credRepo.findByScopeAndScopeIdAndIntegrationType(
                    Scope.PROJECT.name(), projectId, type).isPresent()) {
                continue;
            }
            Map<String, String> params = cfg.getConnectionParams() == null
                    ? Map.of() : cfg.getConnectionParams();
            Split split = split(params);
            if (split.secret.isEmpty()) continue; // nothing secret to protect; leave PIC as-is

            IntegrationCredential cred = new IntegrationCredential(
                    Scope.PROJECT, projectId, type,
                    cfg.getDisplayName(), split.nonSecret.get("base_url"),
                    split.nonSecret, encryptSecret(split.secret));
            cred.setCreatedBy("backfill");
            credRepo.save(cred);
            n++;
