 ...
 *
 * <p>Uses AES-256-GCM. The master key is supplied via the {@code platform.cred.key}
 * property (env var {@code PLATFORM_CRED_KEY}) as a base64-encoded 32-byte key.
 * Ciphertext is encoded as {@code v1:base64(iv(12) || ciphertext || tag)} so the
 * scheme can evolve without ambiguity.</p>
 *
 * <p>If no key is configured the cipher fails fast on first use — secrets must
 * never be written or read as plaintext once this layer exists.</p>
 */
@Component
public class CredentialCipher {
    ...
    public CredentialCipher(@Value("${platform.cred.key:}") String base64Key) {
        if (base64Key == null || base64Key.isBlank()) {
            this.key = null;
            log.warn("[CredentialCipher] platform.cred.key (PLATFORM_CRED_KEY) is not set. "
                    + "Encrypting or decrypting integration credentials will fail until it is configured.");
        } else {
            byte[] raw;
            try {
            ...
            if (raw.length != KEY_BYTES) {
                throw new IllegalStateException(
                        "platform.cred.key must decode to exactly " + KEY_BYTES
                                + " bytes (AES-256); got " + raw.length + " bytes");
            }
    ...
    public String decrypt(String token) {
        ...
        if (!token.startsWith(VERSION_PREFIX)) {
            throw new IllegalStateException("Unrecognized credential ciphertext format (missing version prefix)");
        }
        try {
