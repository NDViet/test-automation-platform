...
/**
 * Re-encrypts all {@code integration_credentials.secret_ciphertext} rows from an old AES-256-GCM
 * key to the current key.
 *
 * <p>Activate by setting <strong>both</strong>:
 *
 * <ul>
 *   <li>{@code PLATFORM_CRED_KEY} — the new (replacement) key
 *   <li>{@code PLATFORM_CRED_KEY_PREVIOUS} — the old key to decrypt existing rows
 * </ul>
 *
 * <p>The runner is <strong>idempotent</strong>: rows already encrypted with the new key cannot be
 * decrypted by the old key, so they are skipped automatically. After a successful run, unset {@code
 * PLATFORM_CRED_KEY_PREVIOUS} and redeploy.
 */
@Component
  ...
  public void run(ApplicationArguments args) {
    ...
    for (var cred : credRepo.findAll()) {
      ...
      try {
        ...
        skipped++;
        log.debug(
            "[KeyRotation] Skipped credential id={} (already rotated or unreadable): {}",
            cred.getId(),
            e.getMessage());
      }
    }
    ...
    log.info(
        "[KeyRotation] Rotation complete: rotated={}, skipped={}, errors={}",
        rotated,
        skipped,
        errors);
