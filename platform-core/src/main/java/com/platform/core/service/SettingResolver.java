...
/**
 * Resolves an effective setting value by deep-merging the ORG, TEAM and PROJECT
 * scopes with precedence PROJECT &gt; TEAM &gt; ORG.
 *
 * <p>ORG defaults live in {@code platform_settings} (the existing global table);
 * TEAM/PROJECT overrides live in {@code scoped_settings}. This mirrors
 * {@link CredentialResolver} but for plain key/value settings (e.g. the AI
 * provider/model/enable flags).</p>
 */
@Service
public class SettingResolver {
    ...
    public SettingResolver(PlatformSettingRepository orgRepo,
    ...
    /**
     * Effective value for a project: PROJECT override → org-wide default
     * ({@code platform_settings}). ADO-first hierarchy Org → Project → Team.
     */
    ...
    public void set(Scope scope, UUID scopeId, String key, String value) {
        ScopedSetting existing = scopedRepo
    ...
    private Optional<String> scoped(Scope scope, UUID scopeId, String key) {
        return scopedRepo.findByScopeAndScopeIdAndKey(scope.name(), scopeId, key)
                .map(ScopedSetting::getValue)
                .filter(v -> v != null && !v.isBlank());
    }

    private Optional<String> org(String key) {
        return orgRepo.findById(key)
                .map(s -> s.getValue())
                .filter(v -> v != null && !v.isBlank());
    }
}
