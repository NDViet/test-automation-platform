 ...
 *
 * <p>Enforces the "confirmed gate": only {@code APPROVED} test cases may enter a run (mirrors
 * Kiwi's DRAFT→CONFIRMED requirement). Each case's properties expand into one {@link
 * TestCaseExecution} per value combination (full or pairwise), with the chosen values recorded as
 * {@link TestExecutionProperty} rows.
 */
@Service
public class TcmRunService {
  ...
  /**
   * Creates a run for the given approved test cases, expanding each case's property matrix into
   * executions.
   *
   * @throws IllegalArgumentException if any requested case is missing, belongs to another project,
   *     or is not {@code APPROVED}.
   */
  @Transactional
