 ...
 *
 * <p>One row per (scope, scopeId, integrationType). {@code scope} is one of {@code ORG} / {@code
 * TEAM} / {@code PROJECT}; {@code scopeId} is NULL for ORG, the team id for TEAM, or the project id
 * for PROJECT. Resolution deep-merges the three scopes with precedence PROJECT &gt; TEAM &gt; ORG
 * (see {@code CredentialResolver}).
 *
 * <p>{@code connectionParams} holds NON-secret fields (org, project_key, area_path, repo, owner,
 * etc.). {@code secretCiphertext} holds the AES-GCM encrypted JSON of secret fields (e.g. {@code
 * {"pat":"...","clientSecret":"..."}}).
 */
@Entity
...
public class IntegrationCredential {
  ...
  public void setConnectionParams(Map<String, String> v) {
    this.connectionParams = v;
  }

  public void setSecretCiphertext(String v) {
    this.secretCiphertext = v;
  }
  ...
  public void setSyncIntervalMinutes(int v) {
