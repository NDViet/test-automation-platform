...
/**
 * Maps a test ID ↔ issue tracker ticket (JIRA key, Linear ID, GitHub issue number). Unique per
 * (testId, projectId, trackerType) so each test has at most one open ticket per tracker.
 */
@Entity
@Table(
    name = "issue_tracker_links",
    uniqueConstraints =
        @UniqueConstraint(
