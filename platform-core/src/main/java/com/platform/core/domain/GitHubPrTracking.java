...
/**
 * One row per open PR per project.
 * Records the last head SHA that triggered an analysis workflow.
 * A new workflow is only triggered when head_sha changes — filtering out
 * comment/label/CI-rerun noise that bumps GitHub's updated_at without changing code.
 */
@Entity
@Table(name = "github_pr_tracking",
...
public class GitHubPrTracking {
    ...
    public GitHubPrTracking(UUID projectId, String repoFullName, int prNumber,
                             String headSha, String prUrl, UUID workflowId) {
    ...
    public UUID getTriggeredWorkflowId()  { return triggeredWorkflowId; }
}
