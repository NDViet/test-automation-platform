...
export const api = {
  ...
  createApiKey: (body: { name: string; ttlDays?: number }) => post<{ id: string; prefix: string; rawKey: string; expiresAt: string | null }>('/api-keys', body),
  ...
  requirements: (projectId: string, params?: { status?: string; issueType?: string; search?: string; area?: string; team?: string; iteration?: string }) => {
  ...
  requirementsPage: (projectId: string, params: { page?: number; size?: number; status?: string; issueType?: string; search?: string; area?: string; team?: string; iteration?: string }) => {
    const q = new URLSearchParams()
  ...
  uploadOrgLogo: (id: string, file: File) => {
    ...
    return fetch(`${BASE}/organizations/${id}/logo`, { method: 'POST', body: form })
      .then(r => { if (!r.ok) throw new Error('Upload failed'); return r.json() as Promise<import('./types').Organization> })
  },
  ...
  suiteCases: (projectId: string, suiteId: string) => get<import('./types').SelectableTestCase[]>(`/projects/${projectId}/test-suites/${suiteId}/cases`),
  caseSuites: (projectId: string, tcId: string) => get<string[]>(`/projects/${projectId}/test-cases/${tcId}/suites`),
  ...
  coverage: (projectId: string, params?: { area?: string; team?: string; iteration?: string }) => {
    ...
    if (params?.iteration) q.set('iteration', params.iteration)
    const qs = q.toString()
    return get<import('./types').CoverageReport>(`/projects/${projectId}/coverage${qs ? '?' + qs : ''}`)
  },
  ...
  // Test Cases
  testCases: (projectId: string, params?: { status?: string; suiteId?: string; search?: string; area?: string; teamId?: string; iteration?: string }) => {
    const q = new URLSearchParams()
    if (params?.status)    q.set('status', params.status)
  ...
  selectableTestCases: (projectId: string, params?: { status?: string; area?: string; iteration?: string; teamId?: string; q?: string }) => {
    ...
    return get<import('./types').SelectableTestCase[]>(`/projects/${projectId}/test-cases/selectable${qs ? '?' + qs : ''}`)
  },
  ...
  // Releases
  releases: (projectId: string) => get<import('./types').Release[]>(`/projects/${projectId}/releases`),
  ...
  unifiedExecutions: (projectId: string, params?: {
  ...
  updateExecutionScope: (projectId: string, runId: string, params: { iterationPath?: string; areaSlug?: string }) => {
    const q = new URLSearchParams()
    if (params.iterationPath) q.set('iterationPath', params.iterationPath)
    if (params.areaSlug)      q.set('areaSlug',      params.areaSlug)
    return patch(`/projects/${projectId}/executions/${encodeURIComponent(runId)}/scope?${q.toString()}`)
  },

  // Test Execution — release board (grouped by team, with coverage)
  testExecutionBoard: (projectId: string, scope?: { iteration?: string; area?: string; team?: string }) => {
    const q = new URLSearchParams()
    if (scope?.iteration) q.set('iteration', scope.iteration)
    if (scope?.area)      q.set('area', scope.area)
    if (scope?.team)      q.set('team', scope.team)
    const qs = q.toString()
    return get<import('./types').ReleaseBoard>(`/projects/${projectId}/test-execution-board${qs ? '?' + qs : ''}`)
  },
  // Test Execution monitor (rollups by Release / Sprint / Area / Team)
  testExecutionBy: (projectId: string, dimension: 'release' | 'sprint' | 'area' | 'team', scope?: { area?: string; team?: string; iteration?: string }) => {
    const q = new URLSearchParams()
    if (scope?.area)      q.set('area', scope.area)
    if (scope?.team)      q.set('team', scope.team)
    if (scope?.iteration) q.set('iteration', scope.iteration)
    const qs = q.toString()
    return get<import('./types').ExecMonitorOverview>(`/projects/${projectId}/test-execution/by-${dimension}${qs ? '?' + qs : ''}`)
  },
  testExecutionRuns: (projectId: string, dimension: 'release' | 'sprint' | 'area' | 'team', value?: string | null) => {
    const q = new URLSearchParams({ dimension })
    if (value) q.set('value', value)
    return get<import('./types').ExecRunSummary[]>(`/projects/${projectId}/test-execution-runs?${q.toString()}`)
  },
  ...
  automatedTests: (projectId: string, params?: {
    days?: number
    search?: string
    status?: string
    tags?: string[]
    ...
    return get<import('./types').AutomatedTestSummary[]>(`/projects/${projectId}/automated-tests?${q.toString()}`)
  },
  ...
  automatedTestLabelValues: (projectId: string, days = 30, labelKey: string) =>
    get<string[]>(`/projects/${projectId}/automated-tests/label-values?days=${days}&labelKey=${encodeURIComponent(labelKey)}`),
  /** Returns the absolute URL to download a trace ZIP (used to open the Playwright trace viewer). */
  traceUrl: (resultId: string) =>
    `${window.location.origin}/api/portal/traces/${resultId}`,
  ...
  githubRepos: (credId: string) => get<import('./types').GithubRepo[]>(`/credentials/${credId}/github/repos`),
  ...
  updateSyncInterval: (credId: string, minutes: number) =>
    patchJson<import('./types').Credential>(`/credentials/${credId}/sync-interval?minutes=${minutes}`),
  ...
  setProjectGitHubRepos: (projectId: string, assignments: { repoFullName: string; role: string; credentialId: string }[]) =>
    put<import('./types').ProjectRepoAssignment[]>(`/projects/${projectId}/github/repos`, { assignments }),
  ...
  triggerWorkflow: (projectId: string, body: { repoFullName: string; workflowId: number; ref: string; inputs?: Record<string, string> }) =>
    post<import('./types').WorkflowDispatchResult>(`/projects/${projectId}/github/workflow-dispatch`, body),
}
