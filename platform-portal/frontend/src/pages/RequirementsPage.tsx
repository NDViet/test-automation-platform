...
import {
  ChevronRight, ChevronDown, ChevronLeft, Search, X,
...
const PRIORITY_COLORS: Record<string, string> = {
  CRITICAL: 'text-red-700 bg-red-100', HIGH: 'text-orange-700 bg-orange-100',
  MEDIUM: 'text-yellow-700 bg-yellow-100', LOW: 'text-slate-600 bg-slate-100',
}
const ISSUE_TYPE_COLORS: Record<string, string> = {
  EPIC: 'text-purple-700 bg-purple-100', CAPABILITY: 'text-fuchsia-700 bg-fuchsia-100',
  ENABLER: 'text-indigo-700 bg-indigo-100', FEATURE: 'text-sky-700 bg-sky-100',
  REQUIREMENT: 'text-cyan-700 bg-cyan-100', STORY: 'text-blue-700 bg-blue-100',
  BUG: 'text-red-700 bg-red-100', DEFECT: 'text-red-700 bg-red-100',
  TASK: 'text-slate-600 bg-slate-100', SUBTASK: 'text-teal-700 bg-teal-100',
...
function fmtDate(iso: string | null)   { return iso ? new Date(iso).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : '—' }

// ── Tree data structure ───────────────────────────────────────────────────────

interface TreeNode extends Requirement { children: TreeNode[] }
  ...
  return (
    <div className="flex items-center gap-1.5 group">
      <button onClick={() => onNavigate(r.id)}
        ...
        {r.externalId && <span className="text-xs font-mono text-slate-400 shrink-0">#{r.externalId}</span>}
        <span className="text-xs text-slate-700 truncate">{r.title}</span>
      </button>
      {r.sourceUrl && (
        <a href={r.sourceUrl} target="_blank" rel="noreferrer" title="Open in Azure DevOps"
           className="text-slate-300 hover:text-blue-600 shrink-0"><ExternalLink size={11} /></a>
      )}
    </div>
  )
}

function RequirementDetail({ req, projectId, onClose, onNavigate }: {
  req: Requirement; projectId: string; onClose: () => void; onNavigate: (id: string) => void
}) {
    ...
    <div className="w-96 shrink-0 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col
      ...
      <div className="overflow-y-auto p-5 space-y-5 flex-1">
        ...
        {(relations?.parent || (relations?.children?.length ?? 0) > 0) && (
          <div className="space-y-2">
            ...
            {relations && relations.children.length > 0 && (
              <div>
                <p className="text-xs font-medium text-slate-500 mb-1">Children ({relations.children.length})</p>
                <div className="space-y-0.5">
                  {relations.children.map(c => <RelationRow key={c.id} r={c} onNavigate={onNavigate} />)}
                </div>
              </div>
            )}
          </div>
        ...
        <div className="border-t border-slate-100 pt-3 space-y-1">
          {req.createdDate && <p className="text-xs text-slate-500">Created {fmtDate(req.createdDate)}</p>}
...
interface TreeFilter { q: string; status: string; issueType: string }
function filterActive(f: TreeFilter): boolean { return !!(f.q || f.status || f.issueType) }
...
function computeVisibleIds(nodes: TreeNode[], f: TreeFilter): Set<string> | null {
  ...
  function check(node: TreeNode): boolean {
    ...
    let childHit = false
    node.children.forEach(c => { if (check(c)) childHit = true })
...
function TreeNodeRow({ node, depth, selected, onSelect, expanded, onToggle, visibleIds, searchQuery }: {
  node: TreeNode; depth: number; selected: Requirement | null; onSelect: (r: Requirement) => void
  expanded: Set<string>; onToggle: (id: string) => void; visibleIds: Set<string> | null; searchQuery: string
}) {
  ...
  return (
    <>
      <div
        className={cn('flex items-center gap-1 pr-4 py-2 cursor-pointer hover:bg-slate-50 transition-colors',
          isSelected && 'bg-blue-50 hover:bg-blue-50', isSelfMatch && !isSelected && 'bg-yellow-50 hover:bg-yellow-50')}
        style={{ paddingLeft: `${depth * 20 + 12}px` }} onClick={() => onSelect(node)}>
        <button className={cn('shrink-0 w-5 h-5 flex items-center justify-center rounded text-slate-400',
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {node.externalId && <span className="text-xs font-mono text-slate-400 shrink-0">{highlightMatch(node.externalId, searchQuery)}</span>}
          <span className={cn('text-sm truncate', isSelected ? 'font-semibold text-blue-900' : 'text-slate-800')}>
            {highlightMatch(node.title, searchQuery)}
          </span>
          {node.changeSummary && <span className="text-xs text-amber-500 shrink-0" title={node.changeSummary}>⚡</span>}
        </div>
        <div className="flex items-center gap-2 shrink-0 ml-2">
          <span className="text-xs text-slate-400 hidden sm:inline" title="Created date">{fmtDate(node.createdDate)}</span>
      ...
      {isExpanded && hasChildren && (
        <div className="relative">
          <div className="absolute top-0 bottom-0 border-l border-slate-200" style={{ left: `${depth * 20 + 20}px` }} />
          {node.children.map(child => (
            <TreeNodeRow key={child.id} node={child} depth={depth + 1} selected={selected} onSelect={onSelect}
              expanded={expanded} onToggle={onToggle} visibleIds={visibleIds} searchQuery={searchQuery} />
          ))}
        </div>
      )}
    </>
...
export default function RequirementsPage() {
  ...
  useEffect(() => { setPage(0) }, [status, issueType, size])
  ...
  useEffect(() => { if (viewMode === 'tree') setExpanded(new Set()) }, [viewMode, treeQ.data]) // start collapsed
  ...
  async function navigateToRequirement(id: string) {
    try { setSelected(await api.requirementDetail(projectId!, id)) }
    catch { /* ignore — item may have been removed */ }
  }
    ...
    <div className="flex flex-col gap-4">
          ...
          <div className="flex rounded-lg border border-slate-200 overflow-hidden">
            {(['list', 'tree'] as ViewMode[]).map(mode => (
              <button key={mode} onClick={() => { setViewMode(mode); setSelected(null) }}
                ...
                {mode === 'list' ? <List size={13} /> : <Network size={13} />}{mode === 'list' ? 'List' : 'Tree'}
              </button>
            ))}
          </div>
          <button onClick={() => navigate(`${base}/pr-analyses`)}
      ...
      <div className="flex gap-2 flex-wrap items-center">
        <div className="relative flex-1 min-w-[16rem]">
          ...
          {searchInput && (
            <button onClick={() => setSearchInput('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"><X size={14} /></button>
          )}
        </div>
        <select value={status} onChange={e => setStatus(e.target.value)}
          ...
          {stats && Object.keys(stats.byStatus).sort().map(s => <option key={s} value={s}>{s} ({stats.byStatus[s]})</option>)}
        </select>
        <select value={issueType} onChange={e => setIssueType(e.target.value)}
          className="text-sm border border-slate-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
          <option value="">All types</option>
          {issueTypeOptions.map(t => <option key={t} value={t}>{t} ({stats!.byIssueType[t]})</option>)}
        </select>
        {(status || issueType || searchInput) && (
          <button onClick={() => { setStatus(''); setIssueType(''); setSearchInput('') }}
            className="text-sm text-slate-500 hover:text-slate-800 px-2">Clear</button>
        )}
      </div>
  ...
  function listView() {
    ...
    if (items.length === 0) {
      return <p className="px-5 py-12 text-sm text-slate-500 text-center">
      ...
      <div className="flex flex-col">
            ...
            <button key={r.id} onClick={() => setSelected(selected?.id === r.id ? null : r)}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    ...
                    {r.sourceUrl && (
                      <span role="link" tabIndex={0}
                        ...
                        onClick={e => { e.stopPropagation(); window.open(r.sourceUrl!, '_blank', 'noopener,noreferrer') }}
                        onKeyDown={e => { if (e.key === 'Enter') { e.stopPropagation(); window.open(r.sourceUrl!, '_blank', 'noopener,noreferrer') } }}
                  ...
                  {r.changeSummary && <p className="text-xs text-amber-600 mt-0.5 truncate">⚡ {r.changeSummary}</p>}
                </div>
                <div className="flex flex-col items-end gap-1 shrink-0">
                  ...
                  <p className="text-xs text-slate-500" title="Created date">Created {fmtDate(r.createdDate)}</p>
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-3 border-t border-slate-100 text-sm">
          <div className="flex items-center gap-2 text-slate-500">
            <span>Page {page + 1} of {Math.max(totalPages, 1)} · {total} item{total !== 1 ? 's' : ''}</span>
  ...
  function treeView() {
    ...
    if (visibleIds && visibleIds.size === 0) {
      return <p className="px-5 py-8 text-sm text-slate-500 text-center">No requirements match the current filters.</p>
    }
    return (
      <div className="py-2">
        {roots.map(root => (
          <TreeNodeRow key={root.id} node={root} depth={0} selected={selected}
            onSelect={r => setSelected(selected?.id === r.id ? null : r)}
            expanded={expanded} onToggle={toggleNode} visibleIds={visibleIds} searchQuery={f.q} />
        ))}
        {capped && (
          <p className="px-5 py-3 text-xs text-amber-600 border-t border-slate-100">
            Showing first {MAX_TREE_ROOTS} of {treeData.length} top-level items — search to narrow, or use the List view.
          </p>
        )}
      </div>
    )
