...
export default function ProductivityPage() {
  ...
  const cycleQ = useQuery({
    queryKey: ['productivity-by-area', projectId],
    queryFn: () => api.productivityByArea(projectId),
  })
  const leadQ = useQuery({
    queryKey: ['productivity-lead', projectId],
    queryFn: () => api.productivityLeadByArea(projectId),
  })

  if (cycleQ.isLoading) return <LoadingSpinner message="Computing cycle times…" />
  if (cycleQ.error || !cycleQ.data)
    return (
      <ErrorMessage
        message="Failed to load productivity metrics."
        onRetry={() => void cycleQ.refetch()}
      />
    )
  const c = cycleQ.data
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Cycle time (work in progress) and lead time (created → completed), by area.
          </p>
        </div>
        <ThresholdEditor
          projectId={projectId}
          current={c.thresholdHours}
      ...
      <SectionTitle
        icon={Timer}
        title="Cycle time"
        subtitle="Running age of started-but-not-completed work"
      />
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Kpi
          ...
          tint="text-slate-700"
          onClick={() =>
            setDrill({ kind: 'wip', area: undefined, over: false, title: 'In progress' })
          }
        />
        <Kpi
          icon={AlertTriangle}
          label={`Over ${fmtH(c.thresholdHours)} target`}
          value={c.totalOver}
          tint="text-red-600"
          onClick={() =>
            setDrill({
              ...
              area: undefined,
              over: true,
              title: `Over ${fmtH(c.thresholdHours)} target`,
            })
          }
        />
        <Kpi
          ...
          label="% over target"
          value={`${pctOver}%`}
          tint={pctOver > 30 ? 'text-red-600' : 'text-amber-600'}
        />
        <Kpi
          icon={Layers}
          label="Areas needing action"
          value={c.areasAffected}
          tint="text-slate-700"
        />
      </div>
                ...
                {c.areas.map((a: AreaProductivity) => {
                  ...
                  return (
                    <tr key={a.area} className="hover:bg-slate-50">
                      <td className="px-5 py-2.5 text-slate-700" title={a.area}>
                        {leaf(a.area)}
                        <span className="text-xs text-slate-400 font-mono ml-2 hidden lg:inline">
                          {a.area}
                        </span>
                      </td>
                      <NumCell
                        ...
                        onClick={() =>
                          setDrill({
                            ...
                            area: a.area,
                            over: false,
                            title: `In progress · ${leaf(a.area)}`,
                          })
                        }
                      />
                      <NumCell
                        value={a.overThreshold}
                        danger
                        onClick={() =>
                          setDrill({
                            kind: 'wip',
      ...
      <SectionTitle
        icon={Hourglass}
        title="Lead time"
        subtitle="From work-item creation until completion"
      />
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <Kpi
          icon={CheckCircle2}
          label="Completed"
          value={lead?.totalCompleted ?? 0}
          tint="text-green-600"
          onClick={() =>
            setDrill({ kind: 'lead', area: undefined, title: 'Completed (lead time)' })
          }
        />
        <Kpi
          icon={Hourglass}
          label="Avg lead time"
          value={fmtH(lead?.avgHours ?? null)}
          tint="text-slate-700"
        />
        <Kpi
          icon={Hourglass}
          label="Max lead time"
          value={fmtH(lead?.maxHours ?? null)}
          tint="text-amber-600"
        />
      </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
      ...
      {clickable ? (
        <button
          onClick={onClick}
          className={`text-2xl font-bold ${tint} hover:underline decoration-dotted underline-offset-4`}
        >
          {value}
        </button>
      ) : (
...
function DrillModal({
  ...
  const wipQ = useQuery({
    ...
    queryFn: () =>
      api.productivityWipItems(projectId, drill.area, drill.kind === 'wip' ? drill.over : true),
      ...
      >
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-200">
          <h2 className="text-sm font-semibold text-slate-800">
            {drill.title}{' '}
            {q.data && <span className="text-slate-400 font-normal">({items.length})</span>}
          </h2>
        ...
        <div className="overflow-y-auto flex-1">
          {q.isLoading ? (
            ...
            <ErrorMessage message="Failed to load." onRetry={() => void q.refetch()} />
          ) : items.length === 0 ? (
            <Empty text="No items." />
          ) : (
            <table className="w-full text-sm">
...
function ThresholdEditor({
  ...
  const save = useMutation({
    mutationFn: () => api.setProductivityThreshold(projectId, Number(val)),
    onSuccess: onSaved,
  })
...
function Empty({ text }: { text: string }) {
  return <p className="px-5 py-10 text-sm text-slate-500 text-center">{text}</p>
}
