...
function shortPath(p: string | null): string {
  if (!p) return '(no area)'; const x = p.split('\\'); return x[x.length - 1]
}
...
function CoverageBar({ g }: { g: CoverageGroup }) {
  ...
  return (
    <div className="flex h-2.5 rounded-full overflow-hidden bg-slate-100 w-full min-w-[140px]">
      <div className="bg-green-500" style={{ width: pct(g.coveredByAutomation) }} title={`${g.coveredByAutomation} automated`} />
      <div className="bg-blue-400"  style={{ width: pct(g.manualOnly) }} title={`${g.manualOnly} manual`} />
      <div className="bg-red-300"   style={{ width: pct(g.uncovered) }} title={`${g.uncovered} gaps`} />
    </div>
  )
}
...
export default function CoverageMatrixPage() {
  ...
  const groupKey = (r: CoverageRow) => groupBy === 'area' ? (r.areaPath ?? '(no area)') : (r.teamName ?? '(unassigned)')
  ...
  const rows = selectedGroup
    ? data.requirements
        ...
        .filter(r => listMode === 'uncovered' ? coverageState(r) === 'gap' : coverageState(r) !== 'gap')
    : []
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <h2 className="text-sm font-semibold text-slate-700">Coverage by {groupBy === 'area' ? 'Area' : 'Team'}</h2>
          <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
            <button onClick={() => { setGroupBy('area'); setSelectedGroup(null) }}
              className={cn('flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-md',
                groupBy === 'area' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700')}>
              <LayoutGrid size={13} /> Area
            </button>
            <button onClick={() => { setGroupBy('team'); setSelectedGroup(null) }}
              className={cn('flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-md',
                groupBy === 'team' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700')}>
              <Users size={13} /> Team
            </button>
          </div>
        </div>
            ...
            {groups.map(g => {
              ...
              return (
                <tr key={g.label}
                    ...
                    className={cn('cursor-pointer transition-colors', active ? 'bg-blue-50' : 'hover:bg-slate-50')}>
                  <td className="px-4 py-2.5 font-medium text-slate-800 max-w-xs truncate" title={g.label}>{labelFor(g)}</td>
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      <CoverageBar g={g} />
                      <span className={`text-xs font-semibold tabular-nums ${pctColorCls(g.coveragePct)}`}>{g.coveragePct}%</span>
                    </div>
                  </td>
                  <td className="px-4 py-2.5 text-right tabular-nums text-slate-700">{g.covered}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums">
                    <span className={pctColorCls(g.automationPct)}>{g.automationPct}%</span>
                  </td>
                  <td className="px-4 py-2.5 text-right tabular-nums">
                    <span className={g.uncovered > 0 ? 'text-red-600 font-medium' : 'text-slate-300'}>{g.uncovered}</span>
                  </td>
      ...
      {!selectedGroup ? (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm py-12 text-center">
          <p className="text-sm text-slate-500">Select an Area or Team above to see its requirements.</p>
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
            ...
            <button onClick={() => setSelectedGroup(null)} className="ml-1.5 hover:text-blue-900">✕</button>
          </span>
          <div className="ml-auto flex gap-1 bg-slate-100 p-1 rounded-lg">
            <button onClick={() => setListMode('covered')}
              className={cn('px-3 py-1 text-xs font-medium rounded-md',
                listMode === 'covered' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700')}>
              Covered
            </button>
            <button onClick={() => setListMode('uncovered')}
              className={cn('px-3 py-1 text-xs font-medium rounded-md',
                listMode === 'uncovered' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700')}>
              Uncovered
            </button>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-50">
              {rows.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-slate-500">
                  {listMode === 'uncovered' ? 'No uncovered requirements 🎉' : 'No covered requirements yet.'}
                </td></tr>
              )}
                  ...
                  <tr key={row.requirementId} className={state === 'gap' ? 'bg-red-50/40' : ''}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {row.externalId && (
                          <span className="text-xs font-mono px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 shrink-0">{row.externalId}</span>
                        )}
                        <span className="text-slate-800 truncate">{row.title}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-xs text-slate-600 truncate" title={`${row.areaPath ?? ''} · ${row.teamName ?? ''}`}>{shortPath(row.areaPath)}</div>
                      <div className="text-[11px] text-slate-400 truncate">{row.teamName ?? '(unassigned)'}</div>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={row.automatedCases > 0 ? 'text-green-700 font-medium' : 'text-slate-300'}>{row.automatedCases}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={row.manualCases > 0 ? 'text-blue-700' : 'text-slate-300'}>{row.manualCases}</span>
                    </td>
          ...
          {rows.length > 500 && (
            <p className="px-4 py-2 text-xs text-slate-400 border-t border-slate-100">Showing first 500 of {rows.length}.</p>
          )}
        </div>
      </div>
      )}
