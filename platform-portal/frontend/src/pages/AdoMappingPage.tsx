...
import { Boxes, Bug, FileText, Ban, Copy, Check, Wand2, SlidersHorizontal, ChevronDown, ChevronRight,
  AlertTriangle, CheckCircle2, RefreshCw } from 'lucide-react'
...
function laneBadge(lane: string) {
  ...
  if (lane === 'REQUIREMENT') return { cls: 'text-blue-700 bg-blue-100', icon: <FileText size={12} /> }
  return { cls: 'text-slate-500 bg-slate-100', icon: <Ban size={12} /> }
}
function catColor(cat: string) {
  switch (cat) {
    case 'Proposed':   return 'text-slate-600 bg-slate-100'
...
export default function AdoMappingPage() {
  ...
  const projectsQ = useQuery({ queryKey: ['adoProjects', projectId], queryFn: () => api.adoProjects(projectId) })
  ...
  const baselineMutation = useMutation({
    ...
    onSuccess: () => void qc.invalidateQueries({ queryKey: ['adoDrift', projectId, effProject, selectedType] }),
  })
  ...
  const copy = async () => { await navigator.clipboard.writeText(profileText); setCopied(true); setTimeout(() => setCopied(false), 1500) }

  const grouped = (typesQ.data ?? []).reduce<Record<string, AdoTypeSummary[]>>((acc, t) => {
    (acc[t.suggestedLane] ??= []).push(t); return acc
  }, {})
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Boxes size={22} /> Mapping Discovery</h1>
        <p className="text-sm text-slate-500 mt-1">
          Explore the Azure DevOps work-item structure and get a suggested mapping profile to start from.
        </p>
      </div>

      <div className="flex items-center gap-3">
        <label className="text-sm font-medium text-slate-700">ADO project:</label>
        <select value={effProject} onChange={e => { setAdoProject(e.target.value); setSelectedType('') }}
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="col-span-1 space-y-3">
          ...
          {laneOrder.filter(l => grouped[l]?.length).map(lane => (
                ...
                {grouped[lane].map(t => {
                  ...
                  return (
                    <button key={t.name} onClick={() => setSelectedType(t.name)}
                      ...
                      <span className={`inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded ${b.cls}`}>{b.icon}</span>
                      <span className="flex-1 truncate text-slate-800">{t.name}</span>
                      {t.custom && <span className="text-[10px] px-1 py-0.5 rounded bg-amber-100 text-amber-700">custom</span>}
                    </button>
                  )
                })}
              </div>
        ...
        <div className="col-span-2 space-y-4">
          {!selectedType && <div className="text-sm text-slate-400 py-10 text-center bg-white rounded-xl border border-slate-200">Select a work-item type to view its fields, states, and a suggested mapping.</div>}
          {selectedType && schemaQ.isLoading && <LoadingSpinner message={`Inspecting ${selectedType}…`} />}
            ...
            <>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="max-h-60 overflow-y-auto divide-y divide-slate-50">
                  {schemaQ.data.fields.filter(f => f.custom || ['System.Title','System.Description','System.Parent','Microsoft.VSTS.Common.Severity','Microsoft.VSTS.Common.Priority','Microsoft.VSTS.Common.BusinessValue'].includes(f.referenceName)).map(f => (
                    <div key={f.referenceName} className="px-4 py-1.5 flex items-center gap-2 text-xs">
                      <code className="font-mono text-slate-700">{f.referenceName}</code>
                      <span className="text-slate-400">{f.name}</span>
                      {f.custom && <span className="px-1 rounded bg-amber-100 text-amber-700">custom</span>}
                      {f.required && <span className="px-1 rounded bg-slate-100 text-slate-500">required</span>}
                    </div>
                  ))}
                  <p className="px-4 py-1.5 text-[11px] text-slate-400">Showing custom + mappable fields. {schemaQ.data.fields.length} total.</p>
                </div>
              </div>

              {/* States */}
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3">
                <p className="text-sm font-semibold text-slate-900 mb-2">States → category (drives status normalization)</p>
                <div className="flex flex-wrap gap-1.5">
                  {schemaQ.data.states.map(s => (
                    <span key={s.name} className={`text-xs px-2 py-0.5 rounded ${catColor(s.category)}`}>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                <div className="px-4 py-2 border-b border-slate-100 flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-900">Suggested mapping profile</span>
                  <div className="ml-auto flex items-center gap-1">
                    {(['yaml', 'json'] as const).map(f => (
                      <button key={f} onClick={() => setFmt(f)}
                        className={`px-2 py-0.5 text-xs rounded ${fmt === f ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>{f.toUpperCase()}</button>
                    ))}
                    <button onClick={copy} className="ml-1 inline-flex items-center gap-1 text-xs px-2 py-0.5 border border-slate-200 rounded hover:bg-slate-50">
                      {copied ? <Check size={12} /> : <Copy size={12} />} {copied ? 'Copied' : 'Copy'}
                    </button>
                  </div>
                </div>
                <pre className="text-xs font-mono text-slate-800 bg-slate-50 p-3 overflow-x-auto max-h-[28rem] leading-relaxed">{profileText}</pre>
              </div>
            </>
          )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
        <button
          ...
          {showRules ? <ChevronDown size={16} className="text-slate-400" /> : <ChevronRight size={16} className="text-slate-400" />}
          <SlidersHorizontal size={15} className="text-slate-500" />
          <span className="text-sm font-semibold text-slate-900">Mapping rules for this project</span>
          <span className="text-xs text-slate-400">override the org default · controls these suggestions</span>
        </button>
        {showRules && (
...
function DriftBanner({ report, onUpdateBaseline, updating }: {
  ...
  if (!report.hasBaseline) {
    return (
      <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs text-slate-500">
        Baseline captured for <span className="font-mono">{report.workItemType}</span>. Future schema changes
        (removed / renamed / added fields) will be flagged here.
      </div>
    )
  }
  if (!report.hasDrift) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center gap-2 text-sm text-green-800">
        ...
        {report.baselineCapturedAt && <span className="text-xs text-green-600">· captured {relativeTime(report.baselineCapturedAt)}</span>}
      </div>
    )
  }
    ...
    <div className="bg-amber-50 border border-amber-300 rounded-xl p-4 space-y-2">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm font-semibold text-amber-900 flex items-center gap-2">
          ...
          {report.baselineCapturedAt && <span className="text-xs font-normal text-amber-700">(baseline {relativeTime(report.baselineCapturedAt)})</span>}
        </p>
        <button onClick={onUpdateBaseline} disabled={updating}
          ...
          <RefreshCw size={12} className={updating ? 'animate-spin' : ''} /> {updating ? 'Updating…' : 'Update baseline'}
        </button>
      </div>

      {mappedRemoved.length > 0 && (
        <p className="text-xs text-red-700 bg-red-50 border border-red-200 rounded px-2 py-1">
          ⚠ {mappedRemoved.length} <strong>mapped</strong> field(s) removed upstream — data for these will go stale:
          {' '}{mappedRemoved.map(f => f.referenceName).join(', ')}
        </p>
      )}
      ...
      <div className="grid grid-cols-3 gap-3 text-xs">
        <DriftCol title={`Removed (${report.removed.length})`} color="text-red-700">
          {report.removed.map(f => (
            <li key={f.referenceName} className="truncate">
              <span className="font-mono">{f.referenceName}</span>{f.mapped && <span className="ml-1 text-[10px] px-1 rounded bg-red-100 text-red-700">mapped</span>}
            </li>
          ))}
        </DriftCol>
        <DriftCol title={`Added (${report.added.length})`} color="text-blue-700">
          {report.added.map(f => (
            <li key={f.referenceName} className="truncate">
              <span className="font-mono">{f.referenceName}</span>{f.mapped && <span className="ml-1 text-[10px] px-1 rounded bg-blue-100 text-blue-700">auto-mappable</span>}
            </li>
          ))}
        </DriftCol>
      ...
      {(report.removedStateCategories.length > 0 || report.addedStateCategories.length > 0) && (
        <p className="text-xs text-amber-800">
          ...
          {report.removedStateCategories.length > 0 && <> · removed: {report.removedStateCategories.join(', ')}</>}
          {report.addedStateCategories.length > 0 && <> · added: {report.addedStateCategories.join(', ')}</>}
...
function DriftCol({ title, color, children }: { title: string; color: string; children: React.ReactNode }) {
  return (
    <div>
