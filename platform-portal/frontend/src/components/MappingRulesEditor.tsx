...
export default function MappingRulesEditor({ scope, id }: Props) {
  ...
  const actor =
    (typeof localStorage !== 'undefined' && localStorage.getItem('platform.actor')) || 'portal'
  ...
  const resetMutation = useMutation({
    mutationFn: () =>
      scope === 'ORG' ? api.resetOrgMappingRules(id) : api.resetProjectMappingRules(id),
    onSuccess: () => {
      setDirty(false)
      void qc.invalidateQueries({ queryKey: key })
    },
  ...
  const resetVerb =
    scope === 'PROJECT'
      ? `Reset to ${data.source === 'ORG' ? 'organization' : 'default'}`
      : 'Reset to default'
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={() => {
            if (validate(draft)) saveMutation.mutate()
          }}
        ...
        <button
          onClick={() => {
            if (confirm(`${resetVerb}? This removes this scope's override.`)) resetMutation.mutate()
          }}
      ...
      <p className="text-xs text-slate-400">
        Rules resolve{' '}
        {scope === 'PROJECT'
          ? 'project → organization → built-in default'
