...
/**
 * BFF proxy for Azure DevOps schema discovery + mapping suggestions.
 * Proxies to platform-ingestion's {@code /api/v1/projects/{id}/ado/**}.
 */
@RestController
...
public class PortalAdoDiscoveryController {
    ...
    public Object schema(@PathVariable String projectId, @PathVariable String type,
                         @RequestParam String adoProject) {
        return ingestionClient.get()
                .uri("/api/v1/projects/{p}/ado/work-item-types/{t}/schema?adoProject={a}", projectId, type, adoProject)
                .accept(MediaType.APPLICATION_JSON)
                .retrieve().body(Object.class);
    }

    @GetMapping("/work-item-types/{type}/drift")
    public Object drift(@PathVariable String projectId, @PathVariable String type,
                        @RequestParam String adoProject,
                        @RequestHeader(value = "X-Actor", required = false) String actor) {
        return ingestionClient.get()
                .uri("/api/v1/projects/{p}/ado/work-item-types/{t}/drift?adoProject={a}", projectId, type, adoProject)
    ...
    public Object captureBaseline(@PathVariable String projectId, @PathVariable String type,
                                  @RequestParam String adoProject,
                                  @RequestHeader(value = "X-Actor", required = false) String actor) {
        return ingestionClient.post()
                .uri("/api/v1/projects/{p}/ado/work-item-types/{t}/drift/baseline?adoProject={a}", projectId, type, adoProject)
                .headers(h -> { if (actor != null) h.set("X-Actor", actor); })
                .accept(MediaType.APPLICATION_JSON)
                .retrieve().body(Object.class);
    }
