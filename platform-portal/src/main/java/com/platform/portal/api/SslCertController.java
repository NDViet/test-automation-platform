 ...
 *
 * <p>Users install it once in their OS / browser trust store so that service workers (Playwright
 * Trace Viewer) work without a "SSL certificate error".
 *
 * <p>Windows : double-click → Install → Local Machine → Trusted Root CAs macOS : double-click → add
 * to System keychain → set "Always Trust" Linux : sudo cp portal-cert.crt
 * /usr/local/share/ca-certificates/ && sudo update-ca-certificates
 *
 * <p>Active only when portal.ssl.cert-file is set (i.e. SSL_ENABLED=true).
 */
@RestController
