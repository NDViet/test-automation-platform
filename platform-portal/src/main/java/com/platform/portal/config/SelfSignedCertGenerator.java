 ...
 *
 * <p>Subject Alternative Names are populated at runtime from every non-loopback IPv4 address found
 * on the host's network interfaces, so the cert is valid for whatever LAN IP the machine currently
 * has — no manual keytool step required.
 */
public final class SelfSignedCertGenerator {
  ...
  private static X509Certificate buildCert(KeyPair keyPair) throws Exception {
    ...
    try {
      ...
      /* best-effort — loopback is always included */
    }
