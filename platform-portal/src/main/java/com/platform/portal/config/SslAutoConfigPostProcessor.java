 ...
 *
 * <p>When SSL_ENABLED=true (the default), this generates an in-memory self-signed certificate that
 * covers every local IP address discovered at startup, writes it to a temp PKCS12 file, and injects
 * the server.ssl.* properties so Tomcat picks it up automatically — no manual keytool step
 * required.
 *
 * <p>To disable HTTPS: set the env var SSL_ENABLED=false.
 */
public class SslAutoConfigPostProcessor implements EnvironmentPostProcessor {
  ...
  public void postProcessEnvironment(ConfigurableEnvironment env, SpringApplication app) {
    ...
    try {
      ...
      System.out.println(
          "[Portal SSL] Or download via https://localhost:"
              + env.getProperty("server.port", "8085")
              + "/portal-cert.crt");
