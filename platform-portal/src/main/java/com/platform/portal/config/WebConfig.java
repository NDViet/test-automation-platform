...
public class WebConfig implements WebMvcConfigurer {
    ...
    public RestClient analyticsClient() {
        return RestClient.builder().baseUrl(analyticsUrl)
                .requestFactory(noVerifyRequestFactory()).build();
    }

    @Bean("aiClient")
    public RestClient aiClient() {
        return RestClient.builder().baseUrl(aiUrl)
                .requestFactory(noVerifyRequestFactory()).build();
    }

    @Bean("integrationClient")
    public RestClient integrationClient() {
        return RestClient.builder().baseUrl(integrationUrl)
                .requestFactory(noVerifyRequestFactory()).build();
    }

    @Bean("agentClient")
    public RestClient agentClient() {
        return RestClient.builder().baseUrl(agentUrl)
                .requestFactory(noVerifyRequestFactory()).build();
    }

    /**
     * Returns a request factory backed by a JDK HttpClient that skips TLS
     * certificate and hostname verification. Used for portal → backend service
     * calls so they work even when backend services use self-signed certs.
     *
     * This is intentional for internal service-to-service calls within the
     * platform. Do not use for requests to external (public) endpoints.
     */
    ...
    /**
     * Base dev origins always permitted (Vite dev server variants).
     * Additional origins can be injected at deploy time via the
     * CORS_ALLOWED_ORIGINS env var (comma-separated):
     *   CORS_ALLOWED_ORIGINS=http://192.168.2.10:5173,https://portal.example.com
     *
     * Use a trailing wildcard pattern instead of a literal origin when the
     * port is variable:
    ...
    public void addCorsMappings(CorsRegistry registry) {
        ...
        if (StringUtils.hasText(extraPatternsRaw)) {
            String[] patterns = Arrays.stream(extraPatternsRaw.split(","))
                    .map(String::trim)
                    .filter(StringUtils::hasText)
                    .toArray(String[]::new);
