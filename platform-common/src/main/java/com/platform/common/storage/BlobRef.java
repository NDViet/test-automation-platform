...
/**
 * Immutable reference to a blob stored in the platform object store.
 * Carries enough information to fetch the content without a database lookup.
 * Safe to serialize into JSONB columns or Kafka messages.
 *
 * Keys are content-addressed (SHA-256), so the same bytes always produce
 * the same BlobRef — deduplication is automatic.
 */
