 ...
 *
 * <ul>
 *   <li>{@code R} — the platform canonical record type (e.g. RequirementRecord)
 *   <li>{@code C} — the outbound command/payload type used for push operations
 * </ul>
 *
 * Each implementation is bound to a specific {@link IntegrationType}. Implementations live in
 * {@code platform-integration-config}.
 */
public interface IntegrationAdapter<R, C> {
  ...
  /**
   * Pull records from the external system using the provided config and cursor. Returns a
   * SyncResult containing the records fetched and the next cursor.
   */
  PagedRecords<R> fetchPage(SourceIntegrationConfig config, SyncCursor cursor);

  /**
   * Push a command (create/update/transition) to the external system. Returns the external ID
   * assigned or updated.
   */
  ...
  /**
   * Map a raw webhook event payload to zero or more canonical records. Return empty list if this
   * event type is not relevant to this adapter.
   */
