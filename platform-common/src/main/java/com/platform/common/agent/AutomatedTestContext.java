...
/**
 * Automated test context for the Hub→Node contract. Carries existing test code references, detected
 * coverage gaps, and repo targeting.
 */
public record AutomatedTestContext(
  ...
  public boolean needsNewAutomation() {
    return !coverageGapAcRefs.isEmpty();
  }

  public boolean hasFailuresForHealing() {
    return !failingTestIds.isEmpty();
  }
}
