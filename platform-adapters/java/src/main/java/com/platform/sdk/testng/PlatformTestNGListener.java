...
/**
 * TestNG listener that publishes {@code testng-results.xml} to the platform after the suite
 * finishes.
 *
 * <p>Register in your {@code testng.xml}:
 *
 * <pre>{@code
 * <listeners>
 ...
 * }</pre>
 *
 * <p>Or via {@code @Listeners(PlatformTestNGListener.class)} on any test class.
 */
public class PlatformTestNGListener implements IReporter {
  ...
  public void generateReport(
    ...
    Path reportDir =
        Paths.get(
            reportDirEnv != null && !reportDirEnv.isBlank()
                ? reportDirEnv
                : (outputDirectory != null ? outputDirectory : DEFAULT_REPORT_DIR));
