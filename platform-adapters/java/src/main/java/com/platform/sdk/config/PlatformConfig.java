...
/**
 * Loads SDK configuration from environment variables (priority 1) or {@code platform.properties} on
 * the classpath (priority 2).
 */
public class PlatformConfig {
  ...
  public static PlatformConfig load() {
    ...
    if (enabled && (url == null || apiKey == null)) {
      log.warn(
          "[Platform SDK] platform.url or platform.api-key not configured — SDK will be disabled");
      enabled = false;
    }
  ...
  private static Properties loadPropertiesFile() {
    ...
    try (InputStream is = PlatformConfig.class.getClassLoader().getResourceAsStream(PROPS_FILE)) {
      if (is != null) {
        props.load(is);
      }
    } catch (Exception e) {
  ...
  private static String resolve(
      Properties props, String propKey, String envVar, String defaultValue) {
  ...
  public PlatformConfig withOverrides(String teamOverride, String projectOverride) {
    String effectiveTeam =
        (teamOverride != null && !teamOverride.isBlank()) ? teamOverride : this.teamId;
    String effectiveProject =
        (projectOverride != null && !projectOverride.isBlank()) ? projectOverride : this.projectId;
  ...
  public boolean isValid() {
    return enabled && url != null && apiKey != null && teamId != null && projectId != null;
  }
}
