...
/**
 * JUnit 5 extension that auto-publishes Surefire/Failsafe XML reports after all tests in a class
 * (or top-level suite) have run.
 *
 * <p>Register via {@code @ExtendWith(PlatformReportingExtension.class)} or auto-register via {@code
 * META-INF/services/org.junit.jupiter.api.extension.Extension}.
 *
 * <p>Report directory defaults to {@code target/surefire-reports} but can be overridden with the
 * {@code PLATFORM_REPORT_DIR} environment variable.
 */
public class PlatformReportingExtension implements AfterAllCallback {
  ...
  public void afterAll(ExtensionContext context) {
    ...
    Path reportDir =
        Paths.get(
            reportDirEnv != null && !reportDirEnv.isBlank() ? reportDirEnv : DEFAULT_REPORT_DIR);
