# platform-adapter-java — Usage Guide

`platform-adapter-java` is the zero-friction integration layer. It auto-publishes test results to the platform
**without requiring any changes to your test code**. Pick the adapter for your framework, drop in a
config file, and you're done.

---

## When to use platform-adapter-java vs platform-testkit-java

| | platform-adapter-java | platform-testkit-java |
|---|---|---|
...
`platform-testkit-java` depends on `platform-adapter-java` — you never need both in the same pom.
...
> **Tip:** if you use `platform-testkit-java`'s `PlatformCucumberPlugin` instead, you get native
...
// Upload a natively-constructed UnifiedTestResult (used by platform-testkit-java)
