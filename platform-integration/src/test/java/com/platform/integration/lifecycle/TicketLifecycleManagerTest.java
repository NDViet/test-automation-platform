...
class TicketLifecycleManagerTest {
  ...
  void updatesExistingOpenTicketOngoingFailure() {
    ...
    IssueTrackerLink link =
        new IssueTrackerLink(
            "com.example.Test#login", projectId, "JIRA", "PROJ-1", "https://jira/PROJ-1", "Bug");
    ...
    manager.processRun(
        failingResult("com.example.Test#login"),
        projectId,
        config(teamId, "JIRA", "PROJ"),
        tracker);
  ...
  void closesTicketWhenTestPassesConsecutively() {
    ...
    IssueTrackerPort tracker = mockTracker("JIRA", null);
    manager.processRun(
        passingResult("com.example.Test#login"),
        projectId,
        config(teamId, "JIRA", "PROJ"),
        tracker);
  ...
  void skipsProcessingWhenNoFailuresAndNoLinks() {
    ...
    IssueTrackerPort tracker = mockTracker("JIRA", null);
    manager.processRun(
        passingResult("com.example.Test#ok"), projectId, config(teamId, "JIRA", "PROJ"), tracker);
  ...
  void highPriorityOnMainBranch() {
    ...
    IssueTrackerPort tracker = mockTracker("JIRA", "PROJ-1");
    manager.processRun(
        failingResultOnBranch("com.example.Test#m", "main"),
        projectId,
        config(teamId, "JIRA", "PROJ"),
        tracker);
  ...
  private IssueTrackerPort mockTracker(String type, String newIssueKey) {
    ...
    if (newIssueKey != null) {
      when(tracker.createIssue(any()))
          .thenReturn(
              new IssueReference(newIssueKey, "https://jira/browse/" + newIssueKey, "Open", "Bug"));
    }
    return tracker;
  }
}
