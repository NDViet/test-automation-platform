 ...
 *
 * <ol>
 *   <li><b>Close</b>: test passed ≥ N consecutive runs AND open ticket exists
 *   <li><b>Reopen</b>: test failing AND closed/done ticket exists
 *   <li><b>Update</b>: test failing AND open ticket exists → add occurrence comment
 *   <li><b>Skip</b>: test not currently failing AND no open ticket
 *   <li><b>Create TEST_MAINTENANCE</b>: flakiness score > threshold AND no open ticket
 *   <li><b>Create BUG</b>: consecutive failures ≥ min threshold AND no open ticket
 *   <li><b>Skip</b>: not enough signal yet
 * </ol>
 */
@Component
public class IssueDecisionEngine {
  ...
  public IssueDecision decide(DecisionInput in) {
    ...
    boolean hasOpenTicket =
        in.existingLink().map(l -> !isDoneOrClosed(l.getIssueStatus())).orElse(false);
    boolean hasClosedTicket =
        in.existingLink().map(l -> isDoneOrClosed(l.getIssueStatus())).orElse(false);
    ...
    if (isCurrentlyFailing && hasClosedTicket) {
      return IssueDecision.reopen(
          "Regression detected — test is failing again after "
              + consecutiveFails
    ...
    if (in.flakinessScore() != null
      ...
      return IssueDecision.create(
          ...
          "Flakiness score "
              + in.flakinessScore().getScore()
              + " exceeds threshold "
              + in.flakinessThreshold()
              + " (classification="
              + in.flakinessScore().getClassification()
              + ").");
    }
  ...
  int countConsecutive(List<TestCaseResult> history, boolean lookingForFailures) {
    ...
    for (TestCaseResult r : history) {
      boolean isFail = r.getStatus() == TestStatus.FAILED || r.getStatus() == TestStatus.BROKEN;
