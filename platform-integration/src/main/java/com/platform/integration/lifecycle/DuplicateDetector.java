...
/**
 * Prevents ticket spam by checking whether an open ticket already exists for a given (testId,
 * projectId, trackerType) before creating a new one.
 *
 * <p>The authoritative duplicate check is the {@code issue_tracker_links} table unique constraint.
 * This component provides an early-exit fast path.
 */
@Component
public class DuplicateDetector {
  ...
  public boolean hasOpenTicket(String testId, UUID projectId, String trackerType) {
    return linkRepo
        .findByTestIdAndProjectIdAndTrackerType(testId, projectId, trackerType)
        .map(l -> !isDone(l.getIssueStatus()))
        .orElse(false);
  }

  /** Returns existing link if any (open or closed) for the test. */
  public Optional<IssueTrackerLink> findExisting(
  ...
  /**
   * All test IDs that have an existing ticket (any status) for this project. Used to identify tests
   * that may need close/reopen even if currently passing.
   */
  ...
  public IssueTrackerLink saveOrUpdate(
      String testId,
      UUID projectId,
      String trackerType,
      String issueKey,
    ...
    IssueTrackerLink link =
        new IssueTrackerLink(testId, projectId, trackerType, issueKey, issueUrl, issueType);
    link.syncStatus(newStatus);
    return linkRepo.save(link);
  }
