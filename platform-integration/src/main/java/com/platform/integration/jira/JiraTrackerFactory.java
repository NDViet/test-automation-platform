...
public class JiraTrackerFactory {
    ...
    /**
     * Creates a {@link JiraIssueTracker} from a {@link ResolvedCredential}
     * (the Org→Team→Project cascade). {@code email}/{@code project_key}/transition
     * names are non-secret connection params; {@code apiToken} is the decrypted secret.
     */
    public JiraIssueTracker create(ResolvedCredential cred) {
        ...
        if (email == null || apiToken == null) {
            throw new IllegalArgumentException(
                    "Resolved JIRA credential is missing email or apiToken");
        }
