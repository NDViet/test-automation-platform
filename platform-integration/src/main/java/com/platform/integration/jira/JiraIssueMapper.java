 ...
 *
 * <p>JIRA v3 uses Atlassian Document Format (ADF) for rich-text fields.
 */
@Component
public class JiraIssueMapper {
  ...
  ObjectNode buildAdfDoc(String text) {
    ...
    for (String para : paragraphs) {
      ...
      if (trimmed.startsWith("```") && trimmed.endsWith("```")) {
        ...
        codeBlock.putArray("content").addObject().put("type", "text").put("text", code);
      } else {
  ...
  private void addParagraph(ArrayNode content, String text) {
    ...
    para.putArray("content").addObject().put("type", "text").put("text", text);
  }
