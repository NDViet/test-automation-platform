...
/**
 * Builds an {@link IssueTrackerPort} for a lifecycle integration type from a {@link
 * ResolvedCredential}. Central place that knows which integration types participate in the
 * failure→ticket lifecycle.
 */
@Component
public class TrackerFactoryRegistry {
  ...
  /**
   * Builds a tracker for the given integration type from a resolved credential. Returns empty if
   * the type is unsupported or the credential is incomplete.
   */
  public Optional<IssueTrackerPort> build(String integrationType, ResolvedCredential cred) {
    try {
      ...
      log.warn(
          "[TrackerRegistry] Could not build tracker for {}: {}", integrationType, e.getMessage());
