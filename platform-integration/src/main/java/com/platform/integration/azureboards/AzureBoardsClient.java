 ...
 *
 * <p>Base URL: {@code https://dev.azure.com/{organization}}. Auth:
 * {@code Authorization: Basic base64(":" + pat)} (PAT as password, empty user).</p>
 */
public class AzureBoardsClient {
    ...
    public AzureBoardsClient(String baseUrl, String organization, String project,
                             String pat, ObjectMapper mapper) {
        ...
        this.baseUrl = (organization != null && !root.endsWith("/" + organization))
    ...
    public JsonNode createWorkItem(String type, String jsonPatchBody) {
        String path = "/" + enc(project) + "/_apis/wit/workitems/$" + enc(type)
    ...
    public JsonNode updateWorkItem(String id, String jsonPatchBody) {
        String path = "/" + enc(project) + "/_apis/wit/workitems/" + enc(id)
                + "?api-version=" + API_VERSION;
    ...
    private JsonNode send(String method, String path, String body, String contentType) {
        try {
            ...
            if (resp.statusCode() >= 300) {
                throw new AzureBoardsApiException("Azure Boards " + method + " " + path
