 ...
 *
 * <p>Open work items are located via a {@code platform:testId:{testId}} tag.
 * Closing/reopening sets {@code System.State} to the configured done/reopen states
 * (default {@code Closed}/{@code Active}, which suit the Agile process; override
 * per credential for Basic/Scrum/CMMI processes).</p>
 */
public class AzureBoardsIssueTracker implements IssueTrackerPort {
    ...
    public AzureBoardsIssueTracker(AzureBoardsClient client, AzureBoardsIssueMapper mapper,
                                   String defaultWorkItemType, String areaPath,
                                   String doneState, String reopenState) {
        this.client              = client;
        this.mapper              = mapper;
    ...
    public IssueReference createIssue(IssueRequest request) {
        String type = (request.issueType() != null && !request.issueType().isBlank())
                ? request.issueType() : defaultWorkItemType;
    ...
    public Optional<IssueReference> findOpenIssue(String testId, String projectKey) {
        ...
        String wiql = "SELECT [System.Id] FROM workitems"
                + " WHERE [System.Tags] CONTAINS '" + tag.replace("'", "''") + "'"
                + " AND [System.State] <> '" + doneState.replace("'", "''") + "'"
