 ...
 *
 * <p>Expected connection params: {@code organization}, {@code project} (or {@code project_key}),
 * optional {@code area_path}, {@code workItemType} (default Bug), {@code doneState} (default
 * Closed), {@code reopenState} (default Active). Secret: {@code pat}.
 */
@Component
public class AzureBoardsTrackerFactory {
  ...
  public AzureBoardsIssueTracker create(ResolvedCredential cred) {
    ...
    AzureBoardsClient client =
        new AzureBoardsClient(cred.baseUrl(), organization, project, pat, objectMapper);

    return new AzureBoardsIssueTracker(
        client,
        mapper,
