 ...
 *
 * <p>Base URL: {@code https://api.github.com} (or a GitHub Enterprise base). Auth: {@code
...
public class GitHubIssuesClient {
  ...
  public GitHubIssuesClient(
      String baseUrl, String owner, String repo, String token, ObjectMapper mapper) {
  ...
  public JsonNode addComment(String number, String body) {
    ...
    return send(
        "POST", "/repos/" + owner + "/" + repo + "/issues/" + number + "/comments", payload);
  }
  ...
  public JsonNode searchByLabel(String label, String state) {
    String q =
        "repo:" + owner + "/" + repo + " is:issue state:" + state + " label:\"" + label + "\"";
    String path =
        "/search/issues?q=" + URLEncoder.encode(q, StandardCharsets.UTF_8) + "&per_page=5";
  ...
  private JsonNode send(String method, String path, String body) {
    try {
      ...
      throw new GitHubApiException(
          "GitHub " + method + " " + path + " failed: " + e.getMessage(), e);
    }
  }
