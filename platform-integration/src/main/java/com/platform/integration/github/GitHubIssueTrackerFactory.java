 ...
 *
 * <p>Expected connection params: {@code owner}, {@code repo} (or a single {@code repository} =
 * "owner/repo"). Secret: {@code pat} (or {@code token}). Optional {@code base_url} for GitHub
 * Enterprise.
 */
@Component
public class GitHubIssueTrackerFactory {
  ...
  public GitHubIssueTracker create(ResolvedCredential cred) {
    ...
    if (owner == null || repo == null || token == null) {
      throw new IllegalArgumentException("GitHub Issues credential requires owner, repo and pat");
    }

    GitHubIssuesClient client =
        new GitHubIssuesClient(cred.baseUrl(), owner, repo, token, objectMapper);
