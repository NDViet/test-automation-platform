 ...
 *
 * <p>Primary path is cascade-driven: for each lifecycle integration type, resolve
 * the effective credential (Org→Team→Project) and run the lifecycle if one exists.
 * If no credentials are configured at all for the project, falls back to the legacy
 * per-team {@link IntegrationConfig} JIRA path so existing installs keep working.</p>
 */
@Component
public class IntegrationEventConsumer {
    ...
    public IntegrationEventConsumer(OrganizationRepository orgRepo,
    ...
    private int runCascade(UnifiedTestResult result, UUID projectId) {
        ...
        for (String type : trackerRegistry.lifecycleTypes()) {
            ...
            if (!cred.hasSecret()) {
                log.debug("[Integration] Resolved {} for project={} but no secret — skipping",
                        type, projectId);
                continue;
            }
            ...
            try {
                ...
                log.warn("[Integration] Lifecycle failed for type={} run={}: {}",
                        type, result.runId(), e.getMessage());
            }
        }
        return run;
    }
     ...
     *
     * <p>ADO-first: teams are now sub-entities of a project, so legacy configs are
     * looked up across every sub-team of the resolved project.</p>
     */
        ...
        for (Team team : teams) {
            ...
            for (IntegrationConfig config : configs) {
                try {
                    ...
                    log.warn("[Integration] Legacy processing failed run={} tracker={}: {}",
    ...
    /**
     * Resolve the project from the incoming slugs. ADO-first: {@code orgSlug} is the
     * top-level organization slug and {@code projectSlug} is unique within that org.
     */
        ...
        return switch (config.getTrackerType()) {
            case "JIRA" -> {
                try { yield jiraFactory.create(config); }
                    ...
                    log.warn("[Integration] Could not build legacy JIRA tracker for team={}: {}",
